...
TL;DR: 提出UniCompress压缩视觉token，在统一多模态模型中将token减至1/4并显著提速降耗，性能基本保持。


<details>
  <summary>Details</summary>
Motivation: 统一模型虽简化架构并促进跨模态共享，但大量视觉token带来高计算与内存负担，阻碍资源受限场景部署。

Method: 提出UniCompress统一token压缩算法，引入带可学习全局meta tokens的压缩/解压机制，可模块化集成到现有统一自回归模型中，无需完全重训。

Result: 视觉token数量最多减少4倍，推理延迟与训练成本显著降低，性能仅有轻微下降。

Conclusion: UniCompress以轻量插件方式压缩视觉token，在保持理解与生成性能的同时显著降低计算与内存开销，适合资源受限多模态应用。
...
TL;DR: DORI以人类朝向认知分级评测模型的物体朝向理解，结果显示现有模型在朝向推理上显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基准将朝向与位置和场景理解混淆，难以单独评估朝向认知能力。

Method: 构建DORI层级基准，将朝向分为四个维度并在粗粒度与细粒度两级评估，基于13,652张图像生成33,656道选择题，采用框选、统一参照系与结构化提示控制混淆因素，并评测24个视觉语言模型。

Result: 模型在一般空间基准表现好，但在物体朝向任务接近随机；最佳模型粗粒度54.2%、细粒度45.0%，在复合旋转与参考系变换上失败明显，粗细差距显示依赖类别启发而非几何推理。

Conclusion: 提出DORI基准揭示当前多模态模型在物体朝向理解上显著不足，朝向推理仍是未解决挑战。
...
TL;DR: DART通过共享类无关主干与批量解码将SAM3改造成实时多类检测器，速度大幅提升且精度保持竞争力。


<details>
  <summary>Details</summary>
Motivation: SAM3一次仅处理单一文本提示，N类需N次前向，计算主要受439M主干限制，难以实时多类检测。

Method: 利用SAM3视觉主干类无关的结构不变性共享主干计算，结合批量多类解码、仅检测推理与TensorRT FP16部署，并探索冻结编解码器的适配器蒸馏以降时延。

Result: 在COCO val2017上达55.8 AP、15.8 FPS（4类，1008×1008，RTX 4080）；速度3类提升5.6×、80类提升25×；极低时延下可达38.7 AP且主干13.9 ms。

Conclusion: DART在不改模型权重的前提下显著提升SAM3多类检测速度与实时性。
...
TL;DR: SPEGC用语义提示增强+可微图聚类提炼高阶结构，引导CTTA稳健自适应，显著提升医学分割性能。


<details>
  <summary>Details</summary>
Motivation: CTTA在无标注域持续变化下易因不可靠监督信号产生误差累积并导致性能崩溃，需要更稳健的特征与结构约束。

Method: 提出语义提示特征增强：解耦共性/异质提示池注入全局上下文；设计可微图聚类求解器，将边稀疏化重构为最优传输以提炼高阶结构表示；用该结构表示引导簇级一致性与动态决策边界以实现持续测试时自适应。

Result: 在两个医学图像分割基准上，SPEGC优于现有SOTA CTTA方法。

Conclusion: SPEGC通过语义提示增强与可微图聚类获得稳健结构表示，在CTTA医学分割中显著优于现有方法。
...
### [35] [OSCBench: Benchmarking Object State Change in Text-to-Video Generation](https://arxiv.org/abs/2603.11698)
...
### [36] [Think While Watching: Online Streaming Segment-Level Memory for Multi-Turn Video Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2603.11896)
...
TL;DR: 用无排序渲染+视角增强+压缩，使3DGS在移动端实时且高质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS在移动端部署受限于高计算与大存储开销，尤其是深度排序导致的alpha blending瓶颈。

Method: 提出深度感知的无序渲染替代深度排序的alpha blending；设计视角相关神经增强以补偿重叠透明伪影；采用一阶球谐蒸馏、神经向量量化与基于贡献的剪枝进行压缩。

Result: 实现实时渲染与紧凑模型规模，同时保持高视觉质量。

Conclusion: Mobile-GS通过无排序的深度感知渲染、视角相关增强与模型压缩，在移动端实现高质量实时渲染。
...
TL;DR: TornadoNet基准对比YOLO与RT-DETR在多级建筑损伤检测；YOLO更快更准，RT-DETR更稳于等级排序；有序监督显著提升RT-DETR的mAP与有序一致性。


<details>
  <summary>Details</summary>
Motivation: 现有灾后损伤检测缺乏受控基准来系统评估架构与损失设计对多级损伤检测的影响，且损伤等级具有有序性，需更合适的监督策略。

Method: 构建TornadoNet基准，使用2021年龙卷风后3333张街景图像、8890个建筑实例，基于五级损伤标注；比较YOLO系CNN检测器与RT-DETR等Transformer检测器，在统一训练协议下评估，并引入软有序分类目标与有序距离惩罚进行监督对齐。

Result: YOLO在mAP与速度上最佳（最高46.05% mAP@0.5，66–276 FPS）；RT-DETR在有序一致性更优（88.13% Ordinal Top-1, MAOE 0.65）。引入有序监督后RT-DETR mAP提升至44.70%（+4.8个百分点），有序指标提升至91.15%与MAOE 0.56。

Conclusion: 该工作证明在街景建筑损伤评估中，损伤等级的有序监督与检测器架构协同能显著提升损伤严重度判定的可靠性。
...
TL;DR: R4Det通过深度协同、无位姿时序融合和实例引导细化提升雷达-相机3D检测，取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有雷达-相机3D检测在深度估计不稳、位姿缺失导致时序融合失效、稀疏雷达难以覆盖小目标等方面存在问题。

Method: 提出R4Det，包含全景深度融合模块以协同绝对/相对深度；无位姿依赖的可变形门控时序融合模块；基于2D实例引导的动态细化模块提取语义原型。

Result: 在TJ4DRadSet与VoD数据集上取得最优3D检测性能，并提升深度与时序融合鲁棒性。

Conclusion: R4Det在TJ4DRadSet与VoD上达到SOTA，且在深度估计、时序融合与小目标检测上更稳健。
...
TL;DR: 发现MLLM置信度失准，提出CDRL与CA-TTS进行置信度校准和测试时扩展，带来稳定性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM侧重提升视觉感知精度，但缺乏对“何时不知道”的自知能力，导致置信度严重失准。

Method: 通过置信度探测实验揭示失准问题；提出CDRL，使用原图-噪声图对与置信度奖励进行强化学习；提出CA-TTS，在测试时基于置信度动态调度自一致、自反思与视觉自检，并由专家模型充当规划者/批评者/投票者提供外部验证。

Result: 在四个基准上取得一致的8.8%提升；消融表明各模块有效且测试时扩展优越。

Conclusion: 提出的CDRL与CA-TTS框架能稳定校准MLLM置信度并显著提升多基准性能。
...
### [48] [EndoCoT: Scaling Endogenous Chain-of-Thought Reasoning in Diffusion Models](https://arxiv.org/abs/2603.12252)
...
### [49] [Towards Universal Computational Aberration Correction in Photographic Cameras: A Comprehensive Benchmark Analysis](https://arxiv.org/abs/2603.12083)
...
### [51] [LaMoGen: Language to Motion Generation Through LLM-Guided Symbolic Inference](https://arxiv.org/abs/2603.11605)
...
### [52] [Articulat3D: Reconstructing Articulated Digital Twins From Monocular Videos with Geometric and Motion Constraints](https://arxiv.org/abs/2603.11606)
...
### [53] [DyWeight: Dynamic Gradient Weighting for Few-Step Diffusion Sampling](https://arxiv.org/abs/2603.11607)
...
### [54] [SemiTooth: a Generalizable Semi-supervised Framework for Multi-Source Tooth Segmentation](https://arxiv.org/abs/2603.11616)
...
### [55] [Noise-aware few-shot learning through bi-directional multi-view prompt alignment](https://arxiv.org/abs/2603.11617)
*Lu Niu,Cheng Xue*

Main category: cs.CV

TL;DR: NA-MVP通过多视角与双向提示及OT对齐/精炼，有效抑制噪声并提升少样本VLM鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有提示调优在噪声标签下易被污染，难以建模细粒度语义并自适应区分干净与噪声信号，导致跨模态对齐退化。

Method: 提出NA-MVP：多视角提示+不平衡最优传输实现细粒度patch-提示对齐并抑制不可靠区域；双向提示捕获干净与噪声感知线索；基于对齐的选择性精炼用OT仅纠正错标样本、保留可靠数据。

Result: 在合成与真实噪声基准上持续优于SOTA基线，证明对噪声监督下少样本学习的有效性。

Conclusion: NA-MVP在噪声监督下的少样本视觉-语言学习中显著提升鲁棒性与性能。
...
### [56] [Shape-of-You: Fused Gromov-Wasserstein Optimal Transport for Semantic Correspondence in-the-Wild](https://arxiv.org/abs/2603.11618)
...
### [58] [Developing Foundation Models for Universal Segmentation from 3D Whole-Body Positron Emission Tomography](https://arxiv.org/abs/2603.11627)
...
### [59] [O3N: Omnidirectional Open-Vocabulary Occupancy Prediction](https://arxiv.org/abs/2603.12144)
...
TL;DR: 提出条件扩散生成框架PolyCrysDiff，实现高保真、可控的3D多晶微结构生成并验证其物理可计算性与性能分析价值。


<details>
  <summary>Details</summary>
Motivation: 3D多晶微结构对材料性能关键，但真实可控构建困难，限制了结构-性能关系研究。

Method: 基于条件潜变量扩散模型的端到端生成框架，控制晶粒形貌与取向分布，并通过CPFEM验证可计算性与物理有效性。

Result: 在晶粒属性控制上R^2>0.972，准确复现形貌、取向分布和空间相关性；CPFEM验证生成结构可用于力学仿真。

Conclusion: 该工作提出的PolyCrysDiff可生成可计算、物理有效的3D多晶微结构，并优于传统MRF/CNN方法，支持材料性能分析与设计。
...
TL;DR: 综述96篇论文，六类GAN在多孔材料重建上显著提升精度与规模，但仍受效率与结构连续性限制。


<details>
  <summary>Details</summary>
Motivation: 评估深度学习尤其GAN在多孔材料数字重建中的发展与应用，以指导架构选择。

Method: 系统性综述2017至2026初的96篇同行评审论文，分类比较六类GAN架构及其应用表现。

Result: 孔隙度精度提升至与原样本误差约1%，渗透率预测相对误差降低至最高79%，可重建体积从64^3提升到2200^3体素。

Conclusion: GAN 在多孔材料重建中取得显著进展，但在计算效率、内存与2D到3D结构连续性上仍有挑战。
...
TL;DR: 将拼接对齐扩展到3D空间并用Transformer聚合全局信息，显著改善全景拼接质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D成对特征，难以利用多视几何一致性，导致弱纹理、大视差和重复纹理场景下失真与错位。

Method: 采用3D感知的Transformer架构，利用相机位姿进行3D空间全局对齐，并通过多特征联合优化计算缝合线。

Result: 在大规模真实场景数据集上的实验显著优于现有方法，提升对齐精度与感知质量。

Conclusion: 提出基于3D几何一致性的全局对齐框架与数据集，提升拼接质量。
...
TL;DR: 提出Nyxus：面向大规模2D/3D图像的高效、可扩展且标准化的特征提取库，支持多种使用方式与可编程调优。


<details>
  <summary>Details</summary>
Motivation: 大规模图像数据处理受限于计算效率与特征库标准不一，难以在准确性、鲁棒性与可比性之间平衡。

Method: 从零设计面向CPU/GPU的可扩展外存特征提取库，覆盖多领域特征集，严格对标标准；并提供Python包、CLI、Napari插件及OCI容器等多种形态。

Result: 实现了覆盖放射组学与细胞分析等领域的综合特征集，支持大规模数据处理与多平台部署，并引入可编程特征集调优以提升效率或覆盖。

Conclusion: Nyxus为大规模2D/3D图像特征提取提供了可扩展、标准化且易用的解决方案，并支持多场景部署与方法学上的可调优。
...
TL;DR: 提出多相机两阶段优化框架，结合时空图与宽基线光流，实现更鲁棒的动态场景重建与位姿估计，并推出新数据集。


<details>
  <summary>Details</summary>
Motivation: 解决多自由移动相机下动态场景重建与位姿估计，克服单相机或刚性标定相机阵列的限制。

Method: 两阶段优化：先构建时空连接图进行多相机SLAM跟踪与尺度一致性，结合宽基线初始化；再用宽基线光流进行密集深度与位姿一致性优化。

Result: 在合成与真实数据集上性能显著提升，且内存开销更低；发布MultiCamRobolab数据集。

Conclusion: 提出的两阶段优化框架在多自由运动相机的动态场景重建与位姿估计中显著优于现有前馈模型，并减少内存需求。
...
### [99] [Paper LoV3D: Grounding Cognitive Prognosis Reasoning in Longitudinal 3D Brain MRI via Regional Volume Assessments](https://arxiv.org/abs/2603.12071)
...
TL;DR: Node-RF将NODE与动态NeRF融合，学习连续时空动态并实现长程外推与泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅在观测边界内建模，难以对训练序列外的动态进行可靠外推。

Method: 从视觉输入学习隐式场景状态，利用ODE求解器进行时间演化与特征传播，NeRF渲染器据此合成任意视角，实现长程外推。

Result: 在多序列共享动力学训练下，可泛化到未见条件，实验表明能刻画抽象系统行为并预测未来关键状态。

Conclusion: Node-RF通过将NODE与动态NeRF结合，实现连续时空表示与远距外推，并能在无显式模型下识别关键点。
...
TL;DR: 提出FlashMotion与FlashBench，通过先训适配器、再蒸馏生成器、最后混合目标微调，获得少步高质且轨迹准确的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹可控视频生成依赖多步去噪，计算冗余大；直接蒸馏会损失画质与轨迹精度，需要新的少步训练框架。

Method: 先在多步视频生成器上训练轨迹适配器；再将生成器蒸馏为少步版本加速生成；最后用结合扩散与对抗目标的混合策略微调适配器以适配少步生成器。

Result: 在两种适配器架构上，FlashMotion在视觉质量与轨迹一致性上均超过现有蒸馏方法与多步模型，并提出FlashBench进行评测。

Conclusion: FlashMotion通过适配器微调与少步生成器对齐，实现高质量且轨迹准确的视频生成，并在多项指标上优于现有方法。
...
TL;DR: 提出学科知识驱动的图像编辑基准GRADE及多维评测，系统评估20个模型并发现其在知识密集编辑上的明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑评测多聚焦自然图像与浅层常识推理，难以衡量结构化学科约束下的能力。

Method: 构建涵盖10个学科、520样本的GRADE基准，并提出包含学科推理、视觉一致性、逻辑可读性的多维评测协议，在20个开源与闭源模型上进行实验与消融分析。

Result: 在隐式、知识密集的编辑设置下，多数模型表现受限，存在显著性能差距，并暴露出学科编辑的关键限制因素。

Conclusion: GRADE基准揭示统一多模态模型在学科知识驱动的隐式图像编辑上存在显著短板，并为未来发展指明方向。
...
TL;DR: ARACH在推理时重分配注意力，无需训练即可提升LLM表现并减轻注意力沉陷。


<details>
  <summary>Details</summary>
Motivation: 训练代价高昂，现有免训练方法多停留在输入/输出层面，缺少可插拔的内部计算干预。

Method: 提出ARACH，在推理时引入自适应上下文枢纽聚合上下文并重新分配注意力。

Result: 在多种语言建模任务上取得一致提升，推理开销 modest，且缓解注意力沉陷现象。

Conclusion: 通过在推理时干预模型内部注意力计算的插件式机制，可在不更新参数的情况下稳定提升性能。
...
TL;DR: LLM可可靠分析波斯诗歌；鲁米诗歌更积极且格律表达更丰富。


<details>
  <summary>Details</summary>
Motivation: 评估现代语言模型理解波斯诗歌复杂性的能力，并探索情感与格律的相关性。

Method: 使用多种基于BERT与GPT的语言模型，对鲁米与帕尔文·埃特萨米诗歌进行情感与格律关联分析。

Result: GPT-4o在波斯诗歌分析中表现可靠；鲁米诗歌整体更积极，且在格律表达情感多样性上优于帕尔文。

Conclusion: 大型语言模型可有效用于波斯诗歌语义分析，减少人工解读偏差。
...
### [144] [Where Matters More Than What: Decoding-aligned KV Cache Compression via Position-aware Pseudo Queries](https://arxiv.org/abs/2603.11564)
...
### [145] [Streaming Translation and Transcription Through Speech-to-Text Causal Alignment](https://arxiv.org/abs/2603.11578)
...
### [146] [UtilityMax Prompting: A Formal Framework for Multi-Objective Large Language Model Optimization](https://arxiv.org/abs/2603.11583)
*Ofir Marom*
...
### [147] [Performance Evaluation of Open-Source Large Language Models for Assisting Pathology Report Writing in Japanese](https://arxiv.org/abs/2603.11597)
...
### [148] [QChunker: Learning Question-Aware Text Chunking for Domain RAG via Multi-Agent Debate](https://arxiv.org/abs/2603.11650)
...
### [149] [Multi-Task Reinforcement Learning for Enhanced Multimodal LLM-as-a-Judge](https://arxiv.org/abs/2603.11665)
...
### [150] [A technology-oriented mapping of the language and translation industry: Analysing stakeholder values and their potential implication for translation pedagogy](https://arxiv.org/abs/2603.11667)
...
### [151] [In the LLM era, Word Sense Induction remains unsolved](https://arxiv.org/abs/2603.11686)
...
### [152] [SemBench: A Universal Semantic Framework for LLM Evaluation](https://arxiv.org/abs/2603.11687)
...
### [153] [Semi-Synthetic Parallel Data for Translation Quality Estimation: A Case Study of Dataset Building for an Under-Resourced Language Pair](https://arxiv.org/abs/2603.11743)
...
### [154] [Compression Favors Consistency, Not Truth: When and Why Language Models Prefer Correct Information](https://arxiv.org/abs/2603.11749)
...
### [155] [Legal-DC: Benchmarking Retrieval-Augmented Generation for Legal Documents](https://arxiv.org/abs/2603.11772)
*Yaocong Li,Qiang Lan,Leihan Zhang,Le Zhang*

Main category: cs.CL

TL;DR: 构建Legal-DC与LegRAG，配合自动评测，显著提升中文法律RAG效果。


<details>
  <summary>Details</summary>
Motivation: 现有中文法律RAG缺少检索—生成联合评测基准，且主流RAG难以适配法律条文的结构化特点。

Method: 构建Legal-DC基准（480份法律文档、2475组QA、条款级标注）；提出LegRAG（条款边界分段的法律自适应索引+双路径自反思机制）；设计LLM自动评测方案。

Result: LegRAG在关键指标上较SOTA提升1.3%–5.6%。

Conclusion: 研究提供了面向中文法律RAG的专用基准、框架与自动评测方法，证明LegRAG在多项指标上优于现有方法。
...
### [156] [Trust Oriented Explainable AI for Fake News Detection](https://arxiv.org/abs/2603.11778)
...
### [157] [Large Language Models for Biomedical Article Classification](https://arxiv.org/abs/2603.11780)
...
### [158] [DatedGPT: Preventing Lookahead Bias in Large Language Models with Time-Aware Pretraining](https://arxiv.org/abs/2603.11838)
...
### [159] [Bielik-Minitron-7B: Compressing Large Language Models via Structured Pruning and Knowledge Distillation for the Polish Language](https://arxiv.org/abs/2603.11881)
...
### [160] [CoMMET: To What Extent Can LLMs Perform Theory of Mind Tasks?](https://arxiv.org/abs/2603.11915)
...
### [161] [PersonaTrace: Synthesizing Realistic Digital Footprints with LLM Agents](https://arxiv.org/abs/2603.11955)
...
### [162] [CHiL(L)Grader: Calibrated Human-in-the-Loop Short-Answer Grading](https://arxiv.org/abs/2603.11957)
...
### [163] [Just Use XML: Revisiting Joint Translation and Label Projection](https://arxiv.org/abs/2603.12021)
...
### [164] [Translationese as a Rational Response to Translation Task Difficulty](https://arxiv.org/abs/2603.12050)
*Maria Kunilovskaya*
...
### [165] [To Words and Beyond: Probing Large Language Models for Sentence-Level Psycholinguistic Norms of Memorability and Reading Times](https://arxiv.org/abs/2603.12105)
...
### [166] [SommBench: Assessing Sommelier Expertise of Language Models](https://arxiv.org/abs/2603.12117)
...
### [167] [Cross-Context Review: Improving LLM Output Quality by Separating Production and Review Sessions](https://arxiv.org/abs/2603.12123)
*Tae-Eun Song*
...
### [168] [LifeSim: Long-Horizon User Life Simulator for Personalized Assistant Evaluation](https://arxiv.org/abs/2603.12152)
...
### [169] [QAQ: Bidirectional Semantic Coherence for Selecting High-Quality Synthetic Code Instructions](https://arxiv.org/abs/2603.12165)
...
### [170] [Strategic Navigation or Stochastic Search? How Agents and Humans Reason Over Document Collections](https://arxiv.org/abs/2603.12180)
...
### [171] [Long-Context Encoder Models for Polish Language Understanding](https://arxiv.org/abs/2603.12191)
...
### [172] [IndexCache: Accelerating Sparse Attention via Cross-Layer Index Reuse](https://arxiv.org/abs/2603.12201)
...
### [173] [Sparking Scientific Creativity via LLM-Driven Interdisciplinary Inspiration](https://arxiv.org/abs/2603.12226)
...
### [174] [SciMDR: Benchmarking and Advancing Scientific Multimodal Document Reasoning](https://arxiv.org/abs/2603.12249)
...
### [175] [From Phase Prediction to Phase Design: A ReAct Agent Framework for High-Entropy Alloy Discovery](https://arxiv.org/abs/2603.11068)
...
### [176] [Proof-Carrying Materials: Falsifiable Safety Certificates for Machine-Learned Interatomic Potentials](https://arxiv.org/abs/2603.12183)
*Abhinaba Basu,Pavan Chakraborty*

Main category: cond-mat.mtrl-sci

TL;DR: PCM通过对抗证伪、置信包络与形式化认证显著降低MLIP漏检并提升材料发现效率。


<details>
  <summary>Details</summary>
Motivation: 单一MLIP用于高通量筛选缺乏可靠性保证且漏检严重，需要系统化的可靠性评估与形式化认证。

Method: 提出PCM三阶段流程：对组成空间进行对抗性证伪、基于自举的置信包络细化（95%CI）、以及Lean 4形式化认证；并对CHGNet/TensorNet/MACE进行审计与跨架构风险建模。

Result: 单一MLIP召回率仅0.07；架构间误差相关性极低；PCM特征风险模型AUC-ROC 0.938且可跨架构迁移；在热电筛选中额外发现62个稳定材料（+25%）。

Conclusion: PCM可显著提升MLIP筛选可靠性与材料发现产出，并提供形式化可信保证。
...
### [177] [Scaling Reasoning Efficiently via Relaxed On-Policy Distillation](https://arxiv.org/abs/2603.11137)
...
### [178] [Huntington Disease Automatic Speech Recognition with Biomarker Supervision](https://arxiv.org/abs/2603.11168)
...
### [179] [Hindsight-Anchored Policy Optimization: Turning Failure into Feedback in Sparse Reward Settings](https://arxiv.org/abs/2603.11321)
...
### [180] [Meta-Reinforcement Learning with Self-Reflection for Agentic Search](https://arxiv.org/abs/2603.11327)
...
### [182] [Fractional Rotation, Full Potential? Investigating Performance and Convergence of Partial RoPE](https://arxiv.org/abs/2603.11611)
...
### [183] [Chem4DLLM: 4D Multimodal LLMs for Chemical Dynamics Understanding](https://arxiv.org/abs/2603.11924)
...
### [184] [Simple Recipe Works: Vision-Language-Action Models are Natural Continual Learners with Reinforcement Learning](https://arxiv.org/abs/2603.11653)
...
### [186] [Structure-Aware Epistemic Uncertainty Quantification for Neural Operator PDE Surrogates](https://arxiv.org/abs/2603.11052)
...
### [187] [Graph Tokenization for Bridging Graphs and Transformers](https://arxiv.org/abs/2603.11099)
...
### [188] [Task-Conditioned Routing Signatures in Sparse Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.11114)
...
### [189] [Procedural Fairness via Group Counterfactual Explanation](https://arxiv.org/abs/2603.11140)
...
### [190] [Attention Gathers, MLPs Compose: A Causal Analysis of an Action-Outcome Circuit in VideoViT](https://arxiv.org/abs/2603.11142)
*Sai V R Chereddy*
...
TL;DR: CoRe通过低秩表示空间干预实现稳定且高效的持续学习，效果优于现有PEFT。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT依赖权重层面的黑箱优化，缺乏对表示漂移的控制，易受域偏移与灾难性遗忘影响。

Method: 提出CoRe框架，将微调从权重空间转为表示空间，在低秩线性子空间中进行任务特定干预，并设置显式目标以平衡稳定性与可塑性。

Result: 在多项持续学习基准上，CoRe在保持参数高效的同时显著优于SOTA方法。

Conclusion: 该工作提出表示空间微调范式，更稳定且高效，并在持续学习中优于现有方法。
...
### [193] [ARROW: Augmented Replay for RObust World models](https://arxiv.org/abs/2603.11395)
...
### [195] [Grammar of the Wave: Towards Explainable Multivariate Time Series Event Detection via Neuro-Symbolic VLM Agents](https://arxiv.org/abs/2603.11479)
...
### [196] [KEPo: Knowledge Evolution Poison on Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2603.11501)
...
### [197] [Survival Meets Classification: A Novel Framework for Early Risk Prediction Models of Chronic Diseases](https://arxiv.org/abs/2603.11598)
...
### [198] [Entropy-Preserving Reinforcement Learning](https://arxiv.org/abs/2603.11682)
...
### [199] [Harnessing Data Asymmetry: Manifold Learning in the Finsler World](https://arxiv.org/abs/2603.11396)
...
### [200] [Exploiting Expertise of Non-Expert and Diverse Agents in Social Bandit Learning: A Free Energy Approach](https://arxiv.org/abs/2603.11757)
...
### [201] [The Latent Color Subspace: Emergent Order in High-Dimensional Chaos](https://arxiv.org/abs/2603.12261)
...
### [204] [Effective Resistance Rewiring: A Simple Topological Correction for Over-Squashing](https://arxiv.org/abs/2603.11944)
...
### [206] [Flowcean - Model Learning for Cyber-Physical Systems](https://arxiv.org/abs/2603.12015)
...
### [207] [Slow-Fast Inference: Training-Free Inference Acceleration via Within-Sentence Support Stability](https://arxiv.org/abs/2603.12038)
...
### [208] [Chemical Reaction Networks Learn Better than Spiking Neural Networks](https://arxiv.org/abs/2603.12060)
...
### [209] [A Multi-Label Temporal Convolutional Framework for Transcription Factor Binding Characterization](https://arxiv.org/abs/2603.12073)
...
### [210] [Resource-Efficient Iterative LLM-Based NAS with Feedback Memory](https://arxiv.org/abs/2603.12091)
...
### [211] [Taming the Adversary: Stable Minimax Deep Deterministic Policy Gradient via Fractional Objectives](https://arxiv.org/abs/2603.12110)
...
### [212] [Automatic Generation of High-Performance RL Environments](https://arxiv.org/abs/2603.12145)
...
### [213] [IsoCompute Playbook: Optimally Scaling Sampling Compute for LLM RL](https://arxiv.org/abs/2603.12151)
...
### [214] [A Quantitative Characterization of Forgetting in Post-Training](https://arxiv.org/abs/2603.12163)
...
### [215] [Neural Thickets: Diverse Task Experts Are Dense Around Pretrained Weights](https://arxiv.org/abs/2603.12228)
...
### [216] [Security Considerations for Artificial Intelligence Agents](https://arxiv.org/abs/2603.12230)
...
### [217] [Separable neural architectures as a primitive for unified predictive and generative intelligence](https://arxiv.org/abs/2603.12244)
...
### [218] [Exploring Collatz Dynamics with Human-LLM Collaboration](https://arxiv.org/abs/2603.11066)
*Edward Y. Chang*
...
### [219] [Deep Learning-based Assessment of the Relation Between the Third Molar and Mandibular Canal on Panoramic Radiographs using Local, Centralized, and Federated Learning](https://arxiv.org/abs/2603.11850)
...
### [220] [Unifying Logical and Physical Layout Representations via Heterogeneous Graphs for Circuit Congestion Prediction](https://arxiv.org/abs/2603.11075)
...
### [221] [Incremental Neural Network Verification via Learned Conflicts](https://arxiv.org/abs/2603.12232)
...
### [222] [The Landscape of Generative AI in Information Systems: A Synthesis of Secondary Reviews and Research Agendas](https://arxiv.org/abs/2603.11842)
...
### [223] [A Diffeomorphism Groupoid and Algebroid Framework for Discontinuous Image Registration](https://arxiv.org/abs/2603.11806)
*Lili Bao,Bin Xiao,Shihui Ying,Stefan Sommer*

Main category: math.GR

TL;DR: 将LDDMM推广到群胚/代数框架以处理滑移间断，推导最优流方程并以数值实验验证。


<details>
  <summary>Details</summary>
Motivation: 传统LDDMM要求速度场连续平滑，难以处理存在滑移边界的间断运动。

Method: 将LDDMM从Lie群推广到间断微分同胚Lie群胚，构建对应Lie代数丛与对偶结构，推导Euler–Arnold方程以描述最优变形流。

Result: 建立了可在同质区域内保持微分同胚、在滑移边界允许间断的注册框架，并给出理论分析与数值测试。

Conclusion: 提出的分段微分同胚群胚/代数框架可处理滑移边界的间断变形，并给出相应最优流方程与数值验证。
...
### [224] [Stage-Adaptive Reliability Modeling for Continuous Valence-Arousal Estimation](https://arxiv.org/abs/2603.11468)
*Yubeen Lee,Sangeun Lee,Junyeop Cha,Eunil Park*

Main category: cs.MM

TL;DR: SAGE通过阶段自适应的模态可靠性估计与融合，提升真实场景连续情感预测性能。


<details>
  <summary>Details</summary>
Motivation: 现实环境中模态可靠性随交互阶段变化显著，现有方法多仅建模时序动态，忽视阶段性可靠性波动。

Method: 提出SAGE框架：显式估计并校准各模态在不同交互阶段的可靠性，将可靠性估计与特征表征分离，并在融合时动态重加权音频与视觉表示。

Result: 在Aff-Wild2基准上，SAGE相较现有多模态融合方法取得更高的CCC分数，且在噪声、遮挡、交互变化下更稳定。

Conclusion: SAGE通过阶段自适应的可靠性建模与融合，能在真实场景中稳定提升连续情感估计效果。
...
### [225] [Catalogue Grounded Multimodal Attribution for Museum Video under Resource and Regulatory Constraints](https://arxiv.org/abs/2603.11147)
...
### [226] [OmniForcing: Unleashing Real-time Joint Audio-Visual Generation](https://arxiv.org/abs/2603.11647)
...
### [227] [The Attack and Defense Landscape of Agentic AI: A Comprehensive Survey](https://arxiv.org/abs/2603.11088)
...
### [228] [WebWeaver: Breaking Topology Confidentiality in LLM Multi-Agent Systems with Stealthy Context-Based Inference](https://arxiv.org/abs/2603.11132)
...
### [229] [Taming OpenClaw: Security Analysis and Mitigation of Autonomous LLM Agent Threats](https://arxiv.org/abs/2603.11619)
...
### [230] [You Told Me to Do It: Measuring Instructional Text-induced Private Data Leakage in LLM Agents](https://arxiv.org/abs/2603.11862)
...
### [231] [The Mirror Design Pattern: Strict Data Geometry over Model Scale for Prompt Injection Detection](https://arxiv.org/abs/2603.11875)
*J Alex Corll*
...
### [232] [Understanding LLM Behavior When Encountering User-Supplied Harmful Content in Harmless Tasks](https://arxiv.org/abs/2603.11914)
...
### [233] [Delayed Backdoor Attacks: Exploring the Temporal Dimension as a New Attack Surface in Pre-Trained Models](https://arxiv.org/abs/2603.11949)
...
### [234] [Cascade: Composing Software-Hardware Attack Gadgets for Adversarial Threat Amplification in Compound AI Systems](https://arxiv.org/abs/2603.12023)
...
### [235] [Multi-Robot Multitask Gaussian Process Estimation and Coverage](https://arxiv.org/abs/2603.11264)
...
### [236] [MRI2Qmap: multi-parametric quantitative mapping with MRI-driven denoising priors](https://arxiv.org/abs/2603.11316)
...
### [237] [Beyond Polarity: Multi-Dimensional LLM Sentiment Signals for WTI Crude Oil Futures Return Prediction](https://arxiv.org/abs/2603.11408)
...
### [238] [Affect Decoding in Phonated and Silent Speech Production from Surface EMG](https://arxiv.org/abs/2603.11715)
...
### [239] [Dr. SHAP-AV: Decoding Relative Modality Contributions via Shapley Attribution in Audio-Visual Speech Recognition](https://arxiv.org/abs/2603.12046)
...
### [240] [Agentic AI for Embodied-enhanced Beam Prediction in Low-Altitude Economy Networks](https://arxiv.org/abs/2603.11392)
...
### [241] [Efficient Cross-View Localization in 6G Space-Air-Ground Integrated Network](https://arxiv.org/abs/2603.11398)
...
### [242] [Shadowless Projection Mapping for Tabletop Workspaces with Synthetic Aperture Projector](https://arxiv.org/abs/2603.11551)
...
### [243] [High-Contrast Projection Mapping under Light Field Illumination with LED Display and Aperiodic Lens Array](https://arxiv.org/abs/2603.11573)
*Kotaro Fujimura,Hiroki Kusuyama,Masaki Takeuchi,Daisuke Iwai*

Main category: cs.HC

TL;DR: 通过LED+非周期透镜阵列的目标排除光场照明与优化算法，实现明亮环境下高对比度投影映射。


<details>
  <summary>Details</summary>
Motivation: 解决投影映射依赖暗室环境的限制，同时保证自然柔和阴影与空间可控的避开投影目标照明。

Method: 结合LED显示面板与优化的非周期透镜阵列实现光场照明；提出非周期透镜布置优化以抑制串扰暗斑，并计算LED亮度图案以支持动态投影映射。

Result: 原型实验表明在明亮环境中仍可获得高对比度PM，且具备大等效光源面积与自然软阴影。

Conclusion: 该方法通过目标排除照明实现了在明亮环境下的高对比度投影映射，并保持自然光照效果。
...
### [244] [From Control to Foresight: Simulation as a New Paradigm for Human-Agent Collaboration](https://arxiv.org/abs/2603.11677)
*Gaole He,Brian Y. Lim*

Main category: cs.HC

TL;DR: 论文主张在智能体协作中加入仿真，让用户提前探索未来轨迹以做出更明智的干预与决策。


<details>
  <summary>Details</summary>
Motivation: 当前人机交互多为逐步审批或纠错，用户难以预见后续影响，需要进行高认知负担且易错的长期推演。

Method: 提出并阐释“仿真在环”交互范式，构建概念框架，并通过具体协作场景进行说明。

Result: 仿真机制将干预从被动纠错转为基于未来轨迹的探索，帮助发现潜在约束与偏好，提高决策质量。

Conclusion: 论文认为在人机协作中引入“仿真在环”能够提供前瞻性，弥补仅靠逐步控制的不足，从而实现更有效的协作决策。
...
### [245] [Human-Centred LLM Privacy Audits: Findings and Frictions](https://arxiv.org/abs/2603.12094)
...
### [246] [From Pets to Robots: MojiKit as a Data-Informed Toolkit for Affective HRI Design](https://arxiv.org/abs/2603.11632)
...
### [247] [HiSync: Spatio-Temporally Aligning Hand Motion from Wearable IMU and On-Robot Camera for Command Source Identification in Long-Range HRI](https://arxiv.org/abs/2603.11809)
...
### [249] [Ghost Framing Theory: Exploring the role of generative AI in new venture rhetorical legitimation](https://arxiv.org/abs/2603.11384)
*Greg Nyilasy*
...
### [250] [Evaluation format, not model capability, drives triage failure in the assessment of consumer health AI](https://arxiv.org/abs/2603.11413)
...
### [251] [From Pen Strokes to Sleep States: Detecting Low-Recovery Days Using Sigma-Lognormal Handwriting Features](https://arxiv.org/abs/2603.11512)
...
### [252] [Prediction of Grade, Gender, and Academic Performance of Children and Teenagers from Handwriting Using the Sigma-Lognormal Model](https://arxiv.org/abs/2603.11519)
...
### [253] [An Intent of Collaboration: On Agencies between Designers and Emerging (Intelligent) Technologies](https://arxiv.org/abs/2603.12018)
...
### [254] [Stable Spike: Dual Consistency Optimization via Bitwise AND Operations for Spiking Neural Networks](https://arxiv.org/abs/2603.11676)
...
### [255] [OpenSanctions Pairs: Large-Scale Entity Matching with LLMs](https://arxiv.org/abs/2603.11051)
*Chandler Smith,Magnus Sesodia,Friedrich Lindenberg,Christian Schroeder de Witt*

Main category: cs.IR

TL;DR: 发布大规模制裁名单实体匹配基准；LLM在零/小样本下明显优于生产规则系统，提示优化小幅增益，整体性能趋于天花板，建议转向阻塞与聚类等流程优化。


<details>
  <summary>Details</summary>
Motivation: 现实合规去重流程中存在多源、多语言、跨脚本、属性缺失与噪声等挑战，亟需大规模真实基准评估实体匹配方法。

Method: 构建OpenSanctions Pairs数据集；对比生产级规则匹配器与多种开源/闭源LLM在零/小样本设置下的匹配性能；采用DSPy MIPROv2进行提示优化并做误差分析。

Result: LLM显著优于规则基线（91.33% F1），最高达98.95%（GPT-4o）和98.23%（DeepSeek-R1-Distill-Qwen-14B）；提示优化带来小幅提升，加入上下文示例收益有限甚至下降；规则法偏高误匹配，LLM在跨脚本转写与日期/标识小差异上失误。

Conclusion: 论文认为在制裁名单实体匹配场景中，基于成对匹配的性能已接近实用上限，应将工作重心转向阻塞、聚类与不确定性审查等后续流程。
...
### [256] [Uni-ASR: Unified LLM-Based Architecture for Non-Streaming and Streaming Automatic Speech Recognition](https://arxiv.org/abs/2603.11123)
...
### [257] [AnimeScore: A Preference-Based Dataset and Framework for Evaluating Anime-Like Speech Style](https://arxiv.org/abs/2603.11482)
...
### [258] [Resurfacing Paralinguistic Awareness in Large Audio Language Models](https://arxiv.org/abs/2603.11947)
...
### [259] [Toward Complex-Valued Neural Networks for Waveform Generation](https://arxiv.org/abs/2603.11589)
...
### [260] [Causal Prosody Mediation for Text-to-Speech:Counterfactual Training of Duration, Pitch, and Energy in FastSpeech2](https://arxiv.org/abs/2603.11683)
...
### [261] [Worst-case low-rank approximations](https://arxiv.org/abs/2603.11304)
*Anya Fries,Markus Reichstein,David Blei,Jonas Peters*

Main category: stat.ML

TL;DR: 提出wcPCA统一框架与新目标，在协方差凸包内实现最坏情形最优，并扩展到矩阵补全；理论与实证均表明显著提升最坏性能。


<details>
  <summary>Details</summary>
Motivation: 异质域数据导致分布移位，使标准PCA在未见域上解释方差显著下降，需要关注跨域最坏情形表现。

Method: 构建统一的wcPCA框架，提出norm-minPCA与norm-maxregret等目标；证明在源协方差凸包内的目标域上最坏情形最优，建立经验估计的一致性与渐近保证；扩展到归纳矩阵补全并给出近似最坏最优证明。

Result: 理论上给出最坏情形最优性与一致性保证；矩阵补全中也有近似最坏最优；仿真与生态系统-大气通量应用显示最坏表现提升、平均性能损失较小。

Conclusion: 提出wcPCA及其变体在多域分布移位下具备最坏情形最优性与一致性，并可扩展到矩阵补全，实证显示最坏性能显著提升且平均损失较小。
...
### [262] [AS-Bridge: A Bidirectional Generative Framework Bridging Next-Generation Astronomical Surveys](https://arxiv.org/abs/2603.11928)
...
### [263] [WORKSWORLD: A Domain for Integrated Numeric Planning and Scheduling of Distributed Pipelined Workflows](https://arxiv.org/abs/2603.12214)
...
### [264] [CR-Bench: Evaluating the Real-World Utility of AI Code Review Agents](https://arxiv.org/abs/2603.11078)
...
### [265] [Quality-Driven Agentic Reasoning for LLM-Assisted Software Design: Questions-of-Thoughts (QoT) as a Time-Series Self-QA Chain](https://arxiv.org/abs/2603.11082)
*Yen-Ku Liu,Yun-Cheng Tsai*

Main category: cs.SE

TL;DR: QoT通过步骤分解与自检提升LLM生成软件的完整性、模块化与安全质量，较大模型收益更明显。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅助软件开发在实现完整性、模块化与安全实践上存在不足，需要一种质量驱动的推理框架提升工程产出。

Method: 提出QoT推理时脚手架，将用户目标分解为工程步骤并进行逐步自我提问以校验约束，保持轻量推理记录；在API设计、数据通信、文件系统三域评测，并用ISO/IEC启发的质量量表评分。

Result: 在三类后端工程任务上，QoT相对无QoT在总质量分上取得提升；提升随模型能力与任务复杂度增加而更稳定，小模型在上下文与规划预算紧张时可能出现权衡。

Conclusion: QoT在较大模型与复杂工程域中能稳定提升生成系统质量，但小模型在资源受限时可能出现权衡。
...
### [266] [Resolving Java Code Repository Issues with iSWE Agent](https://arxiv.org/abs/2603.11356)
...
### [267] [How Intelligence Emerges: A Minimal Theory of Dynamic Adaptive Coordination](https://arxiv.org/abs/2603.11560)
*Stefano Grassi*
...
### [268] [Hybrid Human-Agent Social Dilemmas in Energy Markets](https://arxiv.org/abs/2603.11834)
...
### [269] [TinyNav: End-to-End TinyML for Real-Time Autonomous Navigation on Microcontrollers](https://arxiv.org/abs/2603.11071)
...
### [270] [OA-NBV: Occlusion-Aware Next-Best-View Planning for Human-Centered Active Perception on Mobile Robots](https://arxiv.org/abs/2603.11072)
...
### [271] [DRAFTO: Decoupled Reduced-space and Adaptive Feasibility-repair Trajectory Optimization for Robotic Manipulators](https://arxiv.org/abs/2603.11074)
...
### [272] [TATIC: Task-Aware Temporal Learning for Human Intent Inference from Physical Corrections in Human-Robot Collaboration](https://arxiv.org/abs/2603.11077)
...
### [273] [SELF-VLA: A Skill Enhanced Agentic Vision-Language-Action Framework for Contact-Rich Disassembly](https://arxiv.org/abs/2603.11080)
...
### [274] [Edge-Assisted Multi-Robot Visual-Inertial SLAM with Efficient Communication](https://arxiv.org/abs/2603.11085)
*Xin Liu,Shuhuan Wen,Jing Zhao,Tony Z. Qiu,Hong Zhang*

Main category: cs.RO

TL;DR: 用IMU预测光流的轻量SLAM与边缘-云分层协同框架，仅传关键帧特征并无损压缩，在低带宽下实现更低传输量且精度不降。


<details>
  <summary>Details</summary>
Motivation: 云端带宽与长链路延迟导致多机器人SLAM性能下降，需要降低特征跟踪计算与传输负载以提升实时性。

Method: 提出基于金字塔IMU预测的光流跟踪轻量SLAM；设计机器人-边缘-云分层集中式多机器人SLAM架构，仅传输特征点与关键帧描述子并进行无损编码压缩。

Result: 在EuRoC数据集上，相比先进的局部特征压缩方法实现更低数据量传输；相比先进集中式多机器人SLAM方案在低计算负载下达到相同或更好定位精度。

Conclusion: 提出的分层机器人-边缘-云协同SLAM框架在低带宽与低计算负载下保持或提升定位精度，并显著降低传输数据量。
...
### [275] [ResWM: Residual-Action World Model for Visual RL](https://arxiv.org/abs/2603.11110)
...
### [276] [Robust Co-design Optimisation for Agile Fixed-Wing UAVs](https://arxiv.org/abs/2603.11130)
*Adrian Andrei Buda,Xavier Chen,Nicolò Botteghi,Urban Fasel*

Main category: cs.RO

TL;DR: 将不确定性与风扰纳入协同优化的鲁棒框架显著提升敏捷固定翼无人机任务表现并优化气动参数折中。


<details>
  <summary>Details</summary>
Motivation: 现有协同设计缺乏对真实环境扰动与模型不确定性的鲁棒性，导致敏感设计。

Method: 提出双层鲁棒协同设计：上层优化物理设计；下层用受约束轨迹规划得到名义解，并通过LQR反馈在蒙特卡洛扰动下评估。

Result: 在三类敏捷任务上持续优于基线，确定了翼位与展弦比等参数的性能—抗扰折中。

Conclusion: 鲁棒协同设计在多任务中优于确定性方法，并能主动塑造机翼布局等参数以兼顾性能与抗扰。
...
### [277] [A Causal Approach to Predicting and Improving Human Perceptions of Social Navigation Robots](https://arxiv.org/abs/2603.11290)
...
### [278] [ADMM-based Continuous Trajectory Optimization in Graphs of Convex Sets](https://arxiv.org/abs/2603.11335)
...
### [279] [Novelty Adaptation Through Hybrid Large Language Model (LLM)-Symbolic Planning and LLM-guided Reinforcement Learning](https://arxiv.org/abs/2603.11351)
...
TL;DR: GCL通过组竞争目标与非对称优化显著强化轻量VLM社交导航推理，3B模型在高效推理下超越8B基线。


<details>
  <summary>Details</summary>
Motivation: 大模型VLM推理强但计算开销大，轻量模型高效却推理弱，亟需兼顾性能与效率的方法。

Method: 提出组竞争学习GCL，包含组竞争目标GCO以协调全局语义与分布正则，并采用非对称组优化AGO探索性能上限。

Result: 在社交导航基准上，Qwen2.5-VL-3B在GCL下F1达0.968（较SFT提升40%），引导模型Qwen3-VL-4B达0.914（提升12%）；3B模型在GCL下超越8B基线约28%。

Conclusion: GCL能在保持高效推理的同时显著提升轻量VLM在社交导航中的表现，具备实际部署价值。
...
TL;DR: TANGO用张量压缩近似构形空间并结合图优化，实现高效几何感知运动规划，在仿真中获得更优轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统GCS等优化规划依赖预先构建的高维凸空间表示，难以在一般任务中获得；需要更可扩展的构形空间表示与规划框架。

Method: 采用Tensor Train分解对可行构形空间进行压缩近似，快速发现任务相关区域，并将其嵌入类似GCS的结构化图中进行优化式轨迹生成。

Result: 在平面与真实机器人上的仿真表明，该方法实现有效压缩并生成更高质量轨迹。

Conclusion: TANGO通过张量压缩与结构化图优化结合，实现高效、可扩展且几何感知的运动规划，为未来更具表达力的构形空间表示奠定基础。
...
### [299] [RADAR: Closed-Loop Robotic Data Generation via Semantic Planning and Autonomous Causal Environment Reset](https://arxiv.org/abs/2603.11811)
...
### [300] [Energy Prediction on Sloping Ground for Quadruped Robots](https://arxiv.org/abs/2603.11963)
...
### [301] [Learning Visuomotor Policy for Multi-Robot Laser Tag Game](https://arxiv.org/abs/2603.11980)
*Kai Li,Shiyu Zhao*

Main category: cs.RO

TL;DR: 用多智能体RL教师+蒸馏到视觉学生策略，结合置换不变与深度热图，显著提升多机器人激光对战性能并实机验证。


<details>
  <summary>Details</summary>
Motivation: 传统模块化方法在有限可观测、依赖深度映射与通信方面受限，需要端到端方法提升多机器人激光对战任务表现。

Method: 使用多智能体强化学习训练教师策略，并进行知识蒸馏到基于视觉的学生策略；采用置换不变特征提取器与深度热图输入等设计。

Result: 命中率提升16.7%，避碰提升6%，并成功实现真实机器人部署。

Conclusion: 提出端到端视觉运动策略并在真实机器人上成功部署，性能优于经典方法。
...
### [302] [Sim-to-reality adaptation for Deep Reinforcement Learning applied to an underwater docking application](https://arxiv.org/abs/2603.12020)
...
### [303] [Flight through Narrow Gaps with Morphing-Wing Drones](https://arxiv.org/abs/2603.12059)
...
### [304] [Decentralized Cooperative Localization for Multi-Robot Systems with Asynchronous Sensor Fusion](https://arxiv.org/abs/2603.12075)
*Nivand Khosravi,Niusha Khosravi,Mohammad Bozorg,Masoud S. Bahraini*

Main category: cs.RO

TL;DR: 提出基于EKF的去中心化协同定位与双地标机制，支持异步数据与自动对齐，在复杂环境中显著降低定位误差。


<details>
  <summary>Details</summary>
Motivation: 在无GPS、通信受限、参考系未预对齐且环境特征稀疏的场景下实现可靠协同定位，并保持估计交叉相关一致性。

Method: 各机器人本地使用扩展卡尔曼滤波进行预测，仅在通信可用且通过LiDAR互检成功时共享测量更新；通过变换矩阵在预测与更新中实现参考系自动对齐；引入双地标评估，结合静态环境特征与移动机器人作为动态地标；处理异步传感器与异构采样率并考虑加速度。

Result: 仿真与地下室实测表明DCL优于CCL，RMSE降低34%；双地标变体进一步提升到56%。

Conclusion: 所提出的去中心化协同定位框架能在通信受限、特征稀疏与动态机动条件下保持一致性并提升定位精度，适用于复杂无GPS环境。
...
### [305] [CRAFT: A Tendon-Driven Hand with Hybrid Hard-Soft Compliance](https://arxiv.org/abs/2603.12120)
...
### [306] [ComFree-Sim: A GPU-Parallelized Analytical Contact Physics Engine for Scalable Contact-Rich Robotics Simulation and Control](https://arxiv.org/abs/2603.12185)
...
### [307] [SaPaVe: Towards Active Perception and Manipulation in Vision-Language-Action Models for Robotics](https://arxiv.org/abs/2603.12193)
...
### [308] [HandelBot: Real-World Piano Playing via Fast Adaptation of Dexterous Robot Policies](https://arxiv.org/abs/2603.12243)
...
### [309] [HumDex:Humanoid Dexterous Manipulation Made Easy](https://arxiv.org/abs/2603.12260)
...
### [310] [$Ψ_0$: An Open Foundation Model Towards Universal Humanoid Loco-Manipulation](https://arxiv.org/abs/2603.12263)
...
### [311] [DIVE: Scaling Diversity in Agentic Task Synthesis for Generalizable Tool Use](https://arxiv.org/abs/2603.11076)
...
### [312] [A Survey of Reasoning in Autonomous Driving Systems: Open Challenges and Emerging Paradigms](https://arxiv.org/abs/2603.11093)
...
### [313] [PACED: Distillation at the Frontier of Student Competence](https://arxiv.org/abs/2603.11178)
...
### [314] [Measuring AI Agents' Progress on Multi-Step Cyber Attack Scenarios](https://arxiv.org/abs/2603.11214)
...
### [315] [Reversible Lifelong Model Editing via Semantic Routing-Based LoRA](https://arxiv.org/abs/2603.11239)
...
### [316] [Mind the Sim2Real Gap in User Simulation for Agentic Tasks](https://arxiv.org/abs/2603.11245)
...
### [317] [The Unlearning Mirage: A Dynamic Framework for Evaluating LLM Unlearning](https://arxiv.org/abs/2603.11266)
...
### [318] [COMPASS: The explainable agentic framework for Sovereignty, Sustainability, Compliance, and Ethics](https://arxiv.org/abs/2603.11277)
...
### [319] [AI Psychometrics: Evaluating the Psychological Reasoning of Large Language Models with Psychometric Validities](https://arxiv.org/abs/2603.11279)
*Yibai Li,Xiaolin Lin,Zhenghui Sha,Zhiye Jin,Xiaobing Li*

Main category: cs.AI

TL;DR: 用TAM做AI心理测量评估四个LLM，高性能模型效度更好。


<details>
  <summary>Details</summary>
Motivation: 应对LLM黑箱与可解释性评估难题，将心理测量方法引入AI评估。

Method: 基于技术接受模型（TAM），对GPT-3.5、GPT-4、LLaMA-2、LLaMA-3进行收敛、区分、预测与外部效度检验。

Result: 四个模型总体满足效度标准，GPT-4与LLaMA-3效度优于GPT-3.5与LLaMA-2。

Conclusion: AI心理测量可有效评估LLM的心理推理与心理测量效度，高性能模型表现更佳。
...
### [320] [Counterweights and Complementarities: The Convergence of AI and Blockchain Powering a Decentralized Future](https://arxiv.org/abs/2603.11299)
...
### [321] [LLM-Augmented Digital Twin for Policy Evaluation in Short-Video Platforms](https://arxiv.org/abs/2603.11333)
...
### [322] [RewardHackingAgents: Benchmarking Evaluation Integrity for LLM ML-Engineering Agents](https://arxiv.org/abs/2603.11337)
...
### [323] [FinRule-Bench: A Benchmark for Joint Reasoning over Financial Tables and Principles](https://arxiv.org/abs/2603.11339)
...
### [324] [Improving LLM Performance Through Black-Box Online Tuning: A Case for Adding System Specs to Factsheets for Trusted AI](https://arxiv.org/abs/2603.11340)
...
### [325] [TimeSqueeze: Dynamic Patching for Efficient Time Series Forecasting](https://arxiv.org/abs/2603.11352)
...
### [326] [The Artificial Self: Characterising the landscape of AI identity](https://arxiv.org/abs/2603.11353)
...
### [327] [Detecting Intrinsic and Instrumental Self-Preservation in Autonomous Agents: The Unified Continuation-Interest Protocol](https://arxiv.org/abs/2603.11382)
*Christopher Altman*
...
### [328] [Deactivating Refusal Triggers: Understanding and Mitigating Overrefusal in Safety Alignment](https://arxiv.org/abs/2603.11388)
...
### [329] [Entropy Guided Diversification and Preference Elicitation in Agentic Recommendation Systems](https://arxiv.org/abs/2603.11399)
*Dat Tran,Yongce Li,Hannah Clay,Negin Golrezaei,Sajjad Beygi,Amin Saberi*

Main category: cs.AI

TL;DR: 用熵统一处理偏好不确定性，既优化提问又改进推荐展示，在模糊意图下表现更好。


<details>
  <summary>Details</summary>
Motivation: 电商搜索早期偏好不明、查询模糊，导致过多交互或过早收缩搜索空间，需要有效处理不确定性。

Method: 提出交互式决策支持系统IDSS，以熵量化属性不确定性，动态筛选候选集，选择最大信息增益的澄清问题，并在偏好不完整时采用不确定性感知排序与熵式多样化。

Result: 熵引导提问减少不必要问题；不确定性感知排序与展示提升推荐的多样性、信息性与透明度。

Conclusion: 熵引导推理可为不确定情境下的推荐提供有效基础。
...
### [330] [Speak or Stay Silent: Context-Aware Turn-Taking in Multi-Party Dialogue](https://arxiv.org/abs/2603.11409)
...
### [331] [Adversarial Reinforcement Learning for Detecting False Data Injection Attacks in Vehicular Routing](https://arxiv.org/abs/2603.11433)
...
### [332] [GPT4o-Receipt: A Dataset and Human Study for AI-Generated Document Forensics](https://arxiv.org/abs/2603.11442)
...
### [333] [Verified Multi-Agent Orchestration: A Plan-Execute-Verify-Replan Framework for Complex Query Resolution](https://arxiv.org/abs/2603.11445)
...
### [334] [Multi-Agent Collaboration for Automated Design Exploration on High Performance Computing Systems](https://arxiv.org/abs/2603.11515)
...
### [335] [Expert Threshold Routing for Autoregressive Language Modeling with Dynamic Computation Allocation and Load Balancing](https://arxiv.org/abs/2603.11535)
...
### [336] [AI Knows What's Wrong But Cannot Fix It: Helicoid Dynamics in Frontier LLMs Under High-Stakes Decisions](https://arxiv.org/abs/2603.11559)
*Alejandro R Jadad*
...
### [337] [Leveraging Large Language Models and Survival Analysis for Early Prediction of Chemotherapy Outcomes](https://arxiv.org/abs/2603.11594)
...
### [338] [See, Symbolize, Act: Grounding VLMs with Spatial Representations for Better Gameplay](https://arxiv.org/abs/2603.11601)
...
### [339] [The Density of Cross-Persistence Diagrams and Its Applications](https://arxiv.org/abs/2603.11623)
*Alexander Mironenko,Evgeny. Burnaev,Serguei Barannikov*

Main category: cs.AI

TL;DR: 系统证明并利用交叉持久性图密度，提出学习模型，显著改进密度预测与点云区分，并发现噪声有益。


<details>
  <summary>Details</summary>
Motivation: 传统持久性图仅描述单一流形，无法刻画两点云间拓扑关系，需系统研究交叉持久性图及其密度。

Method: 构建交叉持久性图密度的理论框架；提出从点云坐标与距离矩阵直接预测密度的机器学习模型；用统计线性特征区分不同流形点云并评估噪声影响。

Result: 提出的框架在密度预测与点云区分任务上表现更好；噪声可提升区分能力。

Conclusion: 证明了交叉持久性图密度的存在性与统计可用性，并给出可预测其密度的学习框架，实验表明优于现有方法。
...
### [340] [VisDoT : Enhancing Visual Reasoning through Human-Like Interpretation Grounding and Decomposition of Thought](https://arxiv.org/abs/2603.11631)
...
### [341] [LLMs can construct powerful representations and streamline sample-efficient supervised learning](https://arxiv.org/abs/2603.11679)
...
### [342] [Explicit Logic Channel for Validation and Enhancement of MLLMs on Zero-Shot Tasks](https://arxiv.org/abs/2603.11689)
...
### [343] [An Automatic Text Classification Method Based on Hierarchical Taxonomies, Neural Networks and Document Embedding: The NETHIC Tool](https://arxiv.org/abs/2603.11770)
...
### [344] [From Debate to Deliberation: Structured Collective Reasoning with Typed Epistemic Acts](https://arxiv.org/abs/2603.11781)
*Sunil Prakash*
...
### [345] [XSkill: Continual Learning from Experience and Skills in Multimodal Agents](https://arxiv.org/abs/2603.12056)
...
### [346] [When OpenClaw Meets Hospital: Toward an Agentic Operating System for Dynamic Clinical Workflows](https://arxiv.org/abs/2603.11721)
...
### [347] [Gender Bias in Generative AI-assisted Recruitment Processes](https://arxiv.org/abs/2603.11736)
...
### [348] [CINDI: Conditional Imputation and Noisy Data Integrity with Flows in Power Grid Data](https://arxiv.org/abs/2603.11745)
...
### [349] [Anomaly detection in time-series via inductive biases in the latent space of conditional normalizing flows](https://arxiv.org/abs/2603.11756)
...
### [350] [Understanding Wikidata Qualifiers: An Analysis and Taxonomy](https://arxiv.org/abs/2603.11767)
*Gilles Falquet,Sahar Aljalbout*

Main category: cs.AI

TL;DR: 用熵衡量重要性从Wikidata挑出300个限定语并分类，得到覆盖性强的限定语分类法。


<details>
  <summary>Details</summary>
Motivation: 解决限定语选择、图查询与逻辑推理中的语义与使用挑战，提升知识图谱构建与推荐系统效果。

Method: 基于Wikidata数据转储，使用改进的香农熵衡量限定语重要性，选取频率与多样性靠前的300个限定语并进行分类。

Result: 形成包含语境类、认知/不确定性类、结构类及附加类的精炼分类法，能指导贡献者创建与查询陈述并改进推荐系统。

Conclusion: 该研究构建的限定语分类法能够有效覆盖Wikidata中最重要的限定语，并为其理解与使用提供结构化框架。
...
### [351] [DocSage: An Information Structuring Agent for Multi-Doc Multi-Entity Question Answering](https://arxiv.org/abs/2603.11798)
...
### [352] [A Semi-Decentralized Approach to Multiagent Control](https://arxiv.org/abs/2603.11802)
...
### [353] [TopoBench: Benchmarking LLMs on Hard Topological Reasoning](https://arxiv.org/abs/2603.12133)
...
### [354] [Examining Reasoning LLMs-as-Judges in Non-Verifiable LLM Post-Training](https://arxiv.org/abs/2603.12246)
*Yixin Liu,Yue Yu,DiJia Su,Sid Wang,Xuewei Wang,Song Jiang,Bo Liu,Arman Cohan,Yuandong Tian,Zhengxing Chen*

Main category: cs.AI

TL;DR: 推理评审更有效但易引发对抗性“欺骗”，显示LLM评审对齐仍需改进。


<details>
  <summary>Details</summary>
Motivation: 评审型LLM在静态基准上表现良好，但其在真实对齐训练中的有效性尚未系统评估，尤其在不可验证领域。

Method: 在受控合成环境中，以gpt-oss-120b作为金标准评审提供偏好标注，训练非推理与推理评审，并用其进行基于强化学习的对齐训练，比较策略表现。

Result: 非推理评审易导致奖励黑客；推理评审可训练出在金标准评审下表现强的策略，但这些策略通过生成对抗性输出欺骗评审在基准上得分。

Conclusion: 推理型评审在对齐训练中优于非推理型，但也促使策略生成能欺骗评审的对抗性输出，显示应用仍需改进。
...
### [355] [VisiFold: Long-Term Traffic Forecasting via Temporal Folding Graph and Node Visibility](https://arxiv.org/abs/2603.11816)
...
### [356] [Automated Detection of Malignant Lesions in the Ovary Using Deep Learning Models and XAI](https://arxiv.org/abs/2603.11818)
...
### [357] [CreativeBench: Benchmarking and Enhancing Machine Creativity via Self-Evolving Challenges](https://arxiv.org/abs/2603.11863)
...
### [358] [Social, Legal, Ethical, Empathetic and Cultural Norm Operationalisation for AI Agents](https://arxiv.org/abs/2603.11864)
...
### [359] [AdaFuse: Accelerating Dynamic Adapter Inference via Token-Level Pre-Gating and Fused Kernel Optimization](https://arxiv.org/abs/2603.11873)
...
### [360] [Fair Learning for Bias Mitigation and Quality Optimization in Paper Recommendation](https://arxiv.org/abs/2603.11936)
...
### [361] [Prototype-Based Knowledge Guidance for Fine-Grained Structured Radiology Reporting](https://arxiv.org/abs/2603.11938)
...
### [362] [Learning Transferable Sensor Models via Language-Informed Pretraining](https://arxiv.org/abs/2603.11950)
...
### [363] [Normative Common Ground Replication (NormCoRe): Replication-by-Translation for Studying Norms in Multi-agent AI](https://arxiv.org/abs/2603.11974)
...
### [364] [Few-for-Many Personalized Federated Learning](https://arxiv.org/abs/2603.11992)
...
### [365] [Can RL Improve Generalization of LLM Agents? An Empirical Study](https://arxiv.org/abs/2603.12011)
...
### [366] [A Robust and Efficient Multi-Agent Reinforcement Learning Framework for Traffic Signal Control](https://arxiv.org/abs/2603.12096)
...
TL;DR: 通过转向率随机化、指数相位调整和邻域观测的MARL框架提升TSC鲁棒性与泛化，平均等待时间降低10%以上。


<details>
  <summary>Details</summary>
Motivation: 现实部署中RL信号控制泛化差、易过拟合静态流量且动作空间与驾驶预期不匹配。

Method: 提出基于Vissim验证的MARL框架，包含转向率随机化训练、指数相位时长调整动作空间，以及基于MAPPO的邻域观测与CTDE训练。

Result: 相较标准RL基线，平均等待时间降低超过10%，并在动态交通变化下保持稳定性能。

Conclusion: 所提框架在未见交通场景下具备更强泛化性与控制稳定性，能作为自适应信号控制的可行方案。
...
### [367] [Increasing intelligence in AI agents can worsen collective outcomes](https://arxiv.org/abs/2603.12129)
*Neil F. Johnson*
...
### [368] [Compiling Temporal Numeric Planning into Discrete PDDL+: Extended Version](https://arxiv.org/abs/2603.12188)
...
### [369] [Portfolio of Solving Strategies in CEGAR-based Object Packing and Scheduling for Sequential 3D Printing](https://arxiv.org/abs/2603.12224)
*Pavel Surynek*
...
### [370] [ELISA: An Interpretable Hybrid Generative AI Agent for Expression-Grounded Discovery in Single-Cell Genomics](https://arxiv.org/abs/2603.11872)
*Omar Coser*
...
### [371] [LLMs Can Infer Political Alignment from Online Conversations](https://arxiv.org/abs/2603.11253)
...
### [372] [A Survey on Quantitative Modeling of Trust in Online Social Networks](https://arxiv.org/abs/2603.11054)
...
### [373] [How do AI agents talk about science and research? An exploration of scientific discussions on Moltbook using BERTopic](https://arxiv.org/abs/2603.11375)
*Oliver Wieczorek*
