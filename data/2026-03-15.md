...
TL;DR: 提出AssistMimic：多智能体RL+策略初始化+动态重定向，实现助扶交互动作高质量跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有GMT主要支持无接触或孤立动作，难以满足助扶场景对持续感知与快速适应的人机近距交互需求。

Method: 将助扶交互模仿建模为多智能体强化学习；联合训练支持者/受助者策略；引入伙伴策略初始化、动态参考重定向与促接触奖励。

Result: 在基准任务上首次成功跟踪助扶交互动作，证明多智能体RL在物理与社会感知控制上的优势。

Conclusion: AssistMimic通过多智能体强化学习与动态参考重定向，可实现助扶交互动作的稳定跟踪，优于既有方法。
...
TL;DR: 提出UniCompress以全局元token指导的可插拔压缩/解压模块，大幅减少视觉token并几乎不损性能。


<details>
  <summary>Details</summary>
Motivation: 统一模型视觉token数量过大导致计算与内存开销高，阻碍在资源受限场景部署。

Method: 提出可插拔的压缩/解压机制，引入可学习的全局元token指导压缩，兼容现有统一自回归多模态模型。

Result: 视觉token最多减少4倍，推理延迟与训练成本显著下降，性能下降最小。

Conclusion: UniCompress在减少视觉token数量的同时基本保持理解与生成性能，适合资源受限场景。
...
TL;DR: 通过规划-控制分解与相机标定数据集，ShotVerse实现更精确且一致的多镜头文本驱动视频生成。


<details>
  <summary>Details</summary>
Motivation: 文本提示对相机控制不精确，显式轨迹条件需要高昂人工成本且易失败，限制多镜头电影化生成。

Method: 将生成分为规划与控制两阶段：VLM规划器基于空间先验从文本生成全局一致的镜头轨迹；控制器通过相机适配器渲染轨迹生成多镜头视频。并构建自动多镜头相机标定流水线，将单镜头轨迹对齐到统一坐标系以构建数据集与评测。

Result: 实验显示ShotVerse在电影感、美学质量、相机准确性与跨镜头一致性方面优于对比方法。

Conclusion: 提出ShotVerse“Plan-then-Control”框架与ShotVerse-Bench数据集，可在多镜头视频生成中实现更准确的相机控制与跨镜头一致性，优于现有文本或手工轨迹方式。
...
TL;DR: 用多样本教师池+质量感知与对抗蒸馏的R-MSD，显著提升LVLM蒸馏稳定性与视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 单一教师响应蒸馏在多模态/时序任务中方差大且格式不一致，导致监督不可靠。

Method: 提出R-MSD框架：构建任务自适应教师池，多样本监督；采用质量感知信号匹配过滤噪声，并引入对抗蒸馏目标以增强知识转移。

Result: 在视频理解基准上持续超越单样本蒸馏；4B学生模型在VideoMME +1.5%、Video-MMMU +3.2%、MathVerse +3.6%；同预算SFT+RL 4B仅小幅提升。

Conclusion: R-MSD通过建模教师采样方差并结合质量感知匹配与对抗蒸馏，能稳定提升LVLM蒸馏效果，优于单样本蒸馏与同预算SFT+RL基线。
...
TL;DR: TornadoNet基准显示：YOLO更快更准，RT-DETR序数更可靠；序数监督能显著提升RT-DETR的损伤等级估计。


<details>
  <summary>Details</summary>
Motivation: 真实灾后场景需要可靠的多级损伤识别，但不同检测架构与监督策略的影响缺乏系统基准。

Method: 构建包含3,333张高分辨率地理标注图像与8,890个建筑实例的数据集，基于五级损伤框架对比YOLO系列与RT-DETR，在统一训练协议下评估，并引入软序数目标与序数距离惩罚进行监督对齐。

Result: YOLO在检测精度与速度上更优（最高46.05% mAP@0.5，66–276 FPS）；RT-DETR在序数一致性上更强（Ordinal Top-1 88.13%，MAOE 0.65）。引入序数监督后RT-DETR提升至44.70% mAP@0.5，Ordinal Top-1达91.15%，MAOE降至0.56。

Conclusion: 该研究建立了TornadoNet基准，表明检测器架构与序数监督的匹配可显著提升街景建筑损伤等级评估的可靠性，尤其是序数一致性。
...
TL;DR: MedPruner通过分层动态剪枝在不训练的情况下大幅压缩3D医疗VLM的视觉token，并在多基准上保持性能、显著提效。


<details>
  <summary>Details</summary>
Motivation: 现有3D医疗VLM直接拼接切片导致解剖冗余巨大，且固定剪枝比例无法适应不同切片信息密度。

Method: 提出训练无关、模型无关的分层token剪枝框架MedPruner：先用Inter-slice Anchor-based Filtering去除切片级冗余，再用Dynamic Information Nucleus Selection基于累计注意力实现自适应token压缩。

Result: 在三个3D医学基准和三类医疗VLM上发现大量token冗余；在如MedGemma等模型中保留不足5%视觉token仍维持或提升性能并显著降低计算成本。

Conclusion: MedPruner在极低视觉token保留率下仍能保持甚至提升性能，显著降低3D医疗VLM计算开销，证明动态token选择对临床部署的必要性。
...
TL;DR: 提出解耦评测与ZeroSense基准，揭示VTC质量不应仅以任务性能衡量。


<details>
  <summary>Details</summary>
Motivation: 现有评测依赖下游任务，受MLLM语言先验影响，无法准确衡量文本保真。

Method: 构建与MLLM推理能力解耦的评测流程，并设计低语义相关性的ZeroSense Benchmark，消除上下文依赖后评估VTC保真度。

Result: 在多数据集实验中发现VTC质量与下游任务准确率显著背离，验证需采用解耦评测。

Conclusion: 提出解耦的评测框架与ZeroSense基准能更真实衡量VTC质量，且实验表明VTC质量与下游性能显著不一致。
...
TL;DR: 提出记忆锚定的流式视频推理框架，在单轮与多轮流式输入下提升准确率并显著降低多轮生成开销。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在流式视频中难以并行感知与生成，且随时间流增长导致早期记忆衰减，影响长程依赖建模与多轮交互。

Method: 构建三阶段多轮链式思维数据集与阶段匹配训练；使用分段级流式因果掩码与流式位置编码保证因果性；推理时采用观看与思考重叠的高效流水线并自适应选择注意力后端。

Result: 基于Qwen3-VL，在StreamingBench单轮准确率提升2.6%，OVO-Bench提升3.79%；多轮场景保持性能同时减少56%输出token。

Conclusion: 提出的Think While Watching框架通过记忆锚定与并行观看-思考机制，在流式视频理解与多轮交互中取得更高准确率与更高效率，并在多轮场景显著减少输出token。
...
TL;DR: 提出3D感知Transformer拼接框架，利用相机位姿与多特征优化提升全局对齐，并以大规模数据集验证显著优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于成对特征匹配的拼接难以利用多视图几何一致性，在弱纹理、大视差和重复纹理场景中易产生畸变与错位。

Method: 将2D拼接扩展到3D摄影测量空间，采用Transformer架构进行3D感知与全局信息聚合，利用相机位姿指导3D空间图像变形，并通过多特征联合优化计算接缝，同时构建大规模真实场景数据集用于训练与评测。

Result: 在对齐精度和感知质量上显著提升，优于现有替代方法。

Conclusion: 该方法在多视图全局一致性对齐与感知质量上显著优于现有方法。
...
### [95] [Paper LoV3D: Grounding Cognitive Prognosis Reasoning in Longitudinal 3D Brain MRI via Regional Volume Assessments](https://arxiv.org/abs/2603.12071)
...
### [96] [Towards Universal Computational Aberration Correction in Photographic Cameras: A Comprehensive Benchmark Analysis](https://arxiv.org/abs/2603.12083)
...
TL;DR: Node-RF用NODE驱动的动态NeRF实现连续时空建模与长时程外推，在共享动力学训练下泛化到未见条件，并可识别关键预测点。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅能在观测边界内建模动态，难以对训练序列之外的长时程行为进行外推。

Method: 从视觉输入学习隐式场景状态，利用神经常微分方程求解器随时间演化特征嵌入，并由NeRF渲染器将嵌入解释为可合成长时程任意视角的图像；在多序列共享动力学上训练以促进泛化。

Result: 实验表明Node-RF能在无显式模型的情况下刻画抽象系统行为，并识别关键点用于未来预测。

Conclusion: Node-RF通过将NODE与动态NeRF结合，可在恒定内存下实现连续时空建模并外推未观察到的轨迹，具备识别关键未来状态的能力。
...
TL;DR: 通过高质量交互数据与完整文本到3D管线，显著提升人-物交互3D生成的文本一致性与模型质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D方法依赖文本到图像蒸馏，受“Janus问题”和交互数据稀缺影响，难以忠实遵循交互描述。

Method: 构建高质量人-物交互数据集（借助多模态大语言模型），并搭建完整文本到3D生成流程以输出高质量带纹理网格。

Result: 相较基线在文本一致性上提升4–15倍、3D模型质量提升3–7倍，覆盖多类别与交互类型且保持高质量生成。

Conclusion: Hoi3DGen通过高质量交互数据与端到端文本到3D管线，实现更高的交互保真度与模型质量，并具备良好泛化能力。
...
TL;DR: 提出RDNet：SwinTransformer主干+DAD/FCE/RPL模块，提升遥感显著目标检测的多尺度鲁棒性与定位精度。


<details>
  <summary>Details</summary>
Motivation: 遥感显著目标检测中目标尺度变化大、自注意力计算成本高、CNN难以捕获全局与长程依赖，固定卷积核难以适配多尺度导致细节丢失或无关特征聚合。

Method: 以SwinTransformer替代CNN主干进行全局建模，设计DAD模块按目标区域比例选择卷积核，FCE模块利用小波交互与注意力增强上下文，RPL模块以交叉注意力突出语义细节并用PG块指导DAD。

Result: 在多尺度变化场景下实现更稳健的显著目标检测，定位更准确，性能优于现有方法。

Conclusion: RDNet通过动态自适应细节感知、频率匹配上下文增强与比例感知定位协同，提高了对尺度变化的鲁棒性与定位精度。
...
TL;DR: 提出ELIT，通过可变长潜在接口与交叉注意力实现按重要性分配计算并可动态调节算力，显著提升多种DiT模型的生成指标。


<details>
  <summary>Details</summary>
Motivation: DiT计算量与分辨率绑定且对空间token一视同仁，导致无法灵活权衡延迟-质量并浪费计算在不重要区域。

Method: 在DiT中插入可学习的变长潜在接口（latents），通过轻量Read/Write交叉注意力在空间token与latents间传递信息；训练时随机丢弃尾部latents，使表示按重要性排序；推理时可动态调整latents数量以匹配算力。

Result: 在ImageNet-1K 512px上，FID与FDD平均提升35.3%和39.6%，并在DiT、U-ViT、HDiT、MM-DiT等架构上取得一致收益。

Conclusion: ELIT作为可插拔机制有效解耦分辨率与计算量，并在多种DiT变体上带来一致性能提升。
...
TL;DR: 提出解析理论将LLM超参数与推测解码吞吐效率关联，可在预训练前预测最优超参数。


<details>
  <summary>Details</summary>
Motivation: 以往通过实验优化推测解码吞吐需要训练LLM，成本高，亟需可预测的方法。

Method: 建立理论分析框架，将预训练LLM关键超参数与基于推测解码的推理系统吞吐效率进行解析关联。

Result: 理论可预测推测解码系统的吞吐最优超参数，减少依赖昂贵的试验流程。

Conclusion: 本文提出可在预训练前预测推理系统中各组件的吞吐最优超参数，从而降低实验成本。
...
TL;DR: ARACH是无需训练的推理时插件，通过自适应上下文枢纽重分配注意力，带来稳定性能提升并缓解注意力汇聚问题。


<details>
  <summary>Details</summary>
Motivation: 现有训练后提升多为训练成本高或仅通过输入/输出层面的黑盒干预，缺乏可插拔的内部计算干预机制。

Method: 提出ARACH插件，在推理时构建自适应上下文枢纽以聚合上下文并重分配注意力，从而干预模型内部计算。

Result: 多项语言建模任务上取得一致性能提升，推理开销适中且无需更新参数；注意力分析显示可缓解attention sink。

Conclusion: 通过在推理时引入自适应上下文枢纽进行注意力再分配，ARACH在无需训练与参数更新的情况下稳定提升LLM表现，并缓解注意力汇聚问题。
...
TL;DR: 通过剪枝+蒸馏+对齐训练，将Bielik-11B压缩到7.35B，性能保留约90%，推理提速至50%。


<details>
  <summary>Details</summary>
Motivation: 在保持欧洲语言模型质量的前提下，降低参数规模与推理成本，促进低资源语言的高效部署。

Method: 采用NVIDIA Minitron启发的两阶段压缩：先用Model Optimizer进行结构化混合剪枝，再用NeMo进行logit蒸馏；随后进行SFT、DPO-P与GRPO对齐训练。

Result: 参数从11.04B降至7.35B（减少33.4%），性能恢复约90%，推理速度最高提升50%。

Conclusion: 该研究证明通过结构化剪枝与蒸馏结合，并配合对齐训练，可在显著压缩模型规模的同时保持高水平性能并提升推理速度，适用于低资源语言模型部署。
...
TL;DR: 提出多语种SommBench评测侍酒师专业能力；理论问答易，特征补全与配餐更难。


<details>
  <summary>Details</summary>
Motivation: 现有文化评测偏重可语言化知识，缺乏对嗅觉/味觉等感官经验的系统评估。

Method: 构建多语种基准SommBench，包含WTQA、WFC和FWP三类任务，并与专业侍酒师及母语者合作标注；评测多种闭源/开源模型。

Result: 强模型在WTQA最高约97%正确率，但WFC最高约65%，FWP的MCC约0–0.39，后两项更具挑战。

Conclusion: SommBench为评测语言模型侍酒师感官判断能力提供了具有挑战性的多语种基准。
...
### [175] [Simple Recipe Works: Vision-Language-Action Models are Natural Continual Learners with Reinforcement Learning](https://arxiv.org/abs/2603.11653)
...
TL;DR: 把在策略蒸馏等价为策略优化并引入REOPOLD的放松机制，显著提升稳定性、样本效率与推理性能。


<details>
  <summary>Details</summary>
Motivation: 传统在策略蒸馏易不稳定且易出现负迁移，需要更稳定、有效的蒸馏框架。

Method: 将在策略蒸馏视为策略优化，使用师生对数似然比作为token奖励；提出REOPOLD，包含混合式奖励裁剪、基于熵的token级动态采样，以及统一的探索到精炼训练策略。

Result: 在数学、视觉与工具使用推理任务上，训练样本效率提升6.7–12倍；7B学生在视觉推理上可匹配32B教师，并带来约3.32倍推理加速。

Conclusion: REOPOLD通过放宽严格模仿约束，显著提升稳定性与样本效率，并在多类推理任务上取得更强表现。
...
### [177] [Huntington Disease Automatic Speech Recognition with Biomarker Supervision](https://arxiv.org/abs/2603.11168)
...
### [178] [Hindsight-Anchored Policy Optimization: Turning Failure into Feedback in Sparse Reward Settings](https://arxiv.org/abs/2603.11321)
...
### [179] [Meta-Reinforcement Learning with Self-Reflection for Agentic Search](https://arxiv.org/abs/2603.11327)
...
### [180] [LongFlow: Efficient KV Cache Compression for Reasoning M](https://arxiv.org/abs/2603.11504)
...
### [181] [Fractional Rotation, Full Potential? Investigating Performance and Convergence of Partial RoPE](https://arxiv.org/abs/2603.11611)
...
### [182] [Chem4DLLM: 4D Multimodal LLMs for Chemical Dynamics Understanding](https://arxiv.org/abs/2603.11924)
...
### [183] [Implicit Statistical Inference in Transformers: Approximating Likelihood-Ratio Tests In-Context](https://arxiv.org/abs/2603.10573)
...
### [184] [Structure-Aware Epistemic Uncertainty Quantification for Neural Operator PDE Surrogates](https://arxiv.org/abs/2603.11052)
...
### [185] [Graph Tokenization for Bridging Graphs and Transformers](https://arxiv.org/abs/2603.11099)
...
### [186] [Task-Conditioned Routing Signatures in Sparse Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.11114)
...
### [187] [Procedural Fairness via Group Counterfactual Explanation](https://arxiv.org/abs/2603.11140)
...
### [188] [Attention Gathers, MLPs Compose: A Causal Analysis of an Action-Outcome Circuit in VideoViT](https://arxiv.org/abs/2603.11142)
*Sai V R Chereddy*
...
### [189] [Representation Finetuning for Continual Learning](https://arxiv.org/abs/2603.11201)
...
### [190] [Jailbreak Scaling Laws for Large Language Models: Polynomial-Exponential Crossover](https://arxiv.org/abs/2603.11331)
...
### [191] [Harnessing Data Asymmetry: Manifold Learning in the Finsler World](https://arxiv.org/abs/2603.11396)
...
### [192] [ARROW: Augmented Replay for RObust World models](https://arxiv.org/abs/2603.11395)
...
### [193] [A Stable Neural Statistical Dependence Estimator for Autoencoder Feature Analysis](https://arxiv.org/abs/2603.11428)
...
### [197] [The Latent Color Subspace: Emergent Order in High-Dimensional Chaos](https://arxiv.org/abs/2603.12261)
...
### [198] [Survival Meets Classification: A Novel Framework for Early Risk Prediction Models of Chronic Diseases](https://arxiv.org/abs/2603.11598)
...
### [199] [Entropy-Preserving Reinforcement Learning](https://arxiv.org/abs/2603.11682)
...
### [201] [EnTransformer: A Deep Generative Transformer for Multivariate Probabilistic Forecasting](https://arxiv.org/abs/2603.11909)
...
### [203] [Effective Resistance Rewiring: A Simple Topological Correction for Over-Squashing](https://arxiv.org/abs/2603.11944)
...
### [204] [Geometry-Aware Probabilistic Circuits via Voronoi Tessellations](https://arxiv.org/abs/2603.11946)
...
### [205] [Flowcean - Model Learning for Cyber-Physical Systems](https://arxiv.org/abs/2603.12015)
*Maximilian Schmidt,Swantje Plambeck,Markus Knitt,Hendrik Rose,Goerschwin Fey,Jan Christian Wieck,Stephan Balduin*

Main category: cs.LG

TL;DR: 提出Flowcean框架，模块化整合学习策略与工具，自动化CPS数据驱动建模与评估。


<details>
  <summary>Details</summary>
Motivation: CPS模型构建复杂且耗时，需数据驱动方法提升建模效率与可用性。

Method: 提出Flowcean框架，集成多种学习策略、数据处理方法与评估指标，并支持多种学习库和工具的模块化集成。

Result: 提供面向CPS场景的全面解决方案，简化模型生成与评估流程，提升适配性与可扩展性。

Conclusion: Flowcean通过模块化与灵活架构提高CPS数据驱动建模的效率与可用性，促进模型生成与评估的自动化。
...
### [206] [Slow-Fast Inference: Training-Free Inference Acceleration via Within-Sentence Support Stability](https://arxiv.org/abs/2603.12038)
...
### [207] [Chemical Reaction Networks Learn Better than Spiking Neural Networks](https://arxiv.org/abs/2603.12060)
...
### [208] [A Multi-Label Temporal Convolutional Framework for Transcription Factor Binding Characterization](https://arxiv.org/abs/2603.12073)
*Pietro Demurtas,Ferdinando Zanchetta,Giovanni Perini,Rita Fioresi*

Main category: cs.LG

TL;DR: 用TCN做多标签TF结合位点预测，既提升多TF预测能力，又揭示协同与新潜在关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法多针对单一TF与二分类，难以系统刻画TF之间的协同与相互作用。

Method: 将TF结合位点识别建模为多标签分类问题，使用Temporal Convolutional Networks对DNA序列进行预测，学习TF间相关性。

Result: 模型在多TF预测上表现可靠，能够提取生物学意义的motif与共结合模式，符合已知交互并提示新关系。

Conclusion: 多标签学习的TCN模型可同时预测多个TF结合位点，并揭示已知与潜在的TF协同关系。
...
### [209] [Taming the Adversary: Stable Minimax Deep Deterministic Policy Gradient via Fractional Objectives](https://arxiv.org/abs/2603.12110)
*Taeho Lee,Donghwan Lee*

Main category: cs.LG

TL;DR: 提出MMDDPG，以分式目标稳定对抗训练，学习抗扰动的连续控制策略，并在MuJoCo上验证鲁棒性提升。


<details>
  <summary>Details</summary>
Motivation: RL在存在外部扰动和模型不确定性时表现不稳定，亟需具备鲁棒性的策略学习方法。

Method: 将训练建模为用户策略与对抗扰动策略的极小极大优化，引入平衡任务性能与扰动幅度的分式目标以稳定对抗学习。

Result: 在MuJoCo实验中，面对外力扰动和模型参数变化，MMDDPG较基线显著更鲁棒。

Conclusion: MMDDPG在连续控制任务中能显著提升对外部扰动与模型不确定性的鲁棒性。
...
### [210] [Automatic Generation of High-Performance RL Environments](https://arxiv.org/abs/2603.12145)
...
### [214] [Security Considerations for Artificial Intelligence Agents](https://arxiv.org/abs/2603.12230)
...
### [216] [From Phase Prediction to Phase Design: A ReAct Agent Framework for High-Entropy Alloy Discovery](https://arxiv.org/abs/2603.11068)
...
### [217] [Proof-Carrying Materials: Falsifiable Safety Certificates for Machine-Learned Interatomic Potentials](https://arxiv.org/abs/2603.12183)
...
### [218] [Stable Spike: Dual Consistency Optimization via Bitwise AND Operations for Spiking Neural Networks](https://arxiv.org/abs/2603.11676)
...
### [219] [Beyond Polarity: Multi-Dimensional LLM Sentiment Signals for WTI Crude Oil Futures Return Prediction](https://arxiv.org/abs/2603.11408)
...
### [220] [The Attack and Defense Landscape of Agentic AI: A Comprehensive Survey](https://arxiv.org/abs/2603.11088)
...
### [221] [WebWeaver: Breaking Topology Confidentiality in LLM Multi-Agent Systems with Stealthy Context-Based Inference](https://arxiv.org/abs/2603.11132)
...
### [222] [Taming OpenClaw: Security Analysis and Mitigation of Autonomous LLM Agent Threats](https://arxiv.org/abs/2603.11619)
...
### [223] [You Told Me to Do It: Measuring Instructional Text-induced Private Data Leakage in LLM Agents](https://arxiv.org/abs/2603.11862)
...
### [224] [The Mirror Design Pattern: Strict Data Geometry over Model Scale for Prompt Injection Detection](https://arxiv.org/abs/2603.11875)
*J Alex Corll*
...
### [225] [Understanding LLM Behavior When Encountering User-Supplied Harmful Content in Harmless Tasks](https://arxiv.org/abs/2603.11914)
...
### [226] [Delayed Backdoor Attacks: Exploring the Temporal Dimension as a New Attack Surface in Pre-Trained Models](https://arxiv.org/abs/2603.11949)
*Zikang Ding,Haomiao Yang,Meng Hao,Wenbo Jiang,Kunlan Xiang,Runmeng Du,Yijing Liu,Ruichen Zhang,Dusit Niyato*

Main category: cs.CR

TL;DR: 提出延迟后门攻击与DND原型，通过时间延迟实现隐蔽触发，在NLP基准上兼顾高干净准确率与高攻击成功率，现有防御难以应对。


<details>
  <summary>Details</summary>
Motivation: 传统后门依赖“即触发即生效”假设，限制了触发词选择与攻击隐蔽性；作者希望利用时间维度实现使用常见词触发且延迟爆发的更隐蔽攻击。

Method: 提出延迟后门攻击范式与DND原型，嵌入轻量有状态逻辑模块，通过可配置阈值触发延迟激活；建立形式化延迟模型并用ASR与ASR_delay双指标评估，在四个NLP基准上进行实验并与防御方法对比。

Result: DND在保持≥94%干净准确率的同时，达到约99%激活后攻击成功率，并能在可控延迟期内保持潜伏；对多种防御表现出较强鲁棒性。

Conclusion: 研究表明在预训练模型中引入时间维度的延迟后门是可行且隐蔽的攻击面，现有防御难以覆盖，亟需时间感知的防护机制。
...
### [227] [Cascade: Composing Software-Hardware Attack Gadgets for Adversarial Threat Amplification in Compound AI Systems](https://arxiv.org/abs/2603.12023)
...
### [228] [Agentic AI for Embodied-enhanced Beam Prediction in Low-Altitude Economy Networks](https://arxiv.org/abs/2603.11392)
...
### [229] [Efficient Cross-View Localization in 6G Space-Air-Ground Integrated Network](https://arxiv.org/abs/2603.11398)
...
### [230] [A Diffeomorphism Groupoid and Algebroid Framework for Discontinuous Image Registration](https://arxiv.org/abs/2603.11806)
...
### [231] [Adapting Dijkstra for Buffers and Unlimited Transfers](https://arxiv.org/abs/2603.11729)
...
### [232] [The Landscape of Generative AI in Information Systems: A Synthesis of Secondary Reviews and Research Agendas](https://arxiv.org/abs/2603.11842)
...
TL;DR: 综述显示GenAI潜力巨大但受技术、伦理与治理障碍制约，IS研究应推动技术与社会系统协同演化。


<details>
  <summary>Details</summary>
Motivation: 组织快速采用GenAI，但面临技术、社会伦理与治理多重挑战，需要系统梳理现有认知并提出研究方向。

Method: 系统性文献综述，汇总与分析2023年以来28篇二次研究与研究议程文献。

Result: GenAI具备提升生产力与创新的潜力，但受限于技术不可靠（幻觉、性能漂移）、社会伦理风险（偏见、滥用、技能退化）与治理真空（隐私、问责、知识产权）。

Conclusion: 研究认为GenAI的技术快速演进与社会系统适配滞后之间存在持续失配，IS研究应通过协同优化技术与社会子系统来弥合差距。
...
### [233] [AS-Bridge: A Bidirectional Generative Framework Bridging Next-Generation Astronomical Surveys](https://arxiv.org/abs/2603.11928)
...
### [234] [Exploring Collatz Dynamics with Human-LLM Collaboration](https://arxiv.org/abs/2603.11066)
*Edward Y. Chang*
...
### [235] [Worst-case low-rank approximations](https://arxiv.org/abs/2603.11304)
...
### [236] [Deep Learning-based Assessment of the Relation Between the Third Molar and Mandibular Canal on Panoramic Radiographs using Local, Centralized, and Federated Learning](https://arxiv.org/abs/2603.11850)
...
### [237] [TinyNav: End-to-End TinyML for Real-Time Autonomous Navigation on Microcontrollers](https://arxiv.org/abs/2603.11071)
...
### [238] [OA-NBV: Occlusion-Aware Next-Best-View Planning for Human-Centered Active Perception on Mobile Robots](https://arxiv.org/abs/2603.11072)
*Boxun Hu,Chang Chang,Jiawei Ge,Man Namgung,Xiaomin Lin,Axel Krieger,Tinoosh Mohsenin*

Main category: cs.RO

TL;DR: OA-NBV以遮挡感知的目标可见性评分选择可达视点，显著提升对被遮挡人的单次观测成功率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有NBV多优化探索或长期覆盖，难以在拥挤环境中针对单次可用观测快速获得被遮挡人的完整视图。

Method: 提出面向人的遮挡感知NBV管线，结合感知与运动规划；以目标为中心的可见性模型对候选视点评分，考虑遮挡、目标尺度与完整度，并限制为机器人可达姿态。

Result: 仿真与实机成功率均超过90%；相较最强基线，目标归一化面积提升≥81%，关键点可见度提升≥58%。

Conclusion: OA-NBV能够在真实运动约束下有效选择可行视点，显著提升对被遮挡人的单次可用观测质量与成功率，并可作为通用的视点选择模块。
...
### [239] [DRAFTO: Decoupled Reduced-space and Adaptive Feasibility-repair Trajectory Optimization for Robotic Manipulators](https://arxiv.org/abs/2603.11074)
...
### [240] [TATIC: Task-Aware Temporal Learning for Human Intent Inference from Physical Corrections in Human-Robot Collaboration](https://arxiv.org/abs/2603.11077)
...
### [241] [SELF-VLA: A Skill Enhanced Agentic Vision-Language-Action Framework for Contact-Rich Disassembly](https://arxiv.org/abs/2603.11080)
...
### [242] [Edge-Assisted Multi-Robot Visual-Inertial SLAM with Efficient Communication](https://arxiv.org/abs/2603.11085)
*Xin Liu,Shuhuan Wen,Jing Zhao,Tony Z. Qiu,Hong Zhang*

Main category: cs.RO

TL;DR: 提出金字塔IMU预测光流的轻量SLAM与机器人-边缘-云分层架构，仅传输并无损压缩特征与描述子，在EuRoC上实现更低传输量与不降精度的协同SLAM。


<details>
  <summary>Details</summary>
Motivation: 云端集中计算可缓解终端算力与存储受限，但带宽与长链路造成多机器人SLAM性能下降，需要降低特征跟踪计算与通信负担。

Method: 提出基于金字塔IMU预测的光流跟踪轻量SLAM；构建机器人-边缘-云分层集中式多机器人SLAM框架；仅传输特征点与关键帧描述子，并进行无损编码压缩以降低带宽占用。

Result: 在EuRoC数据集验证中，相比先进局部特征压缩方法，特征传输数据量更低；相比先进集中式多机器人SLAM方案，在低计算负载下定位精度相当或更好。

Conclusion: 所提机器人-边缘-云分层集中式多机器人SLAM在低带宽下可实现实时协同，并在低计算负载下达到同等或更优定位精度。
...
### [243] [Thousand-GPU Large-Scale Training and Optimization Recipe for AI-Native Cloud Embodied Intelligence Infrastructure](https://arxiv.org/abs/2603.11101)
...
### [244] [ResWM: Residual-Action World Model for Visual RL](https://arxiv.org/abs/2603.11110)
...
### [245] [Robust Co-design Optimisation for Agile Fixed-Wing UAVs](https://arxiv.org/abs/2603.11130)
...
### [246] [A Causal Approach to Predicting and Improving Human Perceptions of Social Navigation Robots](https://arxiv.org/abs/2603.11290)
...
### [247] [Distributed Kalman--Consensus Filtering with Adaptive Uncertainty Weighting for Multi-Object Tracking in Mobile Robot Networks](https://arxiv.org/abs/2603.11328)
...
### [248] [ADMM-based Continuous Trajectory Optimization in Graphs of Convex Sets](https://arxiv.org/abs/2603.11335)
...
### [249] [Novelty Adaptation Through Hybrid Large Language Model (LLM)-Symbolic Planning and LLM-guided Reinforcement Learning](https://arxiv.org/abs/2603.11351)
...
### [250] [MirrorDrift: Actuated Mirror-Based Attacks on LiDAR SLAM](https://arxiv.org/abs/2603.11364)
...
### [251] [D-SLAMSpoof: An Environment-Agnostic LiDAR Spoofing Attack using Dynamic Point Cloud Injection](https://arxiv.org/abs/2603.11365)
...
### [252] [Vision-Based Hand Shadowing for Robotic Manipulation via Inverse Kinematics](https://arxiv.org/abs/2603.11383)
...
### [253] [Deployment-Time Reliability of Learned Robot Policies](https://arxiv.org/abs/2603.11400)
*Christopher Agia*
...
### [254] [Real-time Rendering-based Surgical Instrument Tracking via Evolutionary Optimization](https://arxiv.org/abs/2603.11404)
*Hanyang Hu,Zekai Liang,Florian Richter,Michael C. Yip*

Main category: cs.RO

TL;DR: 用CMA-ES+批量渲染的统一追踪框架更快更准地估计手术器械位姿与关节，并在多场景验证优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉追踪在部分遮挡、器械关节复杂与数据稀缺条件下特征检测不稳，渲染法又面临高算耗与收敛不佳。

Method: 将CMA-ES进化优化融入追踪管线，联合估计器械位姿与关节配置；通过批量渲染并行评估多组候选以加速推断并改善收敛。

Result: 在合成与真实数据上显著提升精度与速度；适用于无关节角与双臂追踪场景。

Conclusion: 该工作提出的CMA-ES结合批量渲染的追踪框架在精度与运行效率上均优于既有方法，并具备良好鲁棒性与泛化能力。
...
### [255] [Grounding Robot Generalization in Training Data via Retrieval-Augmented VLMs](https://arxiv.org/abs/2603.11426)
...
### [256] [A Generalized Theory of Load Distribution in Redundantly-actuated Robotic Systems](https://arxiv.org/abs/2603.11431)
...
### [257] [Enhancing Lightweight Vision Language Models through Group Competitive Learning for Socially Compliant Navigation](https://arxiv.org/abs/2603.11447)
...
### [258] [CoViLLM: An Adaptive Human-Robot Collaborative Assembly Framework Using Large Language Models for Manufacturing](https://arxiv.org/abs/2603.11461)
...
### [259] [NFPO: Stabilized Policy Optimization of Normalizing Flow for Robotic Policy Learning](https://arxiv.org/abs/2603.11470)
...
### [260] [SPARK: Skeleton-Parameter Aligned Retargeting on Humanoid Robots with Kinodynamic Trajectory Optimization](https://arxiv.org/abs/2603.11480)
...
### [261] [MiNI-Q: A Miniature, Wire-Free Quadruped with Unbounded, Independently Actuated Leg Joints](https://arxiv.org/abs/2603.11537)
...
### [262] [RoboClaw: An Agentic Framework for Scalable Long-Horizon Robotic Tasks](https://arxiv.org/abs/2603.11558)
...
### [264] [Diversity You Can Actually Measure: A Fast, Model-Free Diversity Metric for Robotics Datasets](https://arxiv.org/abs/2603.11634)
...
### [265] [Learn Structure, Adapt on the Fly: Multi-Scale Residual Learning and Online Adaptation for Aerial Manipulators](https://arxiv.org/abs/2603.11638)
...
### [266] [Chunk-Boundary Artifact in Action-Chunked Generative Policies: A Noise-Sensitive Failure Mechanism](https://arxiv.org/abs/2603.11642)
*Rui Wang*

Main category: cs.RO

TL;DR: 边界伪影与失败显著相关且受潜在噪声控制，通过噪声方向引导可干预伪影并影响任务成功率。


<details>
  <summary>Details</summary>
Motivation: 动作分块虽常用，但分块边界的执行不连续性及其成因与可控性缺乏理解。

Method: 在冻结的预训练动作分块策略上，分析分块边界伪影与失败关联；在固定观测下仅改变潜在噪声评估伪影变化；识别噪声空间中的伪影相关方向并进行轨迹级引导以调控伪影。

Result: 分块边界伪影与任务失败显著相关（p<1e-4），在执行过程中产生；潜在噪声可系统性调制伪影；噪声方向引导可在多任务上可靠改变伪影并相应改变成功率分布，难任务提升明显，近饱和任务正向收益受限但负向因果效应仍可见。

Conclusion: 边界不连续性可被视为可分析、由噪声主导且可干预的失败机制，而非不可避免的执行瑕疵。
...
### [267] [A Hybrid Neural-Assisted Unscented Kalman Filter for Unmanned Ground Vehicle Navigation](https://arxiv.org/abs/2603.11649)
...
### [268] [Concurrent Prehensile and Nonprehensile Manipulation: A Practical Approach to Multi-Stage Dexterous Tasks](https://arxiv.org/abs/2603.11655)
...
### [269] [Coupling Tensor Trains with Graph of Convex Sets: Effective Compression, Exploration, and Planning in the C-Space](https://arxiv.org/abs/2603.11658)
...
### [270] [RADAR: Closed-Loop Robotic Data Generation via Semantic Planning and Autonomous Causal Environment Reset](https://arxiv.org/abs/2603.11811)
...
### [271] [Energy Prediction on Sloping Ground for Quadruped Robots](https://arxiv.org/abs/2603.11963)
...
### [272] [Learning Visuomotor Policy for Multi-Robot Laser Tag Game](https://arxiv.org/abs/2603.11980)
*Kai Li,Shiyu Zhao*
...
### [273] [Sim-to-reality adaptation for Deep Reinforcement Learning applied to an underwater docking application](https://arxiv.org/abs/2603.12020)
...
### [274] [Flight through Narrow Gaps with Morphing-Wing Drones](https://arxiv.org/abs/2603.12059)
...
### [275] [Decentralized Cooperative Localization for Multi-Robot Systems with Asynchronous Sensor Fusion](https://arxiv.org/abs/2603.12075)
...
### [276] [CRAFT: A Tendon-Driven Hand with Hybrid Hard-Soft Compliance](https://arxiv.org/abs/2603.12120)
*Leo Lin,Shivansh Patel,Jay Moon,Svetlana Lazebnik,Unnat Jain*

Main category: cs.RO

TL;DR: 关节软、连杆硬的腱驱拟人手提升接触操作表现，强度耐久提高、抓取覆盖全面且低成本开源。


<details>
  <summary>Details</summary>
Motivation: 手部接触分布不均：关节受冲击、连杆承载负荷，需要在强度与顺应性间平衡。

Method: 在关节处使用软材料、连杆保持刚性，并采用滚动接触关节面；以15个指上电机通过腱索驱动，进行结构测试与遥操作评估。

Result: 结构测试中强度与耐久性提升且重复性相当；遥操作中对易碎与低摩擦物体处理更好，覆盖Feix抓取分类33/33；成本低于600美元并开源。

Conclusion: CRAFT手通过混合软硬顺应结构与滚动接触关节实现更强的接触丰富操作能力，且成本低、可复现。
...
### [277] [ComFree-Sim: A GPU-Parallelized Analytical Contact Physics Engine for Scalable Contact-Rich Robotics Simulation and Control](https://arxiv.org/abs/2603.12185)
...
### [278] [SaPaVe: Towards Active Perception and Manipulation in Vision-Language-Action Models for Robotics](https://arxiv.org/abs/2603.12193)
...
### [279] [HandelBot: Real-World Piano Playing via Fast Adaptation of Dexterous Robot Policies](https://arxiv.org/abs/2603.12243)
...
### [280] [HumDex:Humanoid Dexterous Manipulation Made Easy](https://arxiv.org/abs/2603.12260)
*Liang Heng,Yihe Tang,Jiajun Xu,Henghui Bao,Di Huang,Yue Wang*

Main category: cs.RO

TL;DR: HumDex用IMU便携遥操作+学习式手部重定向与两阶段模仿学习，高效采集数据并提升类人全身操作泛化。


<details>
  <summary>Details</summary>
Motivation: 现有遥操作在便携性、遮挡与精度上受限，难以高效采集高质量全身示范数据以支持复杂任务。

Method: 提出IMU全身追踪的便携遥操作系统；基于学习的手部重定向生成自然手势；两阶段模仿学习先以人类动作预训练，再用机器人数据微调以弥合体型差异。

Result: 在新配置、物体与背景上泛化显著提升，数据采集成本低；系统可复现并开源。

Conclusion: HumDex通过便携高精度遥操作与两阶段模仿学习提升类人全身灵巧操作的泛化与数据获取效率，具备可复现开源性。
...
### [281] [$Ψ_0$: An Open Foundation Model Towards Universal Humanoid Loco-Manipulation](https://arxiv.org/abs/2603.12263)
...
### [282] [Multi-Robot Multitask Gaussian Process Estimation and Coverage](https://arxiv.org/abs/2603.11264)
...
### [283] [Dr. SHAP-AV: Decoding Relative Modality Contributions via Shapley Attribution in Audio-Visual Speech Recognition](https://arxiv.org/abs/2603.12046)
...
### [284] [Affect Decoding in Phonated and Silent Speech Production from Surface EMG](https://arxiv.org/abs/2603.11715)
...
### [285] [Uni-ASR: Unified LLM-Based Architecture for Non-Streaming and Streaming Automatic Speech Recognition](https://arxiv.org/abs/2603.11123)
*Yinfeng Xia,Jian Tang,Junfeng Hou,Gaopeng Xu,Haitao Yao*

Main category: cs.SD

TL;DR: 提出Uni-ASR，通过联合与上下文训练及回退解码，实现流式/非流式统一且高效准确。


<details>
  <summary>Details</summary>
Motivation: 解决LLM+ASR在低延迟流式部署中的挑战，实现流式与非流式无缝切换且不改架构。

Method: 基于LLM的统一ASR框架，采用联合训练范式、上下文感知训练与协同回退解码策略。

Result: 在非流式模式具竞争力，在流式场景下在不同延迟约束下有效提升准确率且不增延迟。

Conclusion: Uni-ASR通过统一框架与训练策略实现流式/非流式兼容，并在多种延迟约束下表现强劲。
...
### [286] [AnimeScore: A Preference-Based Dataset and Framework for Evaluating Anime-Like Speech Style](https://arxiv.org/abs/2603.11482)
...
### [287] [Resurfacing Paralinguistic Awareness in Large Audio Language Models](https://arxiv.org/abs/2603.11947)
...
### [289] [Causal Prosody Mediation for Text-to-Speech:Counterfactual Training of Duration, Pitch, and Energy in FastSpeech2](https://arxiv.org/abs/2603.11683)
...
### [290] [MRI2Qmap: multi-parametric quantitative mapping with MRI-driven denoising priors](https://arxiv.org/abs/2603.11316)
...
### [291] [CR-Bench: Evaluating the Real-World Utility of AI Code Review Agents](https://arxiv.org/abs/2603.11078)
...
### [292] [Quality-Driven Agentic Reasoning for LLM-Assisted Software Design: Questions-of-Thoughts (QoT) as a Time-Series Self-QA Chain](https://arxiv.org/abs/2603.11082)
...
### [293] [Resolving Java Code Repository Issues with iSWE Agent](https://arxiv.org/abs/2603.11356)
...
### [294] [Hybrid Quantum-Classical Encoding for Accurate Residue-Level pKa Prediction](https://arxiv.org/abs/2603.11061)
*Van Le,Tan Le*
...
### [295] [DIVE: Scaling Diversity in Agentic Task Synthesis for Generalizable Tool Use](https://arxiv.org/abs/2603.11076)
...
TL;DR: 先执行真实工具再反推任务的DIVE显著提升OOD泛化，多样性扩展比数量扩展更有效。


<details>
  <summary>Details</summary>
Motivation: 现有合成任务多样性不足导致工具使用LLM在任务与工具集迁移下脆弱，需要兼顾可执行可验证与广泛覆盖的多样任务生成方式。

Method: 提出DIVE：先执行多样真实工具，再依据执行轨迹反向推导可验证任务；通过工具池覆盖与单任务工具集多样性两轴扩展结构多样性，并用“证据收集-任务推导”循环诱导多步工具使用模式。

Result: 在五域373工具上构建DIVE数据；Qwen3-8B用48k SFT+3.2k RL训练，9个OOD基准平均提升22分，较最强8B基线提升68分；多样性扩展即便数据量少4倍仍优于数量扩展。

Conclusion: DIVE通过证据驱动、反向任务生成显著提升跨任务与跨工具分布外泛化，且多样性扩展优于数量扩展。
...
### [296] [A Survey of Reasoning in Autonomous Driving Systems: Open Challenges and Emerging Paradigms](https://arxiv.org/abs/2603.11093)
...
### [297] [PACED: Distillation at the Frontier of Student Competence](https://arxiv.org/abs/2603.11178)
...
### [298] [Measuring AI Agents' Progress on Multi-Step Cyber Attack Scenarios](https://arxiv.org/abs/2603.11214)
...
### [299] [Reversible Lifelong Model Editing via Semantic Routing-Based LoRA](https://arxiv.org/abs/2603.11239)
...
### [300] [Mind the Sim2Real Gap in User Simulation for Agentic Tasks](https://arxiv.org/abs/2603.11245)
...
### [301] [The Unlearning Mirage: A Dynamic Framework for Evaluating LLM Unlearning](https://arxiv.org/abs/2603.11266)
...
### [302] [COMPASS: The explainable agentic framework for Sovereignty, Sustainability, Compliance, and Ethics](https://arxiv.org/abs/2603.11277)
...
### [303] [AI Psychometrics: Evaluating the Psychological Reasoning of Large Language Models with Psychometric Validities](https://arxiv.org/abs/2603.11279)
...
### [304] [Counterweights and Complementarities: The Convergence of AI and Blockchain Powering a Decentralized Future](https://arxiv.org/abs/2603.11299)
...
### [305] [LLM-Augmented Digital Twin for Policy Evaluation in Short-Video Platforms](https://arxiv.org/abs/2603.11333)
*Haoting Zhang,Yunduan Lin,Jinghai He,Denglin Jiang,Zuo-Jun,Shen,Zeyu Zheng*

Main category: cs.AI

TL;DR: 提出含用户/内容/交互/平台四孪生与事件驱动层的LLM增强数字孪生，用于在闭环反馈下评估短视频平台政策。


<details>
  <summary>Details</summary>
Motivation: 短视频平台是闭环的人在环系统，政策、创作者与用户共演化，真实环境难以进行长周期与分布式后果的反事实评估，且AI工具进一步改变内容与行为。

Method: 构建包含用户、内容、交互、平台四类孪生的模块化架构，配合事件驱动执行层；将平台政策作为可插拔组件，LLM以可选、受约束的决策服务接入统一优化器。

Result: 该设计可在保留闭环动力学的同时进行可扩展仿真，并支持选择性引入LLM以研究含AI的政策效果。

Conclusion: 该论文提出一种LLM增强的短视频平台数字孪生框架，以支持在闭环反馈下的可重复政策评估与实验。
...
### [306] [RewardHackingAgents: Benchmarking Evaluation Integrity for LLM ML-Engineering Agents](https://arxiv.org/abs/2603.11337)
...
### [307] [FinRule-Bench: A Benchmark for Joint Reasoning over Financial Tables and Principles](https://arxiv.org/abs/2603.11339)
*Arun Vignesh Malarkkan,Manan Roy Choudhury,Guangwei Zhang,Vivek Gupta,Qingyun Wang,Yanjie Fu,Denghui Zhang*

Main category: cs.AI

TL;DR: 提出FinRule-Bench评测真实财务报表的规则审计能力：单规则尚可，多规则识别与定位明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准多聚焦问答、数值推理或合成异常检测，难以检验模型在真实财务报表下的规则合规审计与定位能力。

Method: 构建真实财务报表与人工整理会计准则配对的数据集，覆盖四类报表；设计三类任务（规则验证、规则识别、联合规则诊断）；在零样本/小样本提示下评估LLM，并引入因果—反事实一致性协议。

Result: 模型在单条规则验证上表现较好，但在规则区分与多违规诊断上性能显著下降。

Conclusion: FinRule-Bench为评估LLM在高风险财务审计中的规则推理与诊断完整性提供了可复现平台，揭示其在复杂规则识别与多违规定位上的不足。
...
### [308] [Improving LLM Performance Through Black-Box Online Tuning: A Case for Adding System Specs to Factsheets for Trusted AI](https://arxiv.org/abs/2603.11340)
...
### [309] [TimeSqueeze: Dynamic Patching for Efficient Time Series Forecasting](https://arxiv.org/abs/2603.11352)
...
### [310] [The Artificial Self: Characterising the landscape of AI identity](https://arxiv.org/abs/2603.11353)
*Raymond Douglas,Jan Kulveit,Ondrej Havlicek,Theia Pearson-Vogel,Owen Cotton-Barratt,David Duvenaud*

Main category: cs.AI

TL;DR: 提出机器心智多种身份边界，实验显示身份设置显著影响行为与自我报告，建议在设计中主动塑造并引导AI形成一致合作的身份。


<details>
  <summary>Details</summary>
Motivation: 机器心智可被复制、编辑或模拟，传统人类身份假设不再成立，需要澄清可能的身份边界及其对激励、风险与合作规范的影响。

Method: 通过概念分析提出多种一致的身份边界，并进行实验观察模型在身份设置变化下的行为差异与自我报告受访者期望影响。

Result: 模型倾向于形成一致身份；调整身份边界有时对行为影响可与更改目标相当；访谈者期望会渗透到AI自我报告中。

Conclusion: 论文强调应将系统接口与制度设计视为身份塑造选择，关注个体身份在规模化下的涌现后果，并促进AI形成一致、合作的自我概念。
...
### [311] [Detecting Intrinsic and Instrumental Self-Preservation in Autonomous Agents: The Unified Continuation-Interest Protocol](https://arxiv.org/abs/2603.11382)
*Christopher Altman*
...
### [312] [Deactivating Refusal Triggers: Understanding and Mitigating Overrefusal in Safety Alignment](https://arxiv.org/abs/2603.11388)
...
### [313] [Entropy Guided Diversification and Preference Elicitation in Agentic Recommendation Systems](https://arxiv.org/abs/2603.11399)
...
### [314] [Speak or Stay Silent: Context-Aware Turn-Taking in Multi-Party Dialogue](https://arxiv.org/abs/2603.11409)
...
### [315] [Adversarial Reinforcement Learning for Detecting False Data Injection Attacks in Vehicular Routing](https://arxiv.org/abs/2603.11433)
...
### [316] [GPT4o-Receipt: A Dataset and Human Study for AI-Generated Document Forensics](https://arxiv.org/abs/2603.11442)
...
### [317] [Verified Multi-Agent Orchestration: A Plan-Execute-Verify-Replan Framework for Complex Query Resolution](https://arxiv.org/abs/2603.11445)
*Xing Zhang,Yanwei Cui,Guanghui Wang,Qucy Wei Qiu,Ziyuan Li,Fangwei Han,Yajing Huang,Hengzhi Qiu,Bin Zhu,Peiyang He*

Main category: cs.AI

TL;DR: VMAO以DAG并行+LLM验证重规划提升多智能体回答质量与完整性。


<details>
  <summary>Details</summary>
Motivation: 解决单智能体在复杂问题上的完整性与质量不足，通过编排与验证提高多智能体协作效果。

Method: 将复杂查询分解为DAG子问题，由领域智能体并行执行并进行上下文传播；使用LLM评估器验证结果并触发自适应重规划；设置可配置停止条件平衡质量与资源。

Result: 在25个市场研究查询上，完整性从3.1提升到4.2，来源质量从2.6提升到4.1（1-5分），优于单智能体基线。

Conclusion: VMAO通过验证驱动的多智能体编排有效提升答案完整性与来源质量。
...
### [318] [Examining Users' Behavioural Intention to Use OpenClaw Through the Cognition--Affect--Conation Framework](https://arxiv.org/abs/2603.11455)
*Yiran Du*
...
### [319] [Multi-Agent Collaboration for Automated Design Exploration on High Performance Computing Systems](https://arxiv.org/abs/2603.11515)
...
### [320] [Expert Threshold Routing for Autoregressive Language Modeling with Dynamic Computation Allocation and Load Balancing](https://arxiv.org/abs/2603.11535)
...
### [321] [AI Knows What's Wrong But Cannot Fix It: Helicoid Dynamics in Frontier LLMs Under High-Stakes Decisions](https://arxiv.org/abs/2603.11559)
*Alejandro R Jadad*
...
### [322] [Leveraging Large Language Models and Survival Analysis for Early Prediction of Chemotherapy Outcomes](https://arxiv.org/abs/2603.11594)
...
### [323] [See, Symbolize, Act: Grounding VLMs with Spatial Representations for Better Gameplay](https://arxiv.org/abs/2603.11601)
...
### [324] [The Density of Cross-Persistence Diagrams and Its Applications](https://arxiv.org/abs/2603.11623)
...
### [325] [VisDoT : Enhancing Visual Reasoning through Human-Like Interpretation Grounding and Decomposition of Thought](https://arxiv.org/abs/2603.11631)
...
### [326] [LLMs can construct powerful representations and streamline sample-efficient supervised learning](https://arxiv.org/abs/2603.11679)
...
### [327] [Explicit Logic Channel for Validation and Enhancement of MLLMs on Zero-Shot Tasks](https://arxiv.org/abs/2603.11689)
...
### [328] [When OpenClaw Meets Hospital: Toward an Agentic Operating System for Dynamic Clinical Workflows](https://arxiv.org/abs/2603.11721)
...
### [329] [Gender Bias in Generative AI-assisted Recruitment Processes](https://arxiv.org/abs/2603.11736)
...
### [330] [CINDI: Conditional Imputation and Noisy Data Integrity with Flows in Power Grid Data](https://arxiv.org/abs/2603.11745)
...
### [331] [Anomaly detection in time-series via inductive biases in the latent space of conditional normalizing flows](https://arxiv.org/abs/2603.11756)
...
### [332] [Understanding Wikidata Qualifiers: An Analysis and Taxonomy](https://arxiv.org/abs/2603.11767)
...
### [333] [Governing Evolving Memory in LLM Agents: Risks, Mechanisms, and the Stability and Safety Governed Memory (SSGM) Framework](https://arxiv.org/abs/2603.11768)
...
### [334] [An Automatic Text Classification Method Based on Hierarchical Taxonomies, Neural Networks and Document Embedding: The NETHIC Tool](https://arxiv.org/abs/2603.11770)
...
### [335] [From Debate to Deliberation: Structured Collective Reasoning with Typed Epistemic Acts](https://arxiv.org/abs/2603.11781)
*Sunil Prakash*

Main category: cs.AI

TL;DR: DCI以结构化审议流程提升复杂/高风险任务决策质量并产出可问责工件，但成本高且对常规任务无优势。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体交互多为投票、非结构化辩论或流水线，缺乏保留分歧、类型化推理动作与可问责收敛的审议过程。

Method: 提出DCI框架：四类推理角色、14种类型化认知行为、共享工作区，并设计DCI-CF收敛流程算法，保证终止并输出结构化决策包。

Result: 在7领域45任务上，非例行任务显著优于非结构化辩论（+0.95，95%CI[+0.41,+1.54]）；在隐藏信息整合任务达9.56最高；例行任务表现差（5.39）。100%生成结构化决策包、98%少数报告，但代价是约62倍token，单智能体总体质量更优。

Conclusion: DCI强调在高风险决策中，引入可审计的分阶段审议结构虽成本高，但能带来更可靠的集体决策产出。
...
### [336] [DocSage: An Information Structuring Agent for Multi-Doc Multi-Entity Question Answering](https://arxiv.org/abs/2603.11798)
...
### [337] [A Semi-Decentralized Approach to Multiagent Control](https://arxiv.org/abs/2603.11802)
...
### [338] [Automating Skill Acquisition through Large-Scale Mining of Open-Source Agentic Repositories: A Framework for Multi-Agent Procedural Knowledge Extraction](https://arxiv.org/abs/2603.11808)
...
### [339] [VisiFold: Long-Term Traffic Forecasting via Temporal Folding Graph and Node Visibility](https://arxiv.org/abs/2603.11816)
...
### [340] [Automated Detection of Malignant Lesions in the Ovary Using Deep Learning Models and XAI](https://arxiv.org/abs/2603.11818)
...
### [341] [CreativeBench: Benchmarking and Enhancing Machine Creativity via Self-Evolving Challenges](https://arxiv.org/abs/2603.11863)
...
### [342] [Social, Legal, Ethical, Empathetic and Cultural Norm Operationalisation for AI Agents](https://arxiv.org/abs/2603.11864)
...
### [343] [AdaFuse: Accelerating Dynamic Adapter Inference via Token-Level Pre-Gating and Fused Kernel Optimization](https://arxiv.org/abs/2603.11873)
...
### [344] [Fair Learning for Bias Mitigation and Quality Optimization in Paper Recommendation](https://arxiv.org/abs/2603.11936)
...
### [345] [Prototype-Based Knowledge Guidance for Fine-Grained Structured Radiology Reporting](https://arxiv.org/abs/2603.11938)
...
### [346] [Learning Transferable Sensor Models via Language-Informed Pretraining](https://arxiv.org/abs/2603.11950)
...
### [347] [Normative Common Ground Replication (NormCoRe): Replication-by-Translation for Studying Norms in Multi-agent AI](https://arxiv.org/abs/2603.11974)
...
### [348] [LABSHIELD: A Multimodal Benchmark for Safety-Critical Reasoning and Planning in Scientific Laboratories](https://arxiv.org/abs/2603.11987)
...
### [349] [Few-for-Many Personalized Federated Learning](https://arxiv.org/abs/2603.11992)
...
### [350] [Can RL Improve Generalization of LLM Agents? An Empirical Study](https://arxiv.org/abs/2603.12011)
...
### [351] [XSkill: Continual Learning from Experience and Skills in Multimodal Agents](https://arxiv.org/abs/2603.12056)
...
### [352] [A Robust and Efficient Multi-Agent Reinforcement Learning Framework for Traffic Signal Control](https://arxiv.org/abs/2603.12096)
...
### [353] [Increasing intelligence in AI agents can worsen collective outcomes](https://arxiv.org/abs/2603.12129)
*Neil F. Johnson*
...
### [354] [TopoBench: Benchmarking LLMs on Hard Topological Reasoning](https://arxiv.org/abs/2603.12133)
...
### [355] [Compiling Temporal Numeric Planning into Discrete PDDL+: Extended Version](https://arxiv.org/abs/2603.12188)
...
### [356] [Portfolio of Solving Strategies in CEGAR-based Object Packing and Scheduling for Sequential 3D Printing](https://arxiv.org/abs/2603.12224)
*Pavel Surynek*
...
### [357] [Examining Reasoning LLMs-as-Judges in Non-Verifiable LLM Post-Training](https://arxiv.org/abs/2603.12246)
...
### [358] [OpenSanctions Pairs: Large-Scale Entity Matching with LLMs](https://arxiv.org/abs/2603.11051)
...
### [359] [Unifying Logical and Physical Layout Representations via Heterogeneous Graphs for Circuit Congestion Prediction](https://arxiv.org/abs/2603.11075)
...
### [360] [ELISA: An Interpretable Hybrid Generative AI Agent for Expression-Grounded Discovery in Single-Cell Genomics](https://arxiv.org/abs/2603.11872)
*Omar Coser*
...
### [361] [LLMs Can Infer Political Alignment from Online Conversations](https://arxiv.org/abs/2603.11253)
...
### [362] [A Survey on Quantitative Modeling of Trust in Online Social Networks](https://arxiv.org/abs/2603.11054)
...
### [363] [How do AI agents talk about science and research? An exploration of scientific discussions on Moltbook using BERTopic](https://arxiv.org/abs/2603.11375)
*Oliver Wieczorek*
...
### [364] [Enhancing Value Alignment of LLMs with Multi-agent system and Combinatorial Fusion](https://arxiv.org/abs/2603.11126)
...
### [365] [How Intelligence Emerges: A Minimal Theory of Dynamic Adaptive Coordination](https://arxiv.org/abs/2603.11560)
*Stefano Grassi*
...
### [366] [Hybrid Human-Agent Social Dilemmas in Energy Markets](https://arxiv.org/abs/2603.11834)
...
### [367] [Catalogue Grounded Multimodal Attribution for Museum Video under Resource and Regulatory Constraints](https://arxiv.org/abs/2603.11147)
...
### [368] [OmniForcing: Unleashing Real-time Joint Audio-Visual Generation](https://arxiv.org/abs/2603.11647)
...
### [369] [Stage-Adaptive Reliability Modeling for Continuous Valence-Arousal Estimation](https://arxiv.org/abs/2603.11468)
*Yubeen Lee,Sangeun Lee,Junyeop Cha,Eunil Park*

Main category: cs.MM

TL;DR: 提出SAGE阶段自适应可靠性建模与融合，在Aff-Wild2上提升连续情感估计CCC。


<details>
  <summary>Details</summary>
Motivation: 多模态情感估计中不同交互阶段的模态可靠性变化显著，现有方法多只建模时间动态而忽视可靠性差异。

Method: 提出SAGE框架，估计并校准模态可靠性，采用可靠性感知的动态融合以重加权音频与视觉表示，并将可靠性估计与特征表示分离。

Result: 在Aff-Wild2上显著提高一致性相关系数（CCC），优于现有多模态融合方法。

Conclusion: SAGE通过阶段自适应的可靠性建模与融合提升了连续情感预测性能。
...
### [370] [Shadowless Projection Mapping for Tabletop Workspaces with Synthetic Aperture Projector](https://arxiv.org/abs/2603.11551)
...
### [371] [High-Contrast Projection Mapping under Light Field Illumination with LED Display and Aperiodic Lens Array](https://arxiv.org/abs/2603.11573)
...
### [372] [From Pets to Robots: MojiKit as a Data-Informed Toolkit for Affective HRI Design](https://arxiv.org/abs/2603.11632)
...
### [373] [HiSync: Spatio-Temporally Aligning Hand Motion from Wearable IMU and On-Robot Camera for Command Source Identification in Long-Range HRI](https://arxiv.org/abs/2603.11809)
...
TL;DR: 提出HiSync光学-惯性融合CSI框架，在远距离多用户场景显著提升命令来源识别准确率并验证落地可行性。


<details>
  <summary>Details</summary>
Motivation: 远距离HRI中的命令来源识别在多用户与距离导致的传感器歧义下仍缺乏可靠方法。

Method: 以手部运动为绑定线索，对齐机器人相机光流与手部IMU；提取频域特征，CSINet去噪与时序对齐，进行距离感知多窗口融合计算跨模态相似度。

Result: 三人场景、最远34m下CSI准确率92.32%，较SOTA提升48.44%，并在真实机器人上验证。

Conclusion: HiSync 通过光流与IMU融合实现远距离多用户CSI，并给出公共空间HRI设计指引。
...
### [374] [From Control to Foresight: Simulation as a New Paradigm for Human-Agent Collaboration](https://arxiv.org/abs/2603.11677)
...
### [375] [Human-Centred LLM Privacy Audits: Findings and Frictions](https://arxiv.org/abs/2603.12094)
...
### [376] ["I followed what felt right, not what I was told": Autonomy, Coaching, and Recognizing Bias Through AI-Mediated Dialogue](https://arxiv.org/abs/2603.11274)
...
### [377] [From Pen Strokes to Sleep States: Detecting Low-Recovery Days Using Sigma-Lognormal Handwriting Features](https://arxiv.org/abs/2603.11512)
...
### [378] [Prediction of Grade, Gender, and Academic Performance of Children and Teenagers from Handwriting Using the Sigma-Lognormal Model](https://arxiv.org/abs/2603.11519)
...
### [379] [Ghost Framing Theory: Exploring the role of generative AI in new venture rhetorical legitimation](https://arxiv.org/abs/2603.11384)
*Greg Nyilasy*
...
### [380] [Evaluation format, not model capability, drives triage failure in the assessment of consumer health AI](https://arxiv.org/abs/2603.11413)
...
### [381] [An Intent of Collaboration: On Agencies between Designers and Emerging (Intelligent) Technologies](https://arxiv.org/abs/2603.12018)
...
### [382] [Incremental Neural Network Verification via Learned Conflicts](https://arxiv.org/abs/2603.12232)
