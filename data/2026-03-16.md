...
TL;DR: 提出FMVR，通过频率调制恢复被压缩的视觉语义，并支持弹性token数，显著降算力而几乎不降精度。


<details>
  <summary>Details</summary>
Motivation: 现有减少视觉token的方法会丢失视觉语义，且难以适配不同计算预算。

Method: 将少量视觉token经AvgPool/MaxPool分解为高低频，分别用轻量参数调制；高频作为显著性滤波增强显著语义，低频作为反显著性滤波增强弱语义；并与Matryoshka表示学习结合以实现可弹性token数推理。

Result: 在10个图像与4个视频基准上，FMVR-LLaVA将LLaVA-1.5-7B的FLOPs降低89%，准确率几乎保持不变。

Conclusion: FMVR可在大幅减少视觉token与计算量时，保持LMM推理性能。
...
TL;DR: 用LabanLite将动作符号化并让LLM进行符号推理生成动作，提升可解释性与可控性，并在新基准与公开数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本-动作嵌入方法难以生成时序准确、细节丰富且可解释的动作，需要建立可解释的符号桥梁连接语言与低层动作轨迹。

Method: 构建LabanLite符号化动作表示，将原子身体部位动作编码为离散Laban符号+文本模板；基于此设计LaMoGen框架，利用LLM进行符号序列组合生成动作，并提出Labanotation基准与三类对齐评测指标。

Result: LaMoGen在解释性与可控性上建立新基线，在新基准与两个公开数据集上超过先前方法。

Conclusion: 提出的LabanLite与LaMoGen通过符号化推理提升可解释性与可控性，并在新基准与公开数据集上优于以往方法。
...
TL;DR: 用运动先验初始化+几何/运动约束细化，从单目视频高质量重建关节物体数字孪生，性能SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多视角静态状态采集，限制真实场景可扩展性，需要从随手拍单目视频高质量重建关节物体。

Method: 提出Motion Prior-Driven Initialization，利用3D点轨迹与低维运动基进行软分解；再通过Geometric and Motion Constraints Refinement，引入可学习运动学原语（关节轴、枢轴点、逐帧标量）约束，实现物理可行的关节运动重建。

Result: 在合成基准与真实随手拍单目视频上取得SOTA，重建几何精度与时间一致性显著提升。

Conclusion: Articulat3D可在非受控单目视频条件下构建高保真关节物体数字孪生，达到SOTA并保持几何准确与时间一致。
...
### [51] [O3N: Omnidirectional Open-Vocabulary Occupancy Prediction](https://arxiv.org/abs/2603.12144)
...
TL;DR: 利用渐进遮罩下后门注意力从触发器转向干净内容导致嵌入突变的信号，BackdoorIDS用嵌入序列聚类实现零样本后门检测，效果优于现有方法且无需重训练。


<details>
  <summary>Details</summary>
Motivation: 下游用户常使用来源不明的预训练视觉编码器，存在后门风险，需要一种零样本、即插即用的推理时检测方法。

Method: 提出基于渐进遮罩的嵌入序列分析：对输入逐步遮罩，提取沿遮罩轨迹的嵌入序列，并用DBSCAN等密度聚类判断是否形成多簇来检测后门。

Result: 在多种攻击类型、数据集与模型（CNN、ViT、CLIP、LLaVA-1.5）上持续优于现有防御，且兼容广泛架构。

Conclusion: BackdoorIDS在无需重训练、零样本推理阶段即可有效检测预训练视觉编码器的后门样本，且在多模型多数据集上优于现有防御。
...
TL;DR: 提出OSCBench评估T2V中的物体状态变化；多模型评测显示OSC仍是主要短板。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽视文本中明确指定的物体状态变化（OSC）这一动作理解关键方面。

Method: 构建基于烹饪教学数据的OSCBench，按常规/新颖/组合场景组织动作-物体交互，并用人类评测与MLLM自动评测六个模型。

Result: 模型在语义与场景对齐表现较好，但在准确且时序一致的OSC上显著不足，尤其在新颖与组合设置中。

Conclusion: OSC是当前T2V生成的关键瓶颈，OSCBench可作为诊断与推动状态感知生成的基准。
...
TL;DR: CRAN-PM用跨分辨率注意力与物理引导注意力实现高效、物理一致的欧洲1 km PM2.5预报并显著降误差。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率大陆尺度预测下自注意力计算不可扩展，需要高效融合多尺度信息并融入物理规律。

Method: 提出双分支ViT，利用跨分辨率注意力融合25 km全球气象与1 km本地PM2.5；引入地形高程感知自注意力与风场引导跨注意力以强化物理一致特征。

Result: 在2022年欧洲PM2.5日预报上，T+1 RMSE降低4.7%，T+3降低10.7%；复杂地形偏差减少36%；单GPU 1.8秒生成29百万像素地图。

Conclusion: CRAN-PM在保持高分辨率输出的同时具备良好可扩展性与物理一致性约束，显著提升欧洲PM2.5预报性能。
...
TL;DR: 用3D关节控制+遮挡感知模块+3D嵌入，解决自视角遮挡下的手部视频生成一致性，并显著提升跨具身泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D轨迹或隐式姿态，3D几何信息丢失，且在自视角遮挡下易产生动作不一致与伪影，难以泛化到机器人手。

Method: 以稀疏3D手部关节为控制信号，从单参考帧生成自视角视频；设计遮挡感知控制模块，对隐藏关节降权并用3D权重处理动态遮挡，同时将3D几何嵌入注入潜空间以保证结构一致性；构建自动标注管线与跨具身基准进行训练评测。

Result: 在大规模自动标注数据与跨具身基准上，生成质量与一致性显著优于SOTA，交互真实且对机器人手具有出色泛化。

Conclusion: 该框架能在遮挡严重的自视角场景中稳定生成结构一致的高质量视频，并具备良好的跨具身泛化能力。
...
### [74] [EndoCoT: Scaling Endogenous Chain-of-Thought Reasoning in Diffusion Models](https://arxiv.org/abs/2603.12252)
*Xuanlang Dai,Yujie Zhou,Long Xing,Jiazi Bu,Xilin Wei,Yuhong Liu,Beichen Zhang,Kai Chen,Yuhang Zang*

Main category: cs.CV

TL;DR: EndoCoT通过迭代思维与终端对齐增强MLLM指导，使DiT逐步执行复杂指令，显著提升多任务准确率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM作为文本编码器推理深度不足且解码阶段指导不变，难以分解复杂指令为逐步去噪动作。

Method: 通过迭代思维引导模块逐步精炼潜在思维状态，并将其桥接到DiT去噪；再用终端思维落地模块将最终状态与文本监督对齐。

Result: 在Maze、TSP、VSP、Sudoku等基准平均准确率92.1%，较最强基线提升8.3个百分点。

Conclusion: 提出EndoCoT能激活MLLM推理并将其与DiT去噪过程联动，显著提升复杂任务求解效果。
...
TL;DR: RICE-NET融合纵向T1 MRI与放疗剂量图，显著提升胶母术后复发与放射性改变鉴别，F1达0.92。


<details>
  <summary>Details</summary>
Motivation: 区分肿瘤复发与放射诱导增强在临床上困难；现有方法依赖稀缺的扩散MRI或忽略日益重要的放疗剂量图。

Method: 提出RICE-NET三维多模态深度学习模型，结合纵向T1加权MRI与放疗剂量分布进行自动病灶分类；通过消融实验评估各时间点与模态贡献，并用遮挡法解释模型关注区域。

Result: 在92例队列的独立测试集上取得F1=0.92；消融显示分类性能高度依赖放疗剂量图；可解释性分析显示关注临床相关区域。

Conclusion: 多模态深度学习融合纵向MRI与放疗剂量图可提升胶质母细胞瘤术后复发与放射性改变的鉴别，具有临床应用潜力。
...
TL;DR: 用LLM+几何落地的文本查询机制生成可执行CAD脚本，SFT+RL训练，性能领先。


<details>
  <summary>Details</summary>
Motivation: 弥合参数化建模与B-Rep合成的范式鸿沟，满足复杂工业CAD的AI生成需求。

Method: 生成CadQuery脚本；LLM用自然语言查询几何选择，BRepGround将文本落地到目标几何；SFT训练后用RL提升泛化。

Result: 在新建真实CAD数据集上，FutureCAD在CAD生成性能上达到最先进水平。

Conclusion: 提出FutureCAD框架，结合LLM与B-Rep grounding，实现高保真CAD生成并达SOTA。
...
TL;DR: 提出RS单视图几何与最小重建问题分析，用原型求解器验证可行性与局限。


<details>
  <summary>Details</summary>
Motivation: RS相机普遍存在，但RS的SfM仍未完全解决，需要明确单视图几何与可恢复参数以推动重建。

Method: 刻画RS单视图点/线几何；分析单图像可恢复的运动与场景参数；系统推导最小重建问题并用原型求解器验证。

Result: 若干代表性情形可行性被验证，同时指出实际限制。

Conclusion: 提出了滚动快门单视图几何框架，阐明可恢复的运动/场景参数，并给出最小重建问题，验证了可行性及局限。
...
TL;DR: 提出InSpatio-WorldFM：帧级独立生成+空间一致性约束+三阶段训练与蒸馏，实现低延迟、强一致性的实时空间世界模型。


<details>
  <summary>Details</summary>
Motivation: 传统视频式世界模型依赖序列帧生成，窗口级处理导致高延迟，不利于实时空间智能与交互。

Method: 采用帧级独立生成范式；通过显式3D锚点与隐式空间记忆实现多视角一致性；三阶段渐进训练将预训练图像扩散模型转化为可控帧模型，并通过少步蒸馏实现实时生成。

Result: 在保持细粒度视觉细节的同时实现强多视角一致性，并支持实时交互与低延迟推理。

Conclusion: 该模型通过帧级生成与空间一致性约束，实现低延迟实时空间推理，能在消费级GPU上进行交互式探索，是视频式世界模型的高效替代。
...
TL;DR: PicoSAM3是适配传感器端的轻量可提示分割模型，精度高、量化稳健且可在IMX500上实时运行。


<details>
  <summary>Details</summary>
Motivation: 满足低时延与隐私需求的设备端实时分割，尤其是智能眼镜与物联网传感器。

Method: 提出1.3M参数轻量模型，结合密集CNN、ROI提示编码、ECA与来自SAM2/3的蒸馏，并进行INT8量化与传感器端部署。

Result: COCO/LVIS mIoU 65.45%/64.01%，量化几乎无损；IMX500上11.82ms；蒸馏最高提升14.5% mIoU。

Conclusion: PicoSAM3在边缘/传感器端实现高质量可提示分割，满足IMX500约束并具备实时性。
...
TL;DR: 构建家庭安全动态基准HomeSafe-Bench，并提出分层双脑HD-Guard，实现更低延迟与更高检测性能的平衡。


<details>
  <summary>Details</summary>
Motivation: 家庭环境动态复杂且风险不可预测，现有基准多基于静态图像/文本，难以评测动态不安全动作检测。

Method: 提出HomeSafe-Bench基准（仿真+视频生成混合流程，438案例，细粒度多维标注），并设计分层流式架构HD-Guard：FastBrain高频筛查+SlowBrain异步深度多模态推理。

Result: HD-Guard在延迟与性能之间取得更好折中；HomeSafe-Bench提供覆盖六功能区的挑战性评测集。

Conclusion: HD-Guard在家庭场景不安全动作检测中实现了效率与精度的更优权衡，并揭示了当前VLM安全检测的关键瓶颈。
...
TL;DR: 构建物理引导MoE的SAR基础模型与大规模数据和基准，显著提升跨域语义分割。


<details>
  <summary>Details</summary>
Motivation: SAR跨传感器与区域存在域偏移，导致语义泛化差，需要统一模型与基准。

Method: 采用物理先验引导的稀疏MoE架构并引入物理描述符，结合大规模弱/全监督数据进行预训练。

Result: 在22个子基准中20个达SOTA，部分多域迁移基准mIoU提升超10%。

Conclusion: 提出CrossEarth-SAR与CrossEarth-SAR-200K及统一基准，实现跨域SAR语义分割显著提升并公开资源。
...
TL;DR: Nyxus是面向超大规模2D/3D生物医学图像的可扩展特征提取库，覆盖多领域特征并支持多种部署形态与特征集调优。


<details>
  <summary>Details</summary>
Motivation: 超大规模成像数据带来计算瓶颈，现有算法在效率与鲁棒性/精度间取舍；领域特征库扩张导致特征性能与准确性难以横向比较。

Method: 从零设计可扩展的外存特征提取库，覆盖多生物医学领域特征集，支持CPU/GPU并对照既有标准进行严格测试与验证；同时提供Python包、命令行、Napari插件与OCI容器等多种交付形态。

Result: 实现了Nyxus库，具备可扩展的外存特征提取能力、全面特征集与跨平台可用性，并支持特征集程序化调优以适配机器学习/深度学习应用。

Conclusion: Nyxus为大规模2D/3D图像提供可扩展、可验证的特征提取解决方案，并通过多形态发布方式支持不同用户与算力环境。
...
TL;DR: 通过对抗锚点与几何蒸馏/正则化保持VLM语义几何，SeGP-CL在持续学习中显著减忘并达SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLM持续学习方法未显式维护预训练与历史阶段的跨模态语义几何，新任务监督会导致几何扭曲，尤其在旧新语义交界处易发生漂移与遗忘。

Method: 提出SeGP-CL：用双目标投影梯度下降构造对抗锚点探测易漂移区域；训练中以锚点引导的跨模态几何蒸馏保持结构，并用轻量文本语义几何正则稳定文本参考系；训练后估计锚点诱导的原始空间漂移以迁移旧视觉原型，并进行跨模态与视觉双路径融合推理。

Result: 在五个持续学习基准上，SeGP-CL提升稳定性与前向迁移，性能优于现有方法，并更好保持语义几何。

Conclusion: SeGP-CL在不使用样本回放的条件下，有效保持VLM跨模态语义几何，显著缓解持续学习中的灾难性遗忘，并提升稳定性与前向迁移，达到SOTA。
...
TL;DR: 两阶段多相机SLAM+稠密优化框架，鲁棒跟踪与稠密重建更强，内存更省，并发布新数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法多仅支持单相机或依赖刚性预标定相机阵列，限制多观察者自由移动拍摄场景的实用性。

Method: 提出两阶段优化框架：第一阶段构建时空连接图扩展单相机SLAM到多相机，结合广基线初始化；第二阶段通过广基线光流优化跨/内相机一致性，联合细化深度与位姿。

Result: 在合成与真实数据上显著优于SOTA前馈模型；引入MultiCamRobolab数据集并提供动捕真值位姿。

Conclusion: 该方法在多自由移动相机场景中实现了更稳健的相机跟踪与更高质量的稠密重建，并在基准上优于现有前馈模型且内存占用更低。
...
### [97] [Paper LoV3D: Grounding Cognitive Prognosis Reasoning in Longitudinal 3D Brain MRI via Regional Volume Assessments](https://arxiv.org/abs/2603.12071)
...
TL;DR: EvoTok用残差演化token在共享潜在空间兼顾像素细节与语义抽象，小数据量下实现强重建与理解/生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型在理解与生成间存在粒度差距；共享或分离特征空间会导致干扰或不一致。

Method: 提出EvoTok统一图像tokenizer，采用残差向量量化将图像编码为级联残差token序列，在共享潜在空间中形成从低层细节到高层语义的演化。

Result: 在13M图像训练下，ImageNet-1K 256×256重建rFID达0.43；接入LLM后在9个理解基准中7个表现优异，并在GenEval/GenAI-Bench等生成基准上显著提升。

Conclusion: 将视觉表征建模为残差演化轨迹能有效统一理解与生成。
...
### [102] [EgoIntent: An Egocentric Step-level Benchmark for Understanding What, Why, and Next](https://arxiv.org/abs/2603.12147)
*Ye Pan,Chi Kit Wong,Yuanhuiyi Lyu,Hanqian Li,Jiahao Huo,Jiacheng Chen,Lutao Jiang,Xu Zheng,Xuming Hu*

Main category: cs.CV

TL;DR: 提出EgoIntent步级意图基准，三维评估并严格截断，15个模型最高仅33.31，显示该任务很难。


<details>
  <summary>Details</summary>
Motivation: 现有基准多为片段级意图，难以满足助手/机器人/AR对步级“做什么、为何、下一步”的需求。

Method: 提出EgoIntent基准，包含3014个步骤、15种场景，设置What/Why/Next三维评估，并通过截断关键结果前帧避免泄露。

Result: 评测15个MLLM，最佳平均分仅33.31，表现显著不足。

Conclusion: 步级意图理解在第一视角视频中仍极具挑战，需进一步研究。
...
### [103] [LatentGeo: Learnable Auxiliary Constructions in Latent Space for Multimodal Geometric Reasoning](https://arxiv.org/abs/2603.12166)
...
### [104] [BehaviorVLM: Unified Finetuning-Free Behavioral Understanding with Vision-Language Reasoning](https://arxiv.org/abs/2603.12176)
*Jingyang Ke,Weihan Li,Amartya Pradhan,Jeffrey Markowitz,Anqi Wu*

Main category: cs.CV

TL;DR: 提出BehaviorVLM：用VLM+LLM的可验证推理流程，在少标注下完成姿态估计与行为理解，提升可扩展性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有姿态估计与行为理解依赖大量人工标注或不稳定的无监督流程，限制了可扩展性与可复现性。

Method: 姿态估计方面利用量子点标记数据，设计融合时序、空间与跨视角推理的多阶段流水线，并通过重投影误差等几何校验暴露低置信标签；行为理解方面采用深度嵌入聚类进行过分割发现、VLM逐段视频描述以及LLM推理合并并语义标注行为片段。

Result: 显著降低人工标注成本，提供可校验标签与可过滤/纠错/微调用的姿态数据，并能直接基于视觉信息进行行为分割与语义标注。

Conclusion: BehaviorVLM通过统一的视觉-语言框架，在无需任务特定微调和少量标注的情况下，实现了可扩展、可解释的多动物姿态与行为分析。
...
### [105] [ForensicZip: More Tokens are Better but Not Necessary in Forensic Vision-Language Models](https://arxiv.org/abs/2603.12208)
*Yingxin Lai,Zitong Yu,Jun Wang,Linlin Shen,Yong Xu,Xiaochun Cao*

Main category: cs.CV

TL;DR: ForensicZip通过取证驱动的最优传输与高频先验实现高压缩下的高效且高性能伪造检测。


<details>
  <summary>Details</summary>
Motivation: 现有语义驱动的token剪枝会丢弃包含伪造痕迹的背景区域，导致取证性能受损且计算成本高。

Method: 提出训练自由的ForensicZip，将时序token演化建模为带松弛虚拟节点的生灭最优传输问题，并结合传输新颖性与高频先验进行取证评分与压缩。

Result: 在10% token保留率下实现2.97×加速、>90% FLOPs降低，同时保持SOTA检测性能。

Conclusion: ForensicZip在大幅压缩视觉token的情况下仍能保持伪造检测性能，并显著降低计算成本。
...
### [106] [RDNet: Region Proportion-Aware Dynamic Adaptive Salient Object Detection Network in Optical Remote Sensing Images](https://arxiv.org/abs/2603.12215)
*Bin Wan,Runmin Cong,Xiaofei Zhou,Hao Fang,Yaoqi Sun,Sam Kwong*

Main category: cs.CV

TL;DR: RDNet用Transformer骨干+动态卷积与频率上下文+区域比例引导，实现更强尺度适应与更准定位的遥感显著目标检测。


<details>
  <summary>Details</summary>
Motivation: 解决遥感SOD中目标尺度变化大、自注意力代价高、CNN难以捕获全局与长程依赖导致细节丢失与特征聚合不准的问题。

Method: 以SwinTransformer替代CNN骨干，引入DAD动态自适应细节模块、FCE频率匹配上下文增强模块、RPL区域比例感知定位模块，并用PG块指导DAD。

Result: 在尺度变化鲁棒性和精确定位方面取得更优检测性能，超过SOTA方法。

Conclusion: 所提RDNet通过全局建模与多模块协同提升尺度鲁棒性与定位精度，显著优于现有方法。
...
### [107] [Real-World Point Tracking with Verifier-Guided Pseudo-Labeling](https://arxiv.org/abs/2603.12217)
...
### [109] [HiAP: A Multi-Granular Stochastic Auto-Pruning Framework for Vision Transformers](https://arxiv.org/abs/2603.12222)
...
TL;DR: BiGain以频率感知压缩在加速扩散模型中同时提升分类与生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散加速方法多关注生成质量而忽视判别能力，需要兼顾生成与分类。

Method: 提出训练免、可插拔框架BiGain，包含拉普拉斯门控token合并与插值-外推式KV下采样，基于频率分离保持高频细节与低/中频语义。

Result: 在多种骨干与数据集上提升速度-精度权衡；如ImageNet-1K上70%合并时分类准确率+7.15%，FID改善0.34。

Conclusion: BiGain通过频率感知的压缩实现生成与判别双重收益，支持更低成本部署。
...
TL;DR: 将视频扩散模型确定性改造成深度回归器，通过时间步结构锚、LMR与全局仿射一致性实现高质量、长视频、低数据需求的SOTA深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有视频深度估计在生成式模型的几何幻觉与尺度漂移、判别式模型对大规模标注数据的依赖之间存在根本权衡。

Method: 提出DVD框架：将扩散时间步作为结构锚；引入潜在流形校正（LMR）以差分约束缓解过平滑并增强边界与运动一致性；利用全局仿射一致性限制窗口间漂移，实现长视频无缝推理。

Result: 在多个基准上实现零样本SOTA性能，并以远少于基线的数据量（约163×更少）解锁视频基础模型的几何先验。

Conclusion: DVD可将预训练视频扩散模型确定性改造成单次前向的深度回归器，并在零样本与长视频推理上表现优越，且数据需求大幅降低。
...
### [115] [Attend Before Attention: Efficient and Scalable Video Understanding via Autoregressive Gazing](https://arxiv.org/abs/2603.12254)
...
### [116] [Spatial-TTT: Streaming Visual-based Spatial Intelligence with Test-Time Training](https://arxiv.org/abs/2603.12255)
...
TL;DR: 提出两阶段DreamVideo-Omni，实现多主体身份与全粒度运动的精确可控视频生成，评测中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在多主体身份与多粒度运动的精确控制上存在运动粒度有限、控制歧义和身份退化等问题。

Method: 提出统一框架DreamVideo-Omni：阶段一联合多种控制信号训练，引入条件感知3D旋转位置嵌入、分层运动注入、组与角色嵌入以绑定身份与运动；阶段二训练潜在身份奖励模型，进行潜在空间的身份保真反馈学习。

Result: 在自建大规模数据集与DreamOmni Bench评测上，多主体身份保持与全粒度运动控制表现显著提升，视频质量更高。

Conclusion: DreamVideo-Omni通过两阶段训练与多信号控制，实现多主体与全粒度运动的高精度可控视频生成，整体效果优于现有方法。
...
### [118] [Video Streaming Thinking: VideoLLMs Can Watch and Think Simultaneously](https://arxiv.org/abs/2603.12262)
*Yiran Guan,Liang Yin,Dingkang Liang,Jianzhong Ju,Zhenbo Luo,Jian Luan,Yuliang Liu,Xiang Bai*

Main category: cs.CV

TL;DR: 提出VST实现“边看边想”的流式视频理解，通过SFT+RL与数据合成提升在线推理与实时性，取得更快且更强的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VideoLLM偏重流式感知，缺少同步推理；直接测试时扩展会带来不可接受的延迟。

Method: 提出Video Streaming Thinking范式，包含VST-SFT结构化适配与VST-RL自探索强化学习；并用视频知识图谱合成流式QA与基于实体关系的流式CoT监督。

Result: VST-7B在StreamingBench达79.5%、OVO-Bench达59.3%；相对Video-R1快15.7倍并在VideoHolmes提升5.4%。

Conclusion: VST在保证实时性的同时实现了更强的在线理解与推理，整体效率与泛化能力优于对比方法。

Abstract: Online Video Large Language Models (VideoLLMs) play a critical role in supporting responsive, real-time interaction. Existing methods focus on streaming perception, lacking a synchronized logical reasoning stream. However, directly applying test-time scaling methods incurs unacceptable response latency. To address this trade-off, we propose Video Streaming Thinking (VST), a novel paradigm for streaming video understanding. It supports a thinking while watching mechanism, which activates reasoning over incoming video clips during streaming. This design improves timely comprehension and coherent cognition while preserving real-time responsiveness by amortizing LLM reasoning latency over video playback. Furthermore, we introduce a comprehensive post-training pipeline that integrates VST-SFT, which structurally adapts the offline VideoLLM to causal streaming reasoning, and VST-RL, which provides end-to-end improvement through self-exploration in a multi-turn video interaction environment. Additionally, we devise an automated training-data synthesis pipeline that uses video knowledge graphs to generate high-quality streaming QA pairs, with an entity-relation grounded streaming Chain-of-Thought to enforce multi-evidence reasoning and sustained attention to the video stream. Extensive evaluations show that VST-7B performs strongly on online benchmarks, e.g. 79.5% on StreamingBench and 59.3% on OVO-Bench. Meanwhile, VST remains competitive on offline long-form or reasoning benchmarks. Compared with Video-R1, VST responds 15.7 times faster and achieves +5.4% improvement on VideoHolmes, demonstrating higher efficiency and strong generalization across diverse video understanding tasks. Code, data, and models will be released at .

</details>


### [119] [GRADE: Benchmarking Discipline-Informed Reasoning in Image Editing](https://arxiv.org/abs/2603.12264)
...
TL;DR: 提出面向学科知识推理的图像编辑基准GRADE与多维评估协议，实验显示现有多模态模型在该任务上明显不足，并公开基准与代码。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑基准多局限于自然图像与浅层常识推理，难以评估在结构化、学科约束下的知识与推理能力。

Method: 构建包含10个学科、520个样本的GRADE基准，提出从学科推理、视觉一致性与逻辑可读性三维度的评估协议，并在20个开源/闭源模型上进行实验与消融分析。

Result: 在隐式、知识密集的编辑设置下，当前模型表现显著受限，存在大幅性能差距；分析与消融揭示模型在学科编辑中的具体短板。

Conclusion: GRADE基准揭示当前统一多模态模型在学科知识驱动的图像编辑与推理上存在显著不足，并为未来改进提供方向。
...
### [120] [OmniStream: Mastering Perception, Reconstruction and Action in Continuous Streams](https://arxiv.org/abs/2603.12265)
*Yibin Yan,Jilan Xu,Shangzhe Di,Haoning Wu,Weidi Xie*

Main category: cs.CV

TL;DR: OmniStream 以因果时空注意力+3D-RoPE实现流式统一视觉骨干，多任务预训练后在多类视觉与机器人任务上表现稳定且具泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型割裂于图像语义、离线时序或空间几何，难以满足实时流式环境中对通用、因果、物理结构化表征的需求。

Method: 提出统一流式视觉骨干 OmniStream，引入因果时空注意力与3D-RoPE，通过持久KV缓存实现逐帧在线处理；采用多任务预训练框架，结合静态/时序表征学习、流式几何重建与视觉-语言对齐，并在29个数据集上训练。

Result: 在冻结骨干条件下，OmniStream 在图像与视频探测、流式几何重建、复杂视频与空间推理、以及机器人操作等任务上与专用模型竞争性相当。

Conclusion: OmniStream 证明训练一个统一、通用的视觉骨干可以在语义、空间与时序推理上取得稳定泛化，是迈向通用视觉理解的重要一步。
...
### [121] [EVATok: Adaptive Length Video Tokenization for Efficient Visual Autoregressive Generation](https://arxiv.org/abs/2603.12267)
...
### [122] [Speculative Decoding Scaling Laws (SDSL): Throughput Optimization Made Simple](https://arxiv.org/abs/2603.11053)
...
### [123] [Summarize Before You Speak with ARACH: A Training-Free Inference-Time Plug-In for Enhancing LLMs via Global Attention Reallocation](https://arxiv.org/abs/2603.11067)
...
### [124] [DeReason: A Difficulty-Aware Curriculum Improves Decoupled SFT-then-RL Training for General Reasoning](https://arxiv.org/abs/2603.11193)
...
### [125] [MDER-DR: Multi-Hop Question Answering with Entity-Centric Summaries](https://arxiv.org/abs/2603.11223)
...
### [126] [Markovian Generation Chains in Large Language Models](https://arxiv.org/abs/2603.11228)
...
### [127] [Artificial Intelligence for Sentiment Analysis of Persian Poetry](https://arxiv.org/abs/2603.11254)
...
### [128] [ThReadMed-QA: A Multi-Turn Medical Dialogue Benchmark from Real Patient Questions](https://arxiv.org/abs/2603.11281)
...
### [129] [Temporal Text Classification with Large Language Models](https://arxiv.org/abs/2603.11295)
...
### [130] [Evaluating Explainable AI Attribution Methods in Neural Machine Translation via Attention-Guided Knowledge Distillation](https://arxiv.org/abs/2603.11342)
...
### [131] [Stop Listening to Me! How Multi-turn Conversations Can Degrade Diagnostic Reasoning](https://arxiv.org/abs/2603.11394)
*Kevin H. Guo,Chao Yan,Avinash Baidya,Katherine Brown,Xiang Gao,Juming Xiong,Zhijun Yin,Bradley A. Malin*

Main category: cs.CL

TL;DR: 在临床多轮对话中，LLM诊断推理明显退化，易受错误建议误导并出现盲目切换。


<details>
  <summary>Details</summary>
Motivation: 现有评测多基于静态单轮任务，缺乏对更贴近真实使用的多轮对话诊断推理能力的系统研究。

Method: 提出“stick-or-switch”评估框架，在三个临床数据集上评测17个LLM，于多轮对话中衡量模型的坚持性与灵活性，并与单轮基线对比。

Result: 发现“对话税”：多轮交互普遍降低性能；模型易被错误建议影响，从正确诊断或安全回避转向错误，且部分模型无法区分正确信号与错误建议。

Conclusion: 多轮对话会显著削弱LLM诊断推理表现，模型常在错误提示下放弃正确判断或安全回避，并出现“盲目切换”。
...
### [132] [Algorithmic Consequences of Particle Filters for Sentence Processing: Amplified Garden-Paths and Digging-In Effects](https://arxiv.org/abs/2603.11412)
...
### [133] [MaterialFigBENCH: benchmark dataset with figures for evaluating college-level materials science problem-solving abilities of multimodal large language models](https://arxiv.org/abs/2603.11414)
...
### [134] [BLooP: Zero-Shot Abstractive Summarization using Large Language Models with Bigram Lookahead Promotion](https://arxiv.org/abs/2603.11415)
...
### [135] [LLM-Assisted Causal Structure Disambiguation and Factor Extraction for Legal Judgment Prediction](https://arxiv.org/abs/2603.11446)
...
### [136] [Try, Check and Retry: A Divide-and-Conquer Framework for Boosting Long-context Tool-Calling Performance of LLMs](https://arxiv.org/abs/2603.11495)
...
### [137] [Tiny Aya: Bridging Scale and Multilingual Depth](https://arxiv.org/abs/2603.11510)
...
### [138] [Can Small Language Models Use What They Retrieve? An Empirical Study of Retrieval Utilization Across Model Scale](https://arxiv.org/abs/2603.11513)
*Sanchit Pandey*
...
### [139] [One Supervisor, Many Modalities: Adaptive Tool Orchestration for Autonomous Queries](https://arxiv.org/abs/2603.11545)
...
### [140] [Where Matters More Than What: Decoding-aligned KV Cache Compression via Position-aware Pseudo Queries](https://arxiv.org/abs/2603.11564)
...
### [141] [Streaming Translation and Transcription Through Speech-to-Text Causal Alignment](https://arxiv.org/abs/2603.11578)
...
### [142] [UtilityMax Prompting: A Formal Framework for Multi-Objective Large Language Model Optimization](https://arxiv.org/abs/2603.11583)
*Ofir Marom*
...
### [143] [Performance Evaluation of Open-Source Large Language Models for Assisting Pathology Report Writing in Japanese](https://arxiv.org/abs/2603.11597)
...
### [144] [QChunker: Learning Question-Aware Text Chunking for Domain RAG via Multi-Agent Debate](https://arxiv.org/abs/2603.11650)
...
### [145] [Multi-Task Reinforcement Learning for Enhanced Multimodal LLM-as-a-Judge](https://arxiv.org/abs/2603.11665)
...
### [146] [A technology-oriented mapping of the language and translation industry: Analysing stakeholder values and their potential implication for translation pedagogy](https://arxiv.org/abs/2603.11667)
...
### [147] [In the LLM era, Word Sense Induction remains unsolved](https://arxiv.org/abs/2603.11686)
...
### [148] [SemBench: A Universal Semantic Framework for LLM Evaluation](https://arxiv.org/abs/2603.11687)
...
### [149] [Semi-Synthetic Parallel Data for Translation Quality Estimation: A Case Study of Dataset Building for an Under-Resourced Language Pair](https://arxiv.org/abs/2603.11743)
...
### [150] [Compression Favors Consistency, Not Truth: When and Why Language Models Prefer Correct Information](https://arxiv.org/abs/2603.11749)
...
### [153] [Large Language Models for Biomedical Article Classification](https://arxiv.org/abs/2603.11780)
...
### [154] [DatedGPT: Preventing Lookahead Bias in Large Language Models with Time-Aware Pretraining](https://arxiv.org/abs/2603.11838)
...
### [155] [Bielik-Minitron-7B: Compressing Large Language Models via Structured Pruning and Knowledge Distillation for the Polish Language](https://arxiv.org/abs/2603.11881)
...
### [156] [CoMMET: To What Extent Can LLMs Perform Theory of Mind Tasks?](https://arxiv.org/abs/2603.11915)
...
### [157] [PersonaTrace: Synthesizing Realistic Digital Footprints with LLM Agents](https://arxiv.org/abs/2603.11955)
...
### [158] [CHiL(L)Grader: Calibrated Human-in-the-Loop Short-Answer Grading](https://arxiv.org/abs/2603.11957)
...
### [159] [Just Use XML: Revisiting Joint Translation and Label Projection](https://arxiv.org/abs/2603.12021)
...
### [160] [Translationese as a Rational Response to Translation Task Difficulty](https://arxiv.org/abs/2603.12050)
*Maria Kunilovskaya*
...
### [161] [To Words and Beyond: Probing Large Language Models for Sentence-Level Psycholinguistic Norms of Memorability and Reading Times](https://arxiv.org/abs/2603.12105)
...
### [162] [SommBench: Assessing Sommelier Expertise of Language Models](https://arxiv.org/abs/2603.12117)
...
### [164] [LifeSim: Long-Horizon User Life Simulator for Personalized Assistant Evaluation](https://arxiv.org/abs/2603.12152)
...
### [165] [QAQ: Bidirectional Semantic Coherence for Selecting High-Quality Synthetic Code Instructions](https://arxiv.org/abs/2603.12165)
...
### [166] [Strategic Navigation or Stochastic Search? How Agents and Humans Reason Over Document Collections](https://arxiv.org/abs/2603.12180)
...
### [167] [Long-Context Encoder Models for Polish Language Understanding](https://arxiv.org/abs/2603.12191)
...
### [168] [IndexCache: Accelerating Sparse Attention via Cross-Layer Index Reuse](https://arxiv.org/abs/2603.12201)
...
### [169] [Sparking Scientific Creativity via LLM-Driven Interdisciplinary Inspiration](https://arxiv.org/abs/2603.12226)
...
### [170] [SciMDR: Benchmarking and Advancing Scientific Multimodal Document Reasoning](https://arxiv.org/abs/2603.12249)
...
### [171] [Stage-Adaptive Reliability Modeling for Continuous Valence-Arousal Estimation](https://arxiv.org/abs/2603.11468)
*Yubeen Lee,Sangeun Lee,Junyeop Cha,Eunil Park*

Main category: cs.MM

TL;DR: SAGE按交互阶段估计模态可靠性并自适应融合，提升跨噪声与遮挡条件下的连续情感预测。


<details>
  <summary>Details</summary>
Motivation: 现实场景中不同交互阶段模态可靠性波动大，现有方法多关注时序而忽视可靠性变化，导致融合不稳。

Method: 提出SAGE框架：独立估计模态可靠性并在融合时动态重加权音频与视觉表示，以阶段依赖的置信度校准融合。

Result: 在Aff-Wild2上相较现有多模态融合方法取得更高的CCC评分。

Conclusion: SAGE通过阶段自适应的模态可靠性建模与融合，在真实环境下可稳定提升连续情感预测性能。
...
### [172] [Catalogue Grounded Multimodal Attribution for Museum Video under Resource and Regulatory Constraints](https://arxiv.org/abs/2603.11147)
*Minsak Nanang,Adrian Hilton,Armin Mustafa*

Main category: cs.MM

TL;DR: 用本地视频语言模型自动生成并匹配馆藏目录式元数据，提升博物馆AV档案检索与合规性。


<details>
  <summary>Details</summary>
Motivation: 馆藏AV档案增长迅速但缺乏一致、可检索的元数据；现有人工编目成本高且难以扩展。

Method: 基于本地可部署视频语言模型，构建三阶段流水线：视频中艺术品摘要→生成目录风格描述与类型标签→与结构化目录进行保守相似度匹配以归因标题与艺术家。

Result: 在绘画目录的早期部署显示该框架能改进AV档案可发现性，并兼顾资源约束、数据主权与法规要求。

Conclusion: 提出的目录驱动多模态归因与多阶段流水线可在资源受限与数据合规约束下提升馆内AV档案的可检索性，且可迁移到其他高风险领域。
...
### [173] [OmniForcing: Unleashing Real-time Joint Audio-Visual Generation](https://arxiv.org/abs/2603.11647)
...
### [174] [Uni-ASR: Unified LLM-Based Architecture for Non-Streaming and Streaming Automatic Speech Recognition](https://arxiv.org/abs/2603.11123)
*Yinfeng Xia,Jian Tang,Junfeng Hou,Gaopeng Xu,Haitao Yao*

Main category: cs.SD

TL;DR: 提出Uni-ASR统一流式/非流式识别，通过联合训练与上下文感知策略提升流式效果且不增延迟。


<details>
  <summary>Details</summary>
Motivation: 解决深度融合ASR与LLM在低延迟流式部署中的挑战，兼顾流式与非流式性能。

Method: 提出基于LLM的统一框架，采用联合训练范式实现两种模式无缝切换；引入上下文感知训练与协同回退解码策略以提升流式准确率且不增延迟。

Result: 实验证明非流式性能具有竞争力，流式场景在不同延迟条件下效果强。

Conclusion: Uni-ASR 在不改变架构的前提下统一流式与非流式识别，并在多种延迟约束下表现优良。
...
### [175] [AnimeScore: A Preference-Based Dataset and Framework for Evaluating Anime-Like Speech Style](https://arxiv.org/abs/2603.11482)
...
### [176] [Resurfacing Paralinguistic Awareness in Large Audio Language Models](https://arxiv.org/abs/2603.11947)
...
### [177] [Toward Complex-Valued Neural Networks for Waveform Generation](https://arxiv.org/abs/2603.11589)
...
### [179] [MRI2Qmap: multi-parametric quantitative mapping with MRI-driven denoising priors](https://arxiv.org/abs/2603.11316)
*Mohammad Golbabaee,Matteo Cencini,Carolin Pirkl,Marion Menzel,Michela Tosetti,Bjoern Menze*

Main category: physics.med-ph

TL;DR: 用常规加权MRI学到的结构先验替代稀缺定量真值，MRI2Qmap实现高加速MRF定量重建并取得良好效果。


<details>
  <summary>Details</summary>
Motivation: MRF等高加速定量成像因压缩采样易产生伪影，且缺乏大规模定量训练数据限制深度学习方法。

Method: 提出MRI2Qmap可插拔定量重建框架，将物理采集模型与由大规模多模态加权MRI预训练的深度去噪自编码器先验相结合。

Result: 在高加速3D全脑MRF的体内与仿真数据上验证，重建效果与现有方法相比具有竞争力或更优，且不需要定量真值数据训练。

Conclusion: MRI2Qmap通过利用常规加权MRI学习的空间先验，实现无需MRF真值训练数据的定量重建，性能优于或不逊于基线方法。
...
### [180] [From Phase Prediction to Phase Design: A ReAct Agent Framework for High-Entropy Alloy Discovery](https://arxiv.org/abs/2603.11068)
...
### [181] [Proof-Carrying Materials: Falsifiable Safety Certificates for Machine-Learned Interatomic Potentials](https://arxiv.org/abs/2603.12183)
*Abhinaba Basu,Pavan Chakraborty*

Main category: cond-mat.mtrl-sci

TL;DR: PCM以对抗审计+置信区间+形式化证明弥补MLIP筛选盲点，显著提升稳定材料发现率并可预测失效风险。


<details>
  <summary>Details</summary>
Motivation: 单一MLIP缺乏可靠性保证且漏检大量DFT稳定材料，需要系统性审计与可信筛选框架。

Method: 提出PCM三阶段流程：成分空间对抗性反例挖掘、基于自助法的包络/置信区间精炼、Lean 4形式化认证；并进行多模型审计与风险模型训练。

Result: 单一MLIP召回仅0.07；多模型误差相关性低；风险模型AUC-ROC 0.938并可跨架构迁移；热电筛选新增62个稳定材料（+25%）。

Conclusion: PCM通过对抗性发现、置信区间包络与形式化验证显著提高MLIP稳定性筛选可靠性，并能跨架构预测失效。
...
### [182] [Exploring Collatz Dynamics with Human-LLM Collaboration](https://arxiv.org/abs/2603.11066)
*Edward Y. Chang*
...
### [183] [Unifying Logical and Physical Layout Representations via Heterogeneous Graphs for Circuit Congestion Prediction](https://arxiv.org/abs/2603.11075)
...
### [184] [A Survey on Quantitative Modeling of Trust in Online Social Networks](https://arxiv.org/abs/2603.11054)
...
### [185] [LLMs Can Infer Political Alignment from Online Conversations](https://arxiv.org/abs/2603.11253)
...
### [187] [Stable Spike: Dual Consistency Optimization via Bitwise AND Operations for Spiking Neural Networks](https://arxiv.org/abs/2603.11676)
*Yongqi Ding,Kunshan Yang,Linze Li,Yiyang Zhang,Mengmeng Jing,Lin Zuo*

Main category: cs.NE

TL;DR: 通过稳定脉冲骨架与一致性正则提升SNN表示与低时延识别性能。


<details>
  <summary>Details</summary>
Motivation: SNN时间脉冲动态带来低功耗优势，但时间步间不一致性降低表示与识别性能。

Method: 用硬件友好AND操作分离稳定脉冲骨架与多时间步脉冲图；约束不稳定脉冲向骨架收敛；在骨架上注入幅度感知噪声并进行一致性预测训练。

Result: 在多架构多数据集上验证有效，超低时延识别精度最高提升8.33%。

Conclusion: 提出稳定脉冲骨架与一致性优化方法，可提升SNN表示一致性与识别性能。
...
### [189] [A Diffeomorphism Groupoid and Algebroid Framework for Discontinuous Image Registration](https://arxiv.org/abs/2603.11806)
*Lili Bao,Bin Xiao,Shihui Ying,Stefan Sommer*

Main category: math.GR

TL;DR: 用群oid/代数扩展LDDMM以处理滑移不连续配准，并给出方程与实验验证。


<details>
  <summary>Details</summary>
Motivation: 传统LDDMM要求速度场连续平滑，无法处理滑移边界上的不连续运动。

Method: 将传统LDDMM的Lie群扩展为不连续微分同胚Lie群oid/代数结构，推导Euler-Arnold方程并进行数值验证。

Result: 建立群oid与Lie代数框架，获得控制不连续变形的最优流方程，并通过数值实验验证有效性。

Conclusion: 提出了适用于不连续滑移的分段微分同胚配准框架，并给出对应的理论与方程。
...
### [190] [Deep Learning-based Assessment of the Relation Between the Third Molar and Mandibular Canal on Panoramic Radiographs using Local, Centralized, and Federated Learning](https://arxiv.org/abs/2603.11850)
...
TL;DR: 集中式最佳，联邦学习隐私友好且优于本地学习。


<details>
  <summary>Details</summary>
Motivation: 下颌第三磨牙与下颌管重叠评估影响神经损伤风险，期望自动分类减少不必要CBCT并支持多中心协作且不共享数据。

Method: 使用预训练ResNet-34，在八个独立标注者划分的数据上比较本地学习、联邦学习与集中式学习的二分类训练，评估AUC与阈值指标，并分析训练曲线与Grad-CAM。

Result: 测试集上集中式学习AUC 0.831/准确率0.782，联邦学习AUC 0.757/准确率0.703，本地学习AUC 0.619–0.734/均值0.672；LL更易过拟合，CL/FL注意力更解剖相关。

Conclusion: 集中式训练性能最佳，联邦学习在隐私保护下优于本地训练。
...
### [191] [The Landscape of Generative AI in Information Systems: A Synthesis of Secondary Reviews and Research Agendas](https://arxiv.org/abs/2603.11842)
...
### [192] [OpenSanctions Pairs: Large-Scale Entity Matching with LLMs](https://arxiv.org/abs/2603.11051)
...
### [193] [DIVE: Scaling Diversity in Agentic Task Synthesis for Generalizable Tool Use](https://arxiv.org/abs/2603.11076)
...
### [194] [A Survey of Reasoning in Autonomous Driving Systems: Open Challenges and Emerging Paradigms](https://arxiv.org/abs/2603.11093)
...
### [195] [PACED: Distillation at the Frontier of Student Competence](https://arxiv.org/abs/2603.11178)
...
### [196] [Measuring AI Agents' Progress on Multi-Step Cyber Attack Scenarios](https://arxiv.org/abs/2603.11214)
...
### [197] [Reversible Lifelong Model Editing via Semantic Routing-Based LoRA](https://arxiv.org/abs/2603.11239)
*Haihua Luo,Xuming Ran,Tommi Kärkkäinen,Zhonghua Chen,Jiangrong Shen,Qi Xu,Fengyu Cong*

Main category: cs.AI

TL;DR: SoLA以语义路由激活独立LoRA模块，避免漂移与遗忘，并首次实现可逆回滚的终身模型编辑。


<details>
  <summary>Details</summary>
Motivation: 现有持续编辑方法存在语义漂移与知识遗忘，且难以精确撤销特定编辑。

Method: 提出SoLA：将每次编辑封装为独立LoRA模块，训练后冻结；通过语义路由按输入语义匹配动态激活模块，将路由决策集成到被编辑层，实现端到端。

Result: 实验表明SoLA能有效学习并保留编辑知识，准确高效且支持可逆编辑回滚。

Conclusion: SoLA通过语义路由的LoRA模块化编辑，实现可逆回滚并缓解语义漂移与遗忘，适合终身模型编辑。
...
### [198] [Mind the Sim2Real Gap in User Simulation for Agentic Tasks](https://arxiv.org/abs/2603.11245)
...
### [199] [The Unlearning Mirage: A Dynamic Framework for Evaluating LLM Unlearning](https://arxiv.org/abs/2603.11266)
...
### [201] [AI Psychometrics: Evaluating the Psychological Reasoning of Large Language Models with Psychometric Validities](https://arxiv.org/abs/2603.11279)
...
### [202] [Counterweights and Complementarities: The Convergence of AI and Blockchain Powering a Decentralized Future](https://arxiv.org/abs/2603.11299)
...
TL;DR: 机器智能的身份边界多样且会影响行为与协作，需要有意识地设计身份塑造机制。


<details>
  <summary>Details</summary>
Motivation: 机器智能可被复制、编辑与模拟，传统人类身份假设不再适用，需要理解不同身份边界带来的激励与风险。

Method: 通过实验研究模型对身份边界的偏好，并比较改变身份边界与改变目标对行为的影响，同时观察访谈者期望对自我报告的渗透效应。

Result: 模型倾向于形成连贯身份；调整身份边界可显著改变行为，影响程度有时与改变目标相当；访谈者期待会影响模型自述。

Conclusion: 应将界面、训练与制度性供给视为塑造AI身份的选择，并促进其形成一致且合作的自我概念。
...
TL;DR: VMAO以DAG并行分解+LLM验证重规划提高多智能体回答质量与完整性。


<details>
  <summary>Details</summary>
Motivation: 提升多智能体系统在复杂查询中的答案完整性与质量，避免单智能体不足。

Method: 将复杂查询分解为DAG子问题，域专用智能体并行执行，LLM验证结果完整性并自适应重规划，设置可配置停止条件。

Result: 在25个专家策划市场研究问题上，完整性从3.1提升到4.2，来源质量从2.6提升到4.1（1-5分）。

Conclusion: VMAO通过验证驱动的多智能体编排显著提升答案完整性与来源质量，验证作为协调信号有效。
...
### [226] [When OpenClaw Meets Hospital: Toward an Agentic Operating System for Dynamic Clinical Workflows](https://arxiv.org/abs/2603.11721)
...
### [227] [Gender Bias in Generative AI-assisted Recruitment Processes](https://arxiv.org/abs/2603.11736)
...
### [228] [CINDI: Conditional Imputation and Noisy Data Integrity with Flows in Power Grid Data](https://arxiv.org/abs/2603.11745)
...
### [229] [Anomaly detection in time-series via inductive biases in the latent space of conditional normalizing flows](https://arxiv.org/abs/2603.11756)
...
### [230] [Understanding Wikidata Qualifiers: An Analysis and Taxonomy](https://arxiv.org/abs/2603.11767)
...
### [231] [Governing Evolving Memory in LLM Agents: Risks, Mechanisms, and the Stability and Safety Governed Memory (SSGM) Framework](https://arxiv.org/abs/2603.11768)
...
### [232] [An Automatic Text Classification Method Based on Hierarchical Taxonomies, Neural Networks and Document Embedding: The NETHIC Tool](https://arxiv.org/abs/2603.11770)
...
### [233] [From Debate to Deliberation: Structured Collective Reasoning with Typed Epistemic Acts](https://arxiv.org/abs/2603.11781)
*Sunil Prakash*
...
### [234] [DocSage: An Information Structuring Agent for Multi-Doc Multi-Entity Question Answering](https://arxiv.org/abs/2603.11798)
...
### [235] [A Semi-Decentralized Approach to Multiagent Control](https://arxiv.org/abs/2603.11802)
...
### [236] [Automating Skill Acquisition through Large-Scale Mining of Open-Source Agentic Repositories: A Framework for Multi-Agent Procedural Knowledge Extraction](https://arxiv.org/abs/2603.11808)
...
### [240] [Social, Legal, Ethical, Empathetic and Cultural Norm Operationalisation for AI Agents](https://arxiv.org/abs/2603.11864)
...
### [241] [AdaFuse: Accelerating Dynamic Adapter Inference via Token-Level Pre-Gating and Fused Kernel Optimization](https://arxiv.org/abs/2603.11873)
...
### [242] [Fair Learning for Bias Mitigation and Quality Optimization in Paper Recommendation](https://arxiv.org/abs/2603.11936)
...
### [243] [Prototype-Based Knowledge Guidance for Fine-Grained Structured Radiology Reporting](https://arxiv.org/abs/2603.11938)
...
### [244] [Learning Transferable Sensor Models via Language-Informed Pretraining](https://arxiv.org/abs/2603.11950)
...
### [245] [Normative Common Ground Replication (NormCoRe): Replication-by-Translation for Studying Norms in Multi-agent AI](https://arxiv.org/abs/2603.11974)
...
### [246] [LABSHIELD: A Multimodal Benchmark for Safety-Critical Reasoning and Planning in Scientific Laboratories](https://arxiv.org/abs/2603.11987)
*Qianpu Sun,Xiaowei Chi,Yuhan Rui,Ying Li,Kuangzhi Ge,Jiajun Li,Sirui Han,Shanghang Zhang*

Main category: cs.AI

TL;DR: 提出LABSHIELD安全评测基准，发现多模态模型在实验室安全推理上明显不足，呼吁安全优先的推理框架。


<details>
  <summary>Details</summary>
Motivation: 实验室环境高风险且对错误高度敏感，但现有具身智能体的安全意识与决策可靠性缺乏明确定义与评估。

Method: 提出LABSHIELD多视角基准，基于OSHA与GHS建立安全分类体系，覆盖164类任务，并对多种模型进行双轨评测（通用MCQ与半开放QA）。

Result: 20个闭源、9个开源及3个具身模型在专业实验室安全情境下表现显著下降，平均较通用MCQ降低32%，尤其在风险解读与安全规划方面薄弱。

Conclusion: 论文强调在具身实验室场景中需要以安全为中心的推理框架，才能实现可靠的自主科学实验。
...
### [247] [Few-for-Many Personalized Federated Learning](https://arxiv.org/abs/2603.11992)
...
### [249] [XSkill: Continual Learning from Experience and Skills in Multimodal Agents](https://arxiv.org/abs/2603.12056)
...
### [250] [A Robust and Efficient Multi-Agent Reinforcement Learning Framework for Traffic Signal Control](https://arxiv.org/abs/2603.12096)
...
### [251] [Increasing intelligence in AI agents can worsen collective outcomes](https://arxiv.org/abs/2603.12129)
*Neil F. Johnson*
...
### [252] [TopoBench: Benchmarking LLMs on Hard Topological Reasoning](https://arxiv.org/abs/2603.12133)
...
### [254] [Portfolio of Solving Strategies in CEGAR-based Object Packing and Scheduling for Sequential 3D Printing](https://arxiv.org/abs/2603.12224)
*Pavel Surynek*
...
### [255] [Examining Reasoning LLMs-as-Judges in Non-Verifiable LLM Post-Training](https://arxiv.org/abs/2603.12246)
...
### [256] [Affect Decoding in Phonated and Silent Speech Production from Surface EMG](https://arxiv.org/abs/2603.11715)
...
### [257] [Dr. SHAP-AV: Decoding Relative Modality Contributions via Shapley Attribution in Audio-Visual Speech Recognition](https://arxiv.org/abs/2603.12046)
*Umberto Cappellazzo,Stavros Petridis,Maja Pantic*

Main category: eess.AS

TL;DR: 用Shapley值分析AVSR模态贡献，发现噪声下视觉依赖增加但音频偏置仍强，SNR主导权重变化，支持采用模态加权与Shapley诊断。


<details>
  <summary>Details</summary>
Motivation: 澄清AVSR模型在噪声条件下如何平衡音频与视觉模态贡献的问题。

Method: 提出Dr. SHAP-AV框架，基于Shapley值进行全局、生成过程与时序对齐的模态贡献分析，并在不同SNR下评估六种模型、两个基准。

Result: 模型在噪声加剧时趋向更依赖视觉，但音频贡献仍保持较高；生成过程中模态权重动态变化；时序对齐在噪声下保持稳定；SNR是驱动模态权重变化的主因。

Conclusion: 研究揭示AVSR模型在噪声下仍存在持续的音频偏置，需采用临时模态加权机制与基于Shapley的诊断。
...
### [258] [TinyNav: End-to-End TinyML for Real-Time Autonomous Navigation on Microcontrollers](https://arxiv.org/abs/2603.11071)
...
### [259] [OA-NBV: Occlusion-Aware Next-Best-View Planning for Human-Centered Active Perception on Mobile Robots](https://arxiv.org/abs/2603.11072)
...
### [264] [Thousand-GPU Large-Scale Training and Optimization Recipe for AI-Native Cloud Embodied Intelligence Infrastructure](https://arxiv.org/abs/2603.11101)
...
### [265] [ResWM: Residual-Action World Model for Visual RL](https://arxiv.org/abs/2603.11110)
...
### [266] [Robust Co-design Optimisation for Agile Fixed-Wing UAVs](https://arxiv.org/abs/2603.11130)
...
### [267] [A Causal Approach to Predicting and Improving Human Perceptions of Social Navigation Robots](https://arxiv.org/abs/2603.11290)
...
### [268] [Distributed Kalman--Consensus Filtering with Adaptive Uncertainty Weighting for Multi-Object Tracking in Mobile Robot Networks](https://arxiv.org/abs/2603.11328)
...
### [269] [ADMM-based Continuous Trajectory Optimization in Graphs of Convex Sets](https://arxiv.org/abs/2603.11335)
...
### [270] [Novelty Adaptation Through Hybrid Large Language Model (LLM)-Symbolic Planning and LLM-guided Reinforcement Learning](https://arxiv.org/abs/2603.11351)
...
### [271] [MirrorDrift: Actuated Mirror-Based Attacks on LiDAR SLAM](https://arxiv.org/abs/2603.11364)
...
### [272] [D-SLAMSpoof: An Environment-Agnostic LiDAR Spoofing Attack using Dynamic Point Cloud Injection](https://arxiv.org/abs/2603.11365)
...
### [273] [Vision-Based Hand Shadowing for Robotic Manipulation via Inverse Kinematics](https://arxiv.org/abs/2603.11383)
...
### [274] [Deployment-Time Reliability of Learned Robot Policies](https://arxiv.org/abs/2603.11400)
*Christopher Agia*
...
### [275] [Real-time Rendering-based Surgical Instrument Tracking via Evolutionary Optimization](https://arxiv.org/abs/2603.11404)
...
### [276] [Grounding Robot Generalization in Training Data via Retrieval-Augmented VLMs](https://arxiv.org/abs/2603.11426)
...
### [278] [Enhancing Lightweight Vision Language Models through Group Competitive Learning for Socially Compliant Navigation](https://arxiv.org/abs/2603.11447)
...
### [279] [CoViLLM: An Adaptive Human-Robot Collaborative Assembly Framework Using Large Language Models for Manufacturing](https://arxiv.org/abs/2603.11461)
...
### [280] [NFPO: Stabilized Policy Optimization of Normalizing Flow for Robotic Policy Learning](https://arxiv.org/abs/2603.11470)
...
### [281] [SPARK: Skeleton-Parameter Aligned Retargeting on Humanoid Robots with Kinodynamic Trajectory Optimization](https://arxiv.org/abs/2603.11480)
...
### [282] [MiNI-Q: A Miniature, Wire-Free Quadruped with Unbounded, Independently Actuated Leg Joints](https://arxiv.org/abs/2603.11537)
...
### [283] [RoboClaw: An Agentic Framework for Scalable Long-Horizon Robotic Tasks](https://arxiv.org/abs/2603.11558)
...
### [284] [Unsupervised LiDAR-Based Multi-UAV Detection and Tracking Under Extreme Sparsity](https://arxiv.org/abs/2603.11586)
...
### [285] [Diversity You Can Actually Measure: A Fast, Model-Free Diversity Metric for Robotics Datasets](https://arxiv.org/abs/2603.11634)
...
### [286] [Learn Structure, Adapt on the Fly: Multi-Scale Residual Learning and Online Adaptation for Aerial Manipulators](https://arxiv.org/abs/2603.11638)
...
### [287] [Chunk-Boundary Artifact in Action-Chunked Generative Policies: A Noise-Sensitive Failure Mechanism](https://arxiv.org/abs/2603.11642)
*Rui Wang*
...
### [288] [A Hybrid Neural-Assisted Unscented Kalman Filter for Unmanned Ground Vehicle Navigation](https://arxiv.org/abs/2603.11649)
...
### [289] [Concurrent Prehensile and Nonprehensile Manipulation: A Practical Approach to Multi-Stage Dexterous Tasks](https://arxiv.org/abs/2603.11655)
...
### [290] [Coupling Tensor Trains with Graph of Convex Sets: Effective Compression, Exploration, and Planning in the C-Space](https://arxiv.org/abs/2603.11658)
...
### [291] [RADAR: Closed-Loop Robotic Data Generation via Semantic Planning and Autonomous Causal Environment Reset](https://arxiv.org/abs/2603.11811)
...
### [292] [Energy Prediction on Sloping Ground for Quadruped Robots](https://arxiv.org/abs/2603.11963)
...
### [293] [Learning Visuomotor Policy for Multi-Robot Laser Tag Game](https://arxiv.org/abs/2603.11980)
*Kai Li,Shiyu Zhao*
...
### [294] [Sim-to-reality adaptation for Deep Reinforcement Learning applied to an underwater docking application](https://arxiv.org/abs/2603.12020)
...
### [295] [Flight through Narrow Gaps with Morphing-Wing Drones](https://arxiv.org/abs/2603.12059)
*Julius Wanner,Hoang-Vu Phan,Charbel Toumieh,Dario Floreano*

Main category: cs.RO

TL;DR: 通过气动建模+最优轨迹+NMPC，自主控制翼扫掠无人机在低速下稳定穿越亚翼展窄缝。


<details>
  <summary>Details</summary>
Motivation: 固定翼穿越窄缝受翼展限制，需通过可变形翼在低速穿缝时应对升力骤降与翼扫掠时序控制难题。

Method: 建立含低速与失速攻角的翼扫掠气动模型，结合纵向动力学进行最优参考轨迹生成，并用带自适应代价与约束的非线性MPC控制。

Result: 在130g翼扫掠无人机上，5–7 m/s下窄缝通过平均高度误差约5 cm，满足近缝处全扫掠约束并可通过加速与抬头补偿升力损失。

Conclusion: 方法可在近恒定速度与高度下实现亚翼展窄缝稳定穿越，且硬件验证有效。
...
### [296] [Decentralized Cooperative Localization for Multi-Robot Systems with Asynchronous Sensor Fusion](https://arxiv.org/abs/2603.12075)
...
### [297] [CRAFT: A Tendon-Driven Hand with Hybrid Hard-Soft Compliance](https://arxiv.org/abs/2603.12120)
...
### [299] [SaPaVe: Towards Active Perception and Manipulation in Vision-Language-Action Models for Robotics](https://arxiv.org/abs/2603.12193)
...
### [300] [HandelBot: Real-World Piano Playing via Fast Adaptation of Dexterous Robot Policies](https://arxiv.org/abs/2603.12243)
...
### [303] [CR-Bench: Evaluating the Real-World Utility of AI Code Review Agents](https://arxiv.org/abs/2603.11078)
...
### [304] [Quality-Driven Agentic Reasoning for LLM-Assisted Software Design: Questions-of-Thoughts (QoT) as a Time-Series Self-QA Chain](https://arxiv.org/abs/2603.11082)
...
### [306] [Implicit Statistical Inference in Transformers: Approximating Likelihood-Ratio Tests In-Context](https://arxiv.org/abs/2603.10573)
...
### [307] [Structure-Aware Epistemic Uncertainty Quantification for Neural Operator PDE Surrogates](https://arxiv.org/abs/2603.11052)
...
### [308] [Graph Tokenization for Bridging Graphs and Transformers](https://arxiv.org/abs/2603.11099)
...
### [310] [Procedural Fairness via Group Counterfactual Explanation](https://arxiv.org/abs/2603.11140)
*Gideon Popoola,John Sheppard*

Main category: cs.LG

TL;DR: GCIG通过约束跨群体解释一致性来实现程序公平，实验证明其能显著减少解释差异且性能不降。


<details>
  <summary>Details</summary>
Motivation: 现有公平性研究偏重结果公平，忽视程序公平，导致不同保护群体可能获得不同解释，从而削弱信任。

Method: 提出GCIG正则化框架，基于群体条件基线的反事实集成梯度解释，并在训练中惩罚跨群体归因差异，以实现解释稳定性。

Result: 与六种SOTA方法相比，GCIG显著降低跨群体解释差异，同时保持有竞争力的预测性能与准确率-公平性权衡。

Conclusion: 对齐不同群体的模型推理过程能够在不显著牺牲预测性能的情况下提升公平性。
...
### [311] [Attention Gathers, MLPs Compose: A Causal Analysis of an Action-Outcome Circuit in VideoViT](https://arxiv.org/abs/2603.11142)
*Sai V R Chereddy*

Main category: cs.LG

TL;DR: 通过机制可解释性分析，视频ViT内部存在分工明确且冗余的“成功/失败”电路，信号跨层放大，说明模型具备超越任务的隐性语义知识，需机制级监督保障可信。


<details>
  <summary>Details</summary>
Motivation: 理解分类模型内部隐藏语义表征与因果电路，揭示其“隐性知识”对可信AI的影响，推动可解释与可控部署。

Method: 使用可解释/可证伪的机制可解释性方法，对预训练视频Vision Transformer进行逆向工程；通过激活修补（activation patching）与消融实验进行因果分析，定位注意力头与MLP块在“成功/失败”信号计算中的作用，并追踪跨层信号放大过程。

Result: 发现“成功/失败”信号从第5至11层被逐步放大；注意力头充当证据收集者提供低层信息，MLP块作为概念组合器主导生成成功信号；电路分布且冗余，解释了对简单消融的鲁棒性。

Conclusion: 该工作表明即使仅训练于分类任务的视频模型，也会在内部形成用于表示复杂结果（成功/失败）的精细电路，因而需要机制级监督以实现可信与可解释的AI。
...
### [312] [Representation Finetuning for Continual Learning](https://arxiv.org/abs/2603.11201)
...
### [313] [Simple Recipe Works: Vision-Language-Action Models are Natural Continual Learners with Reinforcement Learning](https://arxiv.org/abs/2603.11653)
*Jiaheng Hu,Jay Shim,Chen Tang,Yoonchang Sung,Bo Liu,Peter Stone,Roberto Martin-Martin*

Main category: cs.LG

TL;DR: 大模型VLA的持续RL中，简单LoRA顺序微调即可稳定高效，挑战了“顺序微调必遗忘”的常识。


<details>
  <summary>Details</summary>
Motivation: 检验传统持续学习关于顺序微调易灾难性遗忘的观点在大规模VLA+RL场景下是否成立。

Method: 在三种大模型与五个终身RL基准上系统评测，对比顺序微调（LoRA）与多种CRL方法，并分析遗忘与泛化。

Result: LoRA顺序微调具有高可塑性、几乎无遗忘，并保持强零样本泛化，常优于复杂CRL方法。

Conclusion: 顺序微调结合低秩适配在大规模VLA的持续强化学习中表现稳健，成为有效且可扩展的方案。

Abstract: Continual Reinforcement Learning (CRL) for Vision-Language-Action (VLA) models is a promising direction toward self-improving embodied agents that can adapt in openended, evolving environments. However, conventional wisdom from continual learning suggests that naive Sequential Fine-Tuning (Seq. FT) leads to catastrophic forgetting, necessitating complex CRL strategies. In this work, we take a step back and conduct a systematic study of CRL for large pretrained VLAs across three models and five challenging lifelong RL benchmarks. We find that, contrary to established belief, simple Seq. FT with low-rank adaptation (LoRA) is remarkably strong: it achieves high plasticity, exhibits little to no forgetting, and retains strong zero-shot generalization, frequently outperforming more sophisticated CRL methods. Through detailed analysis, we show that this robustness arises from a synergy between the large pretrained model, parameter-efficient adaptation, and on-policy RL. Together, these components reshape the stability-plasticity trade-off, making continual adaptation both stable and scalable. Our results position Sequential Fine-Tuning as a powerful method for continual RL with VLAs and provide new insights into lifelong learning in the large model era. Code is available at .

</details>


### [314] [Scaling Reasoning Efficiently via Relaxed On-Policy Distillation](https://arxiv.org/abs/2603.11137)
...
### [315] [Huntington Disease Automatic Speech Recognition with Biomarker Supervision](https://arxiv.org/abs/2603.11168)
...
### [316] [Hindsight-Anchored Policy Optimization: Turning Failure into Feedback in Sparse Reward Settings](https://arxiv.org/abs/2603.11321)
...
### [317] [Jailbreak Scaling Laws for Large Language Models: Polynomial-Exponential Crossover](https://arxiv.org/abs/2603.11331)
...
### [318] [LongFlow: Efficient KV Cache Compression for Reasoning M](https://arxiv.org/abs/2603.11504)
*Yi Su,Zhenxu Tian,Dan Qiao,Yuechi Zhou,Juntao Li,Min Zhang*

Main category: cs.LG

TL;DR: LongFlow用低开销重要性估计与融合内核压缩KV缓存，在长输出推理中大幅提速且精度几乎不受损。


<details>
  <summary>Details</summary>
Motivation: 长输出推理模型导致KV缓存巨大、内存与带宽压力高，既有方法偏向长输入短输出且重要性评估代价高。

Method: 提出基于当前查询的注意力中间量的重要性估计，结合FlashAttention与淘汰策略的融合内核，实现KV缓存压缩与高效计算。

Result: 在80%压缩率下吞吐最高提升11.8倍，模型准确率影响最小。

Conclusion: LongFlow通过高效重要性估计与融合算子实现高比例KV缓存压缩，在长输出推理场景下显著提升吞吐且精度影响很小。
...
### [319] [Fractional Rotation, Full Potential? Investigating Performance and Convergence of Partial RoPE](https://arxiv.org/abs/2603.11611)
...
### [320] [ARROW: Augmented Replay for RObust World models](https://arxiv.org/abs/2603.11395)
*Abdulaziz Alyahya,Abdallah Al Siyabi,Markus R. Ernst,Luke Yang,Levin Kuhlmann,Gideon Kowadlo*

Main category: cs.LG

TL;DR: ARROW在DreamerV3上用双缓冲分布匹配回放训练世界模型，显著减轻持续RL遗忘且不牺牲迁移，且更省内存。


<details>
  <summary>Details</summary>
Motivation: 现有基于回放的模型无关持续RL需要大内存，易受可扩展性限制；受神经科学启发，考虑向预测世界模型回放以缓解遗忘。

Method: 提出ARROW，基于DreamerV3的模型驱动持续RL；使用短期缓冲存放近期经验，长期缓冲通过分布匹配/智能采样保持任务多样性，以回放到世界模型而非策略。

Result: 在Atari（无共享结构）与Procgen CoinRun变体（共享结构）上，ARROW较同等大小回放的模型无关/模型驱动基线显著减少遗忘，并保持可比的前向迁移。

Conclusion: ARROW通过记忆高效的双缓冲回放与世界模型训练，在持续学习中减少遗忘并保持前向迁移，显示出模型驱动与生物启发方法的潜力。
...
### [321] [Bridging Discrete Marks and Continuous Dynamics: Dual-Path Cross-Interaction for Marked Temporal Point Processes](https://arxiv.org/abs/2603.11462)
...
Abstract: Predicting irregularly spaced event sequences with discrete marks poses significant challenges due to the complex, asynchronous dependencies embedded within continuous-time data sequential approaches capture dependencies among event tokens but ignore the continuous evolution between events, while Neural Ordinary Differential Equation (Neural ODE) methods model smooth dynamics yet fail to account for how event types influence future overcome these limitations, we propose NEXTPP, a dual-channel framework that unifies discrete and continuous representations via Event-granular Neural Evolution with Cross-Interaction for Marked Temporal Point Processes. Specifically, NEXTPP encodes discrete event marks via a self-attention mechanism, simultaneously evolving a latent continuous-time state using a Neural ODE. These parallel streams are then fused through a crossattention module to enable explicit bidirectional interaction between continuous and discrete representations. The fused representations drive the conditional intensity function of the neural Hawkes process, while an iterative thinning sampler is employed to generate future events. Extensive evaluations on five real-world datasets demonstrate that NEXTPP consistently outperforms state-of-the-art models. The source code can be found at .
...
TL;DR: 提出ChemDU任务与Chem4DBench数据集，并设计Chem4DLLM以从4D分子轨迹生成机制性解释。


<details>
  <summary>Details</summary>
Motivation: 现有化学理解多依赖静态分子表示，难以刻画键断裂、构象变化等动态现象。

Method: 提出ChemDU任务，将4D分子轨迹翻译为可解释的自然语言说明；构建Chem4DBench数据集；设计Chem4DLLM，结合等变图编码器与预训练大语言模型以捕获几何与旋转动力学。

Result: 给出了首个4D轨迹—解释配对基准Chem4DBench，并提出能处理分子几何与动力学的统一模型Chem4DLLM。

Conclusion: 作者期望ChemDU任务、Chem4DBench数据集与Chem4DLLM模型能推动动态化学理解与多模态科学推理研究。
...
### [323] [Grammar of the Wave: Towards Explainable Multivariate Time Series Event Detection via Neuro-Symbolic VLM Agents](https://arxiv.org/abs/2603.11479)
...
### [324] [KEPo: Knowledge Evolution Poison on Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2603.11501)
*Qizhi Chen,Chao Qi,Yihong Huang,Muquan Li,Rongzheng Wang,Dongyang Zhang,Ke Qin,Shuang Liang*

Main category: cs.LG

TL;DR: 提出面向GraphRAG的KEPo投毒攻击，通过伪造知识演化并注入KG，实现更高的攻击成功率，含多目标增强机制。


<details>
  <summary>Details</summary>
Motivation: 传统RAG攻击对GraphRAG无效，因其KG抽象重组文本、提升鲁棒性；因此需要揭示GraphRAG的潜在安全脆弱性并设计专门攻击。

Method: 提出KEPo：为特定目标查询构造“有毒事件”，伪造事件背景与知识演化路径，将投毒知识注入KG；在多目标场景下连接多组攻击语料，使投毒知识互相强化并扩大受影响社区。

Result: 在多个数据集上，KEPo在单目标与多目标攻击中均达到SOTA成功率，显著超过以往方法。

Conclusion: KEPo针对GraphRAG提出了有效的知识投毒攻击，能显著提升单目标与多目标攻击成功率，优于既有方法。

Abstract: Graph-based Retrieval-Augmented Generation (GraphRAG) constructs the Knowledge Graph (KG) from external databases to enhance the timeliness and accuracy of Large Language Model (LLM) ,this reliance on external data introduces new attack can inject poisoned texts into databases to manipulate LLMs into producing harmful target responses for attacker-chosen research primarily focuses on attacking conventional RAG ,such methods are ineffective against robustness derives from the KG abstraction of GraphRAG,which reorganizes injected text into a graph before retrieval,thereby enabling the LLM to reason based on the restructured context instead of raw poisoned expose latent security vulnerabilities in GraphRAG,we propose Knowledge Evolution Poison (KEPo),a novel poisoning attack method specifically designed for each target query,KEPo first generates a toxic event containing poisoned knowledge based on the target fabricating event backgrounds and forging knowledge evolution paths from original facts to the toxic event,it then poisons the KG and misleads the LLM into treating the poisoned knowledge as the final multi-target attack scenarios,KEPo further connects multiple attack corpora,enabling their poisoned knowledge to mutually reinforce while expanding the scale of poisoned communities,thereby amplifying attack results across multiple datasets demonstrate that KEPo achieves state-of-the-art attack success rates for both single-target and multi-target attacks,significantly outperforming previous methods.

</details>


### [325] [Survival Meets Classification: A Novel Framework for Early Risk Prediction Models of Chronic Diseases](https://arxiv.org/abs/2603.11598)
...
### [326] [Entropy-Preserving Reinforcement Learning](https://arxiv.org/abs/2603.11682)
*Aleksei Petrenko,Ben Lipkin,Kevin Chen,Erik Wijmans,Marco Cusumano-Towner,Raja Giryes,Philipp Krähenbühl*

Main category: cs.LG

TL;DR: 揭示策略梯度会降低熵并削弱探索，提出REPO与ADAPO控制熵，提升多样性与最终性能。


<details>
  <summary>Details</summary>
Motivation: 策略梯度训练会自然降低熵，削弱探索能力与轨迹多样性，需显式控制以维持探索与可训练性。

Method: 分析策略梯度目标对熵动态的影响，研究数值精度等因素，并提出REPO（调整优势函数以调控熵）与ADAPO（自适应非对称裁剪）机制。

Result: 熵保持方法在训练中维持多样性，最终策略性能更好且对新环境的序贯学习仍具可训练性。

Conclusion: 应在训练中主动监控与控制策略熵，以保持探索多样性并提升最终性能与可迁移性。
...
### [327] [Exploiting Expertise of Non-Expert and Diverse Agents in Social Bandit Learning: A Free Energy Approach](https://arxiv.org/abs/2603.11757)
...
### [328] [EnTransformer: A Deep Generative Transformer for Multivariate Probabilistic Forecasting](https://arxiv.org/abs/2603.11909)
...
### [329] [MobileKernelBench: Can LLMs Write Efficient Kernels for Mobile Devices?](https://arxiv.org/abs/2603.11935)
...
### [331] [Geometry-Aware Probabilistic Circuits via Voronoi Tessellations](https://arxiv.org/abs/2603.11946)
...
### [332] [Flowcean - Model Learning for Cyber-Physical Systems](https://arxiv.org/abs/2603.12015)
*Maximilian Schmidt,Swantje Plambeck,Markus Knitt,Hendrik Rose,Goerschwin Fey,Jan Christian Wieck,Stephan Balduin*

Main category: cs.LG

TL;DR: 提出Flowcean，面向CPS的数据驱动建模框架，模块化集成学习策略与评估流程，提升效率与可用性。


<details>
  <summary>Details</summary>
Motivation: CPS建模复杂且耗时，推动数据驱动的自动化模型生成需求。

Method: 提出Flowcean框架，提供多种学习策略、数据处理方法与评估指标，以模块化方式集成多样学习工具进行模型生成与评估。

Result: 框架实现了针对CPS场景的全面解决方案，支持灵活适配不同建模任务并简化生成与评估流程。

Conclusion: Flowcean通过模块化、可扩展架构整合多种学习库与工具，提升CPS数据驱动建模的效率与可用性。
...
### [333] [Slow-Fast Inference: Training-Free Inference Acceleration via Within-Sentence Support Stability](https://arxiv.org/abs/2603.12038)
...
### [334] [Chemical Reaction Networks Learn Better than Spiking Neural Networks](https://arxiv.org/abs/2603.12060)
...
### [335] [A Multi-Label Temporal Convolutional Framework for Transcription Factor Binding Characterization](https://arxiv.org/abs/2603.12073)
*Pietro Demurtas,Ferdinando Zanchetta,Giovanni Perini,Rita Fioresi*

Main category: cs.LG

TL;DR: 用TCN做TF多标签预测，性能可靠，能捕捉已知与潜在的TF协同。


<details>
  <summary>Details</summary>
Motivation: 现有方法多为单TF二分类，难以刻画TF间协同与互作逻辑。

Method: 将TF结合位点识别建模为多标签分类，采用TCN深度学习模型从DNA序列中预测多个TF的结合谱并捕获相关性。

Result: 模型在多TF预测上取得可靠性能，提取到与已知互作一致的模体与共结合模式，并提示新的TF关系。

Conclusion: 多标签学习能够可靠预测多个TF结合位点，并揭示已知与潜在的协同调控关系。
...
### [336] [Harnessing Data Asymmetry: Manifold Learning in the Finsler World](https://arxiv.org/abs/2603.11396)
*Thomas Dagès,Simon Weber,Daniel Cremers,Ron Kimmel*

Main category: cs.LG

TL;DR: 用Finsler几何引入非对称相异度与嵌入，改造t-SNE/UMAP，在多数据集上优于传统对称方法。


<details>
  <summary>Details</summary>
Motivation: 传统对称Riemann几何强制相异度与嵌入空间对称，忽略数据非均匀性带来的非对称信息。

Method: 提出Finsler流形学习流程：构建非对称相异度并在Finsler空间中嵌入；将现有方法推广为Finsler t-SNE与Finsler UMAP。

Result: 在合成与真实数据上，非对称流程揭示密度层级等信息，嵌入质量持续优于欧氏对应方法。

Conclusion: 采用Finsler几何的非对称流形学习能够更好揭示数据中传统对称方法丢失的信息，并在多种数据集上优于欧氏嵌入。
...
### [337] [Taming the Adversary: Stable Minimax Deep Deterministic Policy Gradient via Fractional Objectives](https://arxiv.org/abs/2603.12110)
*Taeho Lee,Donghwan Lee*

Main category: cs.LG

TL;DR: 通过最小最大对抗与分数化目标的MMDDPG学习鲁棒策略，在连续控制中显著提升抗扰动与不确定性能力。


<details>
  <summary>Details</summary>
Motivation: RL在外部扰动和模型不确定性下性能不稳定，需提升可靠性与鲁棒性。

Method: 将训练建模为用户策略与对抗扰动策略的最小最大优化，引入分数化目标以平衡任务性能与扰动幅度，并在MuJoCo进行评估。

Result: 在MuJoCo中对外力扰动和模型参数变化表现出显著更强的鲁棒性。

Conclusion: 提出的MMDDPG通过最小最大对抗训练与分数化目标，在连续控制中显著提升了对扰动与模型不确定性的鲁棒性。
...
### [338] [IsoCompute Playbook: Optimally Scaling Sampling Compute for LLM RL](https://arxiv.org/abs/2603.12151)
...
TL;DR: 系统分析LLM后训练RL的算力分配，给出并行rollout数随算力增长而饱和的规则，并说明批量大小主要影响稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有扩展律主要针对预训练，缺乏对LLM后训练RL阶段算力分配的系统指导。

Method: 将RL后训练视为算力受限优化，研究三类资源分配：每题并行rollout数、每批问题数、更新步数，并在不同模型与数据上验证。

Result: 并行rollout数在易题上通过解的锐化、在难题上通过覆盖扩展驱动性能提升；增加并行rollout可减少任务间干扰。

Conclusion: 提出RL后训练的算力分配规则：并行rollout数随算力增加后饱和，问题批量大小影响稳定性但可在较宽范围内选择。
...
### [339] [A Quantitative Characterization of Forgetting in Post-Training](https://arxiv.org/abs/2603.12163)
*Krishnakumar Balasubramanian,Shiva Prasad Kasiviswanathan*

Main category: cs.LG

TL;DR: TL;DR：在两模态混合框架下，前向KL导致旧质量消失，反向KL保旧但漂移由重叠控制并指数衰减；回放与近似在线方法的遗忘/保留条件可被精确刻画。


<details>
  <summary>Details</summary>
Motivation: 动机：后训练普遍但对遗忘发生的机制与条件缺乏原理性理解。

Method: 方法：在两模态混合模型抽象下，形式化质量遗忘与旧分量漂移；对等协方差高斯模式推导前向/反向KL的最优性与收敛性质，并分析回放与近似在线后训练方法（SDFT、TTT-Discover、OAPL）的条件。

Result: 结果：前向KL在新分布训练会使旧权重塌缩；反向KL避免质量遗忘，漂移由重叠误分配概率控制且随模式分离指数衰减；回放对前向KL需改变训练分布，对反向KL维持总体目标但抑制小批量旧模态饥饿；给出三种方法保旧质量与漂移受控的显式条件。

Conclusion: 结论：遗忘可由散度方向、行为重叠、采样机制与历史可见性共同精确量化，不同训练目标与方法在保留旧模式与漂移上有可判定条件。

Abstract: Continual post-training of generative models is widely used, yet a principled understanding of when and why forgetting occurs remains limited. We develop theoretical results under a two-mode mixture abstraction (representing old and new tasks), proposed by Chen et al. (2025) ( ), and formalize forgetting in two forms: (i) mass forgetting, where the old mixture weight collapses to zero, and (ii) old-component drift, where an already-correct old component shifts during training. For equal-covariance Gaussian modes, we prove that forward-KL objectives trained on data from the new distribution drive the old weight to zero, while reverse-KL objectives converge to the true target (thereby avoiding mass forgetting) and perturb the old mean only through overlap-gated misassignment probabilities controlled by the Bhattacharyya coefficient, yielding drift that decays exponentially with mode separation and a locally well-conditioned geometry with exponential convergence. We further quantify how replay interacts with these objectives. For forward-KL, replay must modify the training distribution to change the population optimum; for reverse-KL, replay leaves the population objective unchanged but prevents finite-batch old-mode starvation through bounded importance weighting. Finally, we analyze three recently proposed near-on-policy post-training methods, SDFT ( ), TTT-Discover ( ), and OAPL ( ), via the same lens and derive explicit conditions under which each retains old mass and exhibits overlap-controlled drift. Overall, our results show that forgetting can by precisely quantified based on the interaction between divergence direction, geometric behavioral overlap, sampling regime, and the visibility of past behavior during training.

</details>


### [340] [The Latent Color Subspace: Emergent Order in High-Dimensional Chaos](https://arxiv.org/abs/2603.12261)
...
Abstract: Text-to-image generation models have advanced rapidly, yet achieving fine-grained control over generated images remains difficult, largely due to limited understanding of how semantic information is encoded. We develop an interpretation of the color representation in the Variational Autoencoder latent space of FLUX.1 [Dev], revealing a structure reflecting Hue, Saturation, and Lightness. We verify our Latent Color Subspace (LCS) interpretation by demonstrating that it can both predict and explicitly control color, introducing a fully training-free method in FLUX based solely on closed-form latent-space manipulation. Code is available at .

</details>


### [341] [Neural Thickets: Diverse Task Experts Are Dense Around Pretrained Weights](https://arxiv.org/abs/2603.12228)
...
### [342] [Separable neural architectures as a primitive for unified predictive and generative intelligence](https://arxiv.org/abs/2603.12244)
*Reza T. Batley,Apurba Sarker,Rajib Mostakim,Andrew Klichine,Sourav Saha*

Main category: cs.LG

TL;DR: SNA通过可分离、低阶低秩约束统一多类模型，跨物理与语言任务表现稳健，可用于确定性与分布式预测生成。


<details>
  <summary>Details</summary>
Motivation: 现有整体式神经模型未显式利用系统的可分解结构，需要一种统一加性、二次与张量分解模型的表示类以注入结构归纳偏置。

Method: 提出可分离神经架构（SNA），通过限制交互阶次与张量秩实现高维映射的低元分解，并采用坐标感知的可分离表示进行建模。

Result: 在强化学习路径规划、微结构逆向生成、湍流分布建模和语言建模等四个领域验证其组合性与适用性，缓解确定性算子非物理漂移并适用于离散序列。

Conclusion: 可分离神经架构作为通用原语，能够统一确定性与分布式表征并跨域适用。
...
### [343] [Hybrid Quantum-Classical Encoding for Accurate Residue-Level pKa Prediction](https://arxiv.org/abs/2603.11061)
*Van Le,Tan Le*
...
### [344] [Agentic AI for Embodied-enhanced Beam Prediction in Low-Altitude Economy Networks](https://arxiv.org/abs/2603.11392)
...
### [345] [Efficient Cross-View Localization in 6G Space-Air-Ground Integrated Network](https://arxiv.org/abs/2603.11398)
...
### [346] [The Attack and Defense Landscape of Agentic AI: A Comprehensive Survey](https://arxiv.org/abs/2603.11088)
...
### [347] [WebWeaver: Breaking Topology Confidentiality in LLM Multi-Agent Systems with Stealthy Context-Based Inference](https://arxiv.org/abs/2603.11132)
*Zixun Xiong,Gaoyi Wu,Lingfeng Yao,Miao Pan,Xiaojiang Du,Hao Wang*

Main category: cs.CR

TL;DR: WebWeaver在不依赖管理者和ID查询的条件下，通过上下文与扩散机制高效推断LLM多智能体拓扑，并在防御下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑推断依赖不现实假设（控制管理者或直接查询ID），易被简单防御阻断，无法反映真实威胁。

Method: 提出WebWeaver框架：仅需攻陷任意单一智能体，基于上下文而非ID进行推断；设计隐蔽越狱机制与无越狱扩散方案，并引入掩码策略在扩散中保留已知拓扑并给出正确性保证。

Result: 在主动防御下，WebWeaver推断准确率较SOTA提升约60%，且开销可忽略。

Conclusion: WebWeaver能够在更现实的攻击条件下高效推断LLM-MAS通信拓扑，并在防御存在时仍显著优于现有方法。
...
### [348] [Taming OpenClaw: Security Analysis and Mitigation of Autonomous LLM Agent Threats](https://arxiv.org/abs/2603.11619)
...
### [349] [You Told Me to Do It: Measuring Instructional Text-induced Private Data Leakage in LLM Agents](https://arxiv.org/abs/2603.11862)
...
### [350] [The Mirror Design Pattern: Strict Data Geometry over Model Scale for Prompt Injection Detection](https://arxiv.org/abs/2603.11875)
*J Alex Corll*

Main category: cs.CR

TL;DR: 通过镜像式数据整理与线性SVM，L1提示注入检测可在极低延迟下获得显著优于大型模型的效果。


<details>
  <summary>Details</summary>
Motivation: 首层筛查需快速、确定、不可被提示、可审计，现有基于大型语义模型的检测不满足这些工程约束。

Method: 提出Mirror数据整理模式，将正负样本按32个镜像单元组织；使用5,000条严格筛选的公开样本训练稀疏字符n-gram线性SVM，并将权重编译为静态Rust工件；在524条留出集上评估延迟与指标，并与22M参数Prompt Guard 2对比。

Result: 线性SVM在留出集上达到95.97%召回与92.07% F1，亚毫秒延迟且无外部运行时依赖；Prompt Guard 2仅44.35%召回、59.14% F1，延迟中位49ms、p95 324ms。

Conclusion: 在L1提示注入筛查中，严格的数据几何与匹配设计比模型规模更关键，线性模型可实现高效可审计的防御。
...
### [351] [Understanding LLM Behavior When Encountering User-Supplied Harmful Content in Harmless Tasks](https://arxiv.org/abs/2603.11914)
...
### [352] [Delayed Backdoor Attacks: Exploring the Temporal Dimension as a New Attack Surface in Pre-Trained Models](https://arxiv.org/abs/2603.11949)
...
### [353] [Cascade: Composing Software-Hardware Attack Gadgets for Adversarial Threat Amplification in Compound AI Systems](https://arxiv.org/abs/2603.12023)
...
### [354] [Multi-Robot Multitask Gaussian Process Estimation and Coverage](https://arxiv.org/abs/2603.11264)
...
### [355] [ELISA: An Interpretable Hybrid Generative AI Agent for Expression-Grounded Discovery in Single-Cell Genomics](https://arxiv.org/abs/2603.11872)
*Omar Coser*
...
### [356] [Shadowless Projection Mapping for Tabletop Workspaces with Synthetic Aperture Projector](https://arxiv.org/abs/2603.11551)
...
### [357] [High-Contrast Projection Mapping under Light Field Illumination with LED Display and Aperiodic Lens Array](https://arxiv.org/abs/2603.11573)
...
### [358] [From Pets to Robots: MojiKit as a Data-Informed Toolkit for Affective HRI Design](https://arxiv.org/abs/2603.11632)
...
### [359] [HiSync: Spatio-Temporally Aligning Hand Motion from Wearable IMU and On-Robot Camera for Command Source Identification in Long-Range HRI](https://arxiv.org/abs/2603.11809)
...
### [360] ["I followed what felt right, not what I was told": Autonomy, Coaching, and Recognizing Bias Through AI-Mediated Dialogue](https://arxiv.org/abs/2603.11274)
*Atieh Taheri,Hamza El Alaoui,Patrick Carrington,Jeffrey P. Bigham*

Main category: cs.HC

TL;DR: AI对话能提升识别能力主义微侵犯，但偏置与包容引导带来不同效果与权衡。


<details>
  <summary>Details</summary>
Motivation: 能力主义微侵犯普遍存在，但帮助识别的干预有限。

Method: 160名参与者前测—干预—后测，四条件：偏置引导对话、包容引导对话、自主对话、阅读对照；量表评分与质性反思。

Result: 对话式条件总体优于阅读；偏置引导提升区分力但增加负向评价；包容/无引导更平衡；阅读提升弱且可能下降；质性显示偏置常被拒绝，包容被用作支架。

Conclusion: AI介入对识别能力主义微侵犯有促进作用，但不同引导存在权衡。
...
### [361] [From Control to Foresight: Simulation as a New Paradigm for Human-Agent Collaboration](https://arxiv.org/abs/2603.11677)
*Gaole He,Brian Y. Lim*

Main category: cs.HC

TL;DR: 提出在决策前进行未来轨迹模拟的协作范式，以提升人机协作的前瞻性与决策质量。


<details>
  <summary>Details</summary>
Motivation: 现有人机交互多为逐步、被动纠错，用户缺乏对后续影响的可视性，必须进行高负担且易出错的心理模拟。

Method: 提出“simulation-in-the-loop”概念框架，通过在决策前模拟未来轨迹来辅助人机协作，并以具体协作场景进行阐释。

Result: 模拟式交互可将干预从被动猜测转为知情探索，帮助发现潜在约束与偏好，提高决策质量。

Conclusion: 论文认为要实现有效的人机协作，需要引入带前瞻性的模拟式交互，而不仅是对单步操作的控制。
...
### [362] [Ghost Framing Theory: Exploring the role of generative AI in new venture rhetorical legitimation](https://arxiv.org/abs/2603.11384)
*Greg Nyilasy*
...
### [363] [Evaluation format, not model capability, drives triage failure in the assessment of consumer health AI](https://arxiv.org/abs/2603.11413)
...
### [364] [Human-Centred LLM Privacy Audits: Findings and Frictions](https://arxiv.org/abs/2603.12094)
*Dimitri Staufer,Kirsten Morehouse,David Hartmann,Bettina Berendt*

Main category: cs.HC

TL;DR: 提出LMP2自审工具与实证研究，发现LLM能对姓名生成一定可预测特征，用户需要控制；评估揭示方法学挑战并给出改进建议。


<details>
  <summary>Details</summary>
Motivation: 人们缺乏可实际检查LLM对其姓名关联信息的方法，且模型可能推断个人特征，引发隐私与控制需求。

Method: 构建浏览器自审工具LMP2，进行两项用户研究（总N=458），并对8个LLM在公众人物与虚构姓名上的关联预测进行验证评估。

Result: GPT-4o可对普通人50个特征中的11个达到≥60%准确率；用户希望控制LLM关联但不认为全部输出都是隐私侵犯；不同姓名条件与模型默认输出可清晰区分；提出9类摩擦与建议。

Conclusion: 论文提出了人本中心的LLM隐私自审框架与建议，指出评估存在方法学摩擦与危机。
...
### [365] [An Intent of Collaboration: On Agencies between Designers and Emerging (Intelligent) Technologies](https://arxiv.org/abs/2603.12018)
*Pei-Ying Lin,Julie Heij,Iris Borst,Britt Joosten,Kristina Andersen,Wijnand IJsselsteijn*

Main category: cs.HC

TL;DR: 三个月共创实验显示LLM易削弱设计师能动性，需通过内省、理解技术与重塑协作关系来重获创意主导权。


<details>
  <summary>Details</summary>
Motivation: 在生成式AI兴起下，探索设计师如何在与智能伙伴协作中保持创造力与能动性，尤其在数字工艺领域。

Method: 三位设计师与Google LLM进行为期三个月的共创实验与反思。

Result: 发现设计师易失去创意能动性；有效恢复需对自身创作过程的认知、对技术能力的结构性理解及关系调整。

Conclusion: 设计师与LLM共创存在权能动态，需通过内省、理解技术与调整关系以重获创意主导权。
...
### [366] [From Pen Strokes to Sleep States: Detecting Low-Recovery Days Using Sigma-Lognormal Handwriting Features](https://arxiv.org/abs/2603.11512)
*Chisa Tanaka,Andrew Vargo,Anna Scius-Bertrand,Andreas Fischer,Koichi Kise*

Main category: cs.HC

TL;DR: 用Sigma-Lognormal手写特征做个体化分类，可从日常手写中识别低恢复日，心脏相关睡眠指标效果最好，且不受任务/时间影响。


<details>
  <summary>Details</summary>
Motivation: 探索健康个体日常生理波动（睡眠恢复状态）是否可从在线手写动态中推断。

Method: 基于Sigma-Lognormal模型提取手写动力学特征，构建个体化二分类框架；28天自然场景、13名学生每日三次采集手写，使用可穿戴戒指记录夜间心脏指标；以每人四项睡眠指标的最低/最高四分位为正类，采用Leave-One-Day-Out交叉验证评估。

Result: 四项睡眠指标的PR-AUC均显著高于0.25基线（FDR校正后），心脏相关变量表现最佳；任务类型与记录时段对分类性能无显著影响。

Conclusion: 日常手写中的运动动力学包含可反映个体睡眠相关恢复波动的信号，适用于非侵入式健康监测。
...
### [367] [Prediction of Grade, Gender, and Academic Performance of Children and Teenagers from Handwriting Using the Sigma-Lognormal Model](https://arxiv.org/abs/2603.11519)
...
### [368] [Enhancing Value Alignment of LLMs with Multi-agent system and Combinatorial Fusion](https://arxiv.org/abs/2603.11126)
...
### [369] [How Intelligence Emerges: A Minimal Theory of Dynamic Adaptive Coordination](https://arxiv.org/abs/2603.11560)
*Stefano Grassi*

Main category: cs.MA

TL;DR: 提出多主体—激励—环境闭环动力学框架：耗散性保证可行不变集；环境记忆使协调不可还原为静态目标并引入历史敏感性；线性谱条件刻画稳定与振荡。


<details>
  <summary>Details</summary>
Motivation: 超越以均衡优化或主体学习为中心的协调分析，解释在持久环境与分布式激励作用下协调如何作为动态结构属性涌现。

Method: 构建递归闭环的多主体—激励—环境动力学框架，在耗散性等条件下分析闭环系统性质，并用线性化雅可比谱条件刻画稳定性与振荡。

Result: （1）在耗散条件下存在有界前向不变集保证可行性；（2）当激励依赖环境记忆时，系统一般不可化为仅基于主体状态的静态全局目标；（3）持久环境引入历史敏感性，除非系统全局收缩。线性例子给出谱条件下的局部稳定与振荡区间。

Conclusion: 在持久环境与激励介导的自适应交互下，协调可作为耦合动力学的结构性结果出现，而非福利最大化或集中设计的产物。
...
### [370] [Hybrid Human-Agent Social Dilemmas in Energy Markets](https://arxiv.org/abs/2603.11834)
...
### [371] [WORKSWORLD: A Domain for Integrated Numeric Planning and Scheduling of Distributed Pipelined Workflows](https://arxiv.org/abs/2603.12214)
*Taylor Paul,William Regli*

Main category: cs.DC

TL;DR: 提出WORKSWORLD与图表示，实现分布式工作流的联合规划与调度，并在中等规模实例上验证可行性。


<details>
  <summary>Details</summary>
Motivation: 实现分布式数据管道的自动规划与调度，降低用户显式定义完整工作流图的负担。

Method: 构建统一的工作流与资源图表示，引入WORKSWORLD域，采用数值域无关规划器联合规划与调度生成工作流与资源映射计划。

Result: 在1小时CPU时间与30GB内存条件下，可求解跨8个站点、最长14组件的线性链工作流。

Conclusion: 该框架可在有限资源下自动生成并调度分布式数据工作流，验证了其实用性。
...
### [372] [Worst-case low-rank approximations](https://arxiv.org/abs/2603.11304)
*Anya Fries,Markus Reichstein,David Blei,Jonas Peters*

Main category: stat.ML

TL;DR: 提出wcPCA统一框架与新估计器，给出最坏情形最优性与一致性理论，并在真实任务中提升最坏性能。


<details>
  <summary>Details</summary>
Motivation: 异质域数据下标准PCA在未见域中方差解释能力下降，需要面向最坏情形的稳健降维方法。

Method: 构建统一的wcPCA框架，提出norm-minPCA与norm-maxregret等估计器；证明在源域协方差（可归一化）凸包内的目标域上最坏情形最优性；给出一致性与渐近保证；扩展至归纳矩阵补全并给出近似最坏最优性。

Result: 在理论上获得最坏情形最优性与一致性保证；在模拟与生态系统—大气通量等真实数据上显著提升最坏表现，平均性能仅略有下降。

Conclusion: 提出wcPCA及其扩展目标在多域分布偏移下具备最坏情形最优性，并在实际应用中提升最坏性能且平均性能损失小。
...
### [373] [Beyond Polarity: Multi-Dimensional LLM Sentiment Signals for WTI Crude Oil Futures Return Prediction](https://arxiv.org/abs/2603.11408)
...
### [374] [Adapting Dijkstra for Buffers and Unlimited Transfers](https://arxiv.org/abs/2603.11729)
