...
TL;DR: 将物理现象拆成因果事件链并以物理公式约束，再用转移感知提示保持事件连贯，从而提升物理可行视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型虽能借助大模型常识，但缺乏对物理因果演化的条件约束，容易生成单一瞬时而非连续物理过程。

Method: 提出物理驱动事件链推理模块，将提示拆解为因果事件单元并用物理公式约束；提出转移感知跨模态提示模块，将事件单元转为时序对齐的视觉-语言提示并通过交互编辑合成关键帧。

Result: 在PhyGenBench与VideoPhy上取得更优的物理可行视频生成表现。

Conclusion: 该工作将物理可行视频生成视为因果事件序列建模，并通过两模块提升物理一致性与可生成性，实验显示在多基准上更优。
...
Abstract: Medical vision-language pretraining (VLP) models have recently been investigated for their generalization to diverse downstream tasks. However, current medical VLP methods typically force the model to learn simple and complex concepts simultaneously. This anti-cognitive process leads to suboptimal feature representations, especially under distribution shift. To address this limitation, we propose a Knowledge-driven Cognitive Orchestration for Medical VLP (MedKCO) that involves both the ordering of the pretraining data and the learning objective of vision-language contrast. Specifically, we design a two level curriculum by incorporating diagnostic sensitivity and intra-class sample representativeness for the ordering of the pretraining data. Moreover, considering the inter-class similarity of medical images, we introduce a self-paced asymmetric contrastive loss to dynamically adjust the participation of the pretraining objective. We evaluate the proposed pretraining method on three medical imaging scenarios in multiple vision-language downstream tasks, and compare it with several curriculum learning methods. Extensive experiments show that our method significantly surpasses all baselines. .
...
TL;DR: 用Transformer做图文组合检索，结合局部注意与全局对齐并加权融合，在皮肤癌检索上优于SOTA。


<details>
  <summary>Details</summary>
Motivation: 临床检索需要结合参考病灶图像与文字描述，现有方法难以兼顾局部判别特征与全局语义一致性。

Method: 基于Transformer学习图像-文本组合查询的层次表征，并通过多空间注意力掩码进行局部对齐、全局语义对齐，使用凸组合权重融合相似度。

Result: 在Derm7pt数据集上相较现有方法取得一致提升。

Conclusion: 提出的层次化组合查询与全局-局部联合对齐框架在皮肤癌检索中有效，具有临床应用潜力。
...
TL;DR: PRLF通过估计模态可靠性并渐进对齐，提高缺失模态下情感分析性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景常出现模态缺失导致特征失配，直接融合会损害完整模态表征。

Method: 提出渐进式表示学习框架PRLF，包含自适应模态可靠性估计器(AMRE)与渐进交互模块(ProgInteract)，以主导模态为对齐目标并抑制噪声。

Result: 在CMU-MOSI、CMU-MOSEI、SIMS上在跨/内模态缺失场景优于SOTA。

Conclusion: PRLF在多模态缺失条件下具有更强鲁棒性与泛化能力。
...
TL;DR: 用SegFormer分割多组织HR-pQCT并提取软组织放射组学特征，可显著提升骨质疏松分类表现。


<details>
  <summary>Details</summary>
Motivation: DXA忽略骨微结构与软组织，现有HR-pQCT流程主要关注骨组织，未充分利用数据。

Method: 采用SegFormer实现胫腓骨皮质/松质及软组织自动分割；软组织细分后提取放射组学特征，降维并训练多种分类器进行图像与患者层面分类。

Result: SegFormer分割F1均值95.36%；肌腱肌肉组织特征图像级准确率80.08%、AUROC 0.85优于骨组织；患者级用软组织放射组学将AUROC从0.792提升到0.875。

Conclusion: 多区域HR-pQCT自动分割与软组织放射组学能提升骨质疏松检测，软组织信号具有重要临床价值。
...
TL;DR: 提出HelixTrack事件驱动方法与TQE数据集，实现螺旋桨目标的低时延联合跟踪与RPM估计，性能与速度显著领先基线。


<details>
  <summary>Details</summary>
Motivation: 现有帧式/事件式跟踪在快速周期运动和强干扰下易漂移，无法稳定跟踪螺旋桨等周期目标且缺乏联合跟踪+RPM数据集。

Method: 提出全事件驱动HelixTrack：在线估计单应性将事件反投影到转子平面；用卡尔曼滤波估计相位；批量迭代用相位残差耦合几何以细化位姿。

Result: 在TQE数据集上，HelixTrack以约11.8倍实时速率处理全速事件，微秒级时延，性能持续优于逐事件和聚合式RPM基线。

Conclusion: HelixTrack实现对螺旋桨类目标的微秒级低时延跟踪与RPM估计，并在新数据集上显著优于基线。
...
TL;DR: Poly-DETR用极坐标稀疏多边形回归替代密集掩码，提出注意力与训练策略改进，在多数据集上更准更省内存。


<details>
  <summary>Details</summary>
Motivation: 现有实例分割在高分辨率输入与轻量实时推理之间存在冲突，且密集掩码预测成本高。

Method: 将实例分割重构为基于极坐标表示的稀疏顶点回归；提出Polar Deformable Attention与Position-Aware Training Scheme以动态更新监督并聚焦边界信息；构建并行的掩码式对照模型进行系统比较。

Result: 在MS COCO test-dev上较极坐标SOTA提升4.7 mAP；在Cityscapes高分辨率下内存消耗约减半；在PanNuke与SpaceNet上优于掩码式对照模型。

Conclusion: Poly-DETR通过极坐标稀疏顶点回归与位置感知训练，在高分辨率场景下实现轻量高效的实例分割，并在多数据集上优于或显著改进现有方法。
...
TL;DR: 提出DiP以伪节点与动态信息路径实现高效自适应多模态图学习，实验表现领先。


<details>
  <summary>Details</summary>
Motivation: 现有多模态图方法多为静态结构或密集注意力，灵活性与表达性不足。

Method: 引入模态特定伪节点，在共享状态空间进行近邻引导的动态路由与跨模态信息通路构建，实现线性复杂度的自适应消息传递。

Result: 在链路预测与节点分类任务上，多个基准数据集实验显示DiP一致优于现有方法。

Conclusion: DiP通过动态信息路径与伪节点机制，实现跨模态高效稀疏传播，性能优于基线。
...
### [51] [X-GS: An Extensible Open Framework Unifying 3DGS Architectures with Downstream Multimodal Models](https://arxiv.org/abs/2603.09632)
*Yueen Ma,Irwin King*

Main category: cs.CV

TL;DR: X-GS统一3DGS在线SLAM与语义增强，通过高效管线与在线VQ实现实时，并连接多模态下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法彼此割裂，难以同时覆盖在线SLAM、语义增强与多模态下游应用。

Method: 提出X-GS框架与X-GS-Perceiver管线，输入未定姿态RGB/RGB-D视频联合优化几何与位姿，并将基础模型语义特征蒸馏到3D高斯；通过在线VQ、GPU网格采样与并行化实现实时；X-GS-Thinker调用视觉-语言模型完成下游任务。

Result: 在真实数据集上验证了效率、效果与多模态能力，支持检测、零样本描述等任务。

Conclusion: X-GS 通过统一框架实现了实时3DGS在线SLAM与语义增强，并能对接多模态下游任务。
...
TL;DR: 学习连续特征场并自监督聚类，实现首个开放世界前馈凸分解模型，质量更高且跨表示泛化。


<details>
  <summary>Details</summary>
Motivation: 传统凸分解难以在开放世界中高效前馈生成，需要更可扩展、泛化的学习式方案。

Method: 学习连续特征场并通过自监督几何目标（基于凸性定义）进行聚类，获得凸分解；可用于单形状优化与数据集上的可扩展学习。

Result: 在3D形状上生成比替代方法更高质量的凸分解，并能泛化到网格、CAD与高斯溅射等表示。

Conclusion: 提出基于特征场学习的开放世界凸分解前馈模型，可在多表示上生成高质量凸分解。
...
TL;DR: 提出DenoiseSplat，利用干净2D渲染监督训练的前馈3DGS，在多种噪声下显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF与3DGS对真实噪声和伪影敏感，实际应用中输入常不干净，需在噪声条件下鲁棒重建。

Method: 提出前馈式3D Gaussian Splatting去噪框架DenoiseSplat，采用轻量级MVSplat式骨干；在RE10K上构建噪声-干净对照基准，注入高斯/泊松/散斑/椒盐噪声；仅用干净2D渲染监督端到端训练。

Result: 在噪声RE10K上，DenoiseSplat在PSNR/SSIM与LPIPS上均优于原始MVSplat及两阶段基线（IDF+MVSplat），跨噪声类型与强度稳定领先。

Conclusion: DenoiseSplat在噪声多视图条件下显著提升重建与新视角合成质量，且无需3D真值监督。
...
TL;DR: 将事件流建模为视频并进行三层蒸馏的EventVGGT显著提升事件相机单目深度估计精度与时序一致性，且具备强泛化。


<details>
  <summary>Details</summary>
Motivation: 现有无标注方法将事件流当作独立帧处理，忽视事件数据的时间连续性，无法利用VFM的时序先验，导致深度预测不一致且精度受限。

Method: 提出EventVGGT框架，将事件流视为连贯视频，并从VGGT蒸馏时空与多视几何先验，采用三层蒸馏：CMFM输出级跨模态特征融合、STFD特征级时空蒸馏、TCD时间级跨帧一致性对齐。

Result: 在EventScape上30m绝对均值深度误差由2.30降至1.06（>53%提升），并在DENSE与MVSEC上实现稳健零样本泛化。

Conclusion: EventVGGT通过显式建模事件流为视频序列并进行三层蒸馏，获得更一致、更准确的深度估计，并在多数据集上表现稳健。
...
TL;DR: 提出组件感知两阶段草图到图像框架，显著提升细节与空间一致性并在多数据集领先。


<details>
  <summary>Details</summary>
Motivation: 草图稀疏且风格多样，现有方法难以重建细节、保持空间对齐与跨域泛化。

Method: 提出两阶段组件感知自我精修框架：SA2N提取局部语义结构特征，CGF保持坐标融合空间布局，SARR基于改进StyleGAN2进行空间自适应迭代细化。

Result: 在CelebAMask-HQ等数据集上显著提升FID/IS/KID/SSIM等指标，图像逼真度、语义准确性与感知质量更好。

Conclusion: 该方法在多数据集上优于现有GAN与扩散模型，具有更高的效率与跨域一致性，可用于草图图像生成等应用。
...
TL;DR: 提出DynHiL-EQA与DIVRR，通过视角精化和选择性记忆提升动态EQA鲁棒性与效率。


<details>
  <summary>Details</summary>
Motivation: 动态人群场景中遮挡与非平稳性使EQA证据易瞬时且冗余，需解决视角歧义与紧凑记忆维护。

Method: 提出DIVRR框架，结合相关性引导的视角精化与选择性记忆准入；引入DynHiL-EQA数据集含动态与静态子集。

Result: 在DynHiL-EQA与HM-EQA上优于基线，并保持高推理效率。

Conclusion: DIVRR在动态与静态环境中均提升鲁棒性与效率。
...
### [98] [GeoSolver: Scaling Test-Time Reasoning in Remote Sensing with Fine-Grained Process Supervision](https://arxiv.org/abs/2603.09551)
...
### [99] [GeoAlignCLIP: Enhancing Fine-Grained Vision-Language Alignment in Remote Sensing via Multi-Granular Consistency Learning](https://arxiv.org/abs/2603.09566)
...
### [100] [More than the Sum: Panorama-Language Models for Adverse Omni-Scenes](https://arxiv.org/abs/2603.09573)
...
### [103] [A saccade-inspired approach to image classification using visiontransformer attention maps](https://arxiv.org/abs/2603.09613)
...
### [104] [Physics-Driven 3D Gaussian Rendering for Zero-Shot MRI Super-Resolution](https://arxiv.org/abs/2603.09621)
*Shuting Liu,Lei Zhang,Wei Huang,Zhao Zhang,Zizhou Wang*

Main category: cs.CV

TL;DR: 用显式高斯表示的零样本MRI超分框架，结合物理建模与并行光栅化，在无需配对数据下提升重建质量并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有MRI超分方法在配对数据依赖与计算开销之间受限，需在数据需求与效率间取得平衡。

Method: 提出零样本MRI超分框架，使用MRI定制高斯参数嵌入组织物理属性，结合物理驱动的体渲染（归一化高斯聚合）建模信号形成，并采用砖块式无序光栅化实现并行3D计算。

Result: 在两个公开MRI数据集上实现更高的重建质量与更好的效率，同时降低训练与推理成本。

Conclusion: 基于显式高斯表示的零样本MRI超分方法在重建质量与效率上优于现有方法，具备临床应用潜力。
...
### [105] [Decoder-Free Distillation for Quantized Image Restoration](https://arxiv.org/abs/2603.09624)
...
### [111] [AutoViVQA: A Large-Scale Automatically Constructed Dataset for Vietnamese Visual Question Answering](https://arxiv.org/abs/2603.09689)
...
### [115] [GSStream: 3D Gaussian Splatting based Volumetric Scene Streaming System](https://arxiv.org/abs/2603.09718)
...
### [117] [FetalAgents: A Multi-Agent System for Fetal Ultrasound Image and Video Analysis](https://arxiv.org/abs/2603.09733)
...
### [118] [ENIGMA-360: An Ego-Exo Dataset for Human Behavior Understanding in Industrial Scenarios](https://arxiv.org/abs/2603.09741)
...
### [120] [LogoDiffuser: Training-Free Multilingual Logo Generation and Stylization via Letter-Aware Attention Control](https://arxiv.org/abs/2603.09759)
...
### [121] [Ego: Embedding-Guided Personalization of Vision-Language Models](https://arxiv.org/abs/2603.09771)
...
### [122] [Removing the Trigger, Not the Backdoor: Alternative Triggers and Latent Backdoors](https://arxiv.org/abs/2603.09772)
...
### [123] [What is Missing? Explaining Neurons Activated by Absent Concepts](https://arxiv.org/abs/2603.09787)
...
### [124] [Test-time Ego-Exo-centric Adaptation for Action Anticipation via Multi-Label Prototype Growing and Dual-Clue Consistency](https://arxiv.org/abs/2603.09798)
...
### [126] [ConfCtrl: Enabling Precise Camera Control in Video Diffusion via Confidence-Aware Interpolation](https://arxiv.org/abs/2603.09819)
...
### [127] [BrainSTR: Spatio-Temporal Contrastive Learning for Interpretable Dynamic Brain Network Modeling](https://arxiv.org/abs/2603.09825)
...
### [136] [Adaptive Clinical-Aware Latent Diffusion for Multimodal Brain Image Generation and Missing Modality Imputation](https://arxiv.org/abs/2603.09931)
...
Abstract: Multimodal neuroimaging provides complementary insights for Alzheimer's disease diagnosis, yet clinical datasets frequently suffer from missing modalities. We propose ACADiff, a framework that synthesizes missing brain imaging modalities through adaptive clinical-aware diffusion. ACADiff learns mappings between incomplete multimodal observations and target modalities by progressively denoising latent representations while attending to available imaging data and clinical metadata. The framework employs adaptive fusion that dynamically reconfigures based on input availability, coupled with semantic clinical guidance via GPT-4o-encoded prompts. Three specialized generators enable bidirectional synthesis among sMRI, FDG-PET, and AV45-PET. Evaluated on ADNI subjects, ACADiff achieves superior generation quality and maintains robust diagnostic performance even under extreme 80\% missing scenarios, outperforming all existing baselines. To promote reproducibility, code is available at
...
### [138] [No Image, No Problem: End-to-End Multi-Task Cardiac Analysis from Undersampled k-Space](https://arxiv.org/abs/2603.09945)
...
TL;DR: 用专家-非专家分歧定义WSD，通过多任务或加权损失融入MIL训练，提升前列腺癌WSI分级效果。


<details>
  <summary>Details</summary>
Motivation: 非专家对WSI诊断困难导致标注分歧，需量化并利用样本难度以提升模型学习。

Method: 提出两种利用WSD的方法：多任务学习与加权分类损失，并在前列腺癌Gleason分级的MIL框架中验证。

Result: 在不同特征编码器与MIL方法上，融合WSD均带来一致性能提升，特别是高等级样本。

Conclusion: 引入基于专家与非专家分歧的WSD并在训练中利用，可稳定提升WSI分类表现，尤其对高Gleason等级。
...
### [140] [From Semantics to Pixels: Coarse-to-Fine Masked Autoencoders for Hierarchical Visual Understanding](https://arxiv.org/abs/2603.09955)
...
### [141] [ReCoSplat: Autoregressive Feed-Forward Gaussian Splatting Using Render-and-Compare](https://arxiv.org/abs/2603.09968)
...
### [142] [One Language, Two Scripts: Probing Script-Invariance in LLM Concept Representations](https://arxiv.org/abs/2603.08869)
*Sripad Karne*
...
### [143] [MultiGraSCCo: A Multilingual Anonymization Benchmark with Annotations of Personal Identifiers](https://arxiv.org/abs/2603.08879)
*Ibrahim Baroud,Christoph Otto,Vera Czehmann,Christine Hovhannisyan,Lisa Raithel,Sebastian Möller,Roland Roller*

Main category: cs.CL

TL;DR: 用机器翻译与合成数据构建十语种匿名化基准，质量经专家验证，可用于训练、验证与自动PII检测改进。


<details>
  <summary>Details</summary>
Motivation: 真实患者数据受隐私限制难以获取，限制匿名化系统开发；需要带有个人信息标注的数据集，合成数据与机器翻译可缓解数据稀缺并满足合规。

Method: 采用机器翻译方法将高资源语言的真实或合成数据翻译为低资源语言，保留原始标注，并对人名、地名进行文化与语境适配；随后由医疗专业人员评估翻译质量。

Result: 构建了包含2500+个人信息标注的多语种匿名化基准，翻译质量经医疗人员验证；提供基准与标注指南供研究使用。

Conclusion: 该研究构建了覆盖十种语言的多语种匿名化基准，并证明其翻译质量与个人信息适配性可靠，可用于多种隐私合规场景。
...
### [144] [ConFu: Contemplate the Future for Better Speculative Sampling](https://arxiv.org/abs/2603.08899)
...
### [145] [SciTaRC: Benchmarking QA on Scientific Tabular Data that Requires Language Reasoning and Complex Computation](https://arxiv.org/abs/2603.08910)
...
### [146] [Automated Thematic Analysis for Clinical Qualitative Data: Iterative Codebook Refinement with Full Provenance](https://arxiv.org/abs/2603.08989)
...
TL;DR: 提出具溯源的迭代式自动TA，在多语料上优于基线并与专家主题一致。


<details>
  <summary>Details</summary>
Motivation: 人工TA在规模化与可复现性上受限，现有LLM方法代码本泛化性与可审计性不足。

Method: 提出自动化TA框架，结合迭代代码本精炼与全流程溯源记录，并在五个语料上评估对比六个基线。

Result: 在五个数据集中四个取得最高综合质量分；迭代精炼在四个数据集上显著提升，主要改善代码可复用性与分布一致性且保持描述质量；临床语料主题与专家标注一致。

Conclusion: 框架通过迭代代码本与溯源追踪在多语料上总体优于基线，且主题与专家一致。
...
### [147] [Learning When to Sample: Confidence-Aware Self-Consistency for Efficient LLM Chain-of-Thought Reasoning](https://arxiv.org/abs/2603.08999)
...
### [148] [Bioalignment: Measuring and Improving LLM Disposition Toward Biological Systems for AI Safety](https://arxiv.org/abs/2603.09154)
...
### [149] [DuplexCascade: Full-Duplex Speech-to-Speech Dialogue with VAD-Free Cascaded ASR-LLM-TTS Pipeline and Micro-Turn Optimization](https://arxiv.org/abs/2603.09180)
*Jianing Yang,Yusuke Fujita,Yui Sudo*

Main category: cs.CL

TL;DR: 用分块微轮次和控制token的VAD-free级联系统，实现全双工交互且保留强LLM对话能力。


<details>
  <summary>Details</summary>
Motivation: 级联系统受VAD分割限制为半双工且控制脆弱，而端到端全双工模型又难以保持对话智能。

Method: 提出VAD-free的级联流式ASR-LLM-TTS管线，将长轮次改为分块微轮次交互，并引入对话控制token以在流式约束下协调轮换与时序。

Result: 在Full-DuplexBench与VoiceBench上实现SOTA的全双工轮换与强对话智能（开源系统中）。

Conclusion: DuplexCascade在不使用VAD的前提下实现全双工语音对话，并在基准上达到领先的轮换控制与对话智能。
...
### [150] [DEO: Training-Free Direct Embedding Optimization for Negation-Aware Retrieval](https://arxiv.org/abs/2603.09185)
...
### [153] [SPAR-K: Scheduled Periodic Alternating Early Exit for Spoken Language Models](https://arxiv.org/abs/2603.09215)
...
### [154] [LooComp: Leverage Leave-One-Out Strategy to Encoder-only Transformer for Efficient Query-aware Context Compression](https://arxiv.org/abs/2603.09222)
...
### [155] [TaSR-RAG: Taxonomy-guided Structured Reasoning for Retrieval-Augmented Generation](https://arxiv.org/abs/2603.09341)
*Jiashuo Sun,Yixuan Xie,Jimeng Shi,Shaowen Wang,Jiawei Han*

Main category: cs.CL

TL;DR: TaSR-RAG以taxonomy引导的三元组结构化检索与分步证据选择提升多跳问答表现，并改进证据归因与推理可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG多用非结构化检索与一次生成，导致冗余上下文、信息密度低与多跳推理脆弱；结构化RAG又常依赖昂贵且易错的图构建或过于僵化的实体中心结构。

Method: 将查询与文档表示为关系三元组，采用两级轻量taxonomy约束实体语义；将复杂问题分解为有序三元组子查询并引入显式潜变量；通过语义相似度与类型一致性结合的混合三元组匹配逐步选择证据，并维护实体绑定表以消解中间变量。

Result: 在多项多跳问答基准上相较强RAG与结构化RAG基线最高提升14%，证据归因更清晰、推理更忠实。

Conclusion: 提出TaSR-RAG并在多跳问答上取得显著提升，且无需显式图构建即可提供更清晰的证据归因与推理轨迹。
...
### [156] [Quantifying and extending the coverage of spatial categorization data sets](https://arxiv.org/abs/2603.09373)
...
### [157] [Reward Prediction with Factorized World States](https://arxiv.org/abs/2603.09400)
...
### [159] [Investigating Gender Stereotypes in Large Language Models via Social Determinants of Health](https://arxiv.org/abs/2603.09416)
...
### [160] [Common Sense vs. Morality: The Curious Case of Narrative Focus Bias in LLMs](https://arxiv.org/abs/2603.09434)
...
### [161] [Modelling the Diachronic Emergence of Phoneme Frequency Distributions](https://arxiv.org/abs/2603.09503)
...
### [162] [You Didn't Have to Say It like That: Subliminal Learning from Faithful Paraphrases](https://arxiv.org/abs/2603.09517)
...
### [163] [ALARM: Audio-Language Alignment for Reasoning Models](https://arxiv.org/abs/2603.09556)
...
### [164] [Build, Borrow, or Just Fine-Tune? A Political Scientist's Guide to Choosing NLP Models](https://arxiv.org/abs/2603.09595)
*Shreyas Meher*
...
### [165] [Surgical Repair of Collapsed Attention Heads in ALiBi Transformers](https://arxiv.org/abs/2603.09616)
*Palmer Schallon*
...
### [166] [Tracking Cancer Through Text: Longitudinal Extraction From Radiology Reports Using Open-Source Large Language Models](https://arxiv.org/abs/2603.09638)
...
### [167] [Understanding the Interplay between LLMs' Utilisation of Parametric and Contextual Knowledge: A keynote at ECIR 2025](https://arxiv.org/abs/2603.09654)
*Isabelle Augenstein*
...
### [168] [Automatic Cardiac Risk Management Classification using large-context Electronic Patients Health Records](https://arxiv.org/abs/2603.09685)
...
### [169] [Fusing Semantic, Lexical, and Domain Perspectives for Recipe Similarity Estimation](https://arxiv.org/abs/2603.09688)
...
### [170] [ESAinsTOD: A Unified End-to-End Schema-Aware Instruction-Tuning Framework for Task-Oriented Dialog Modeling](https://arxiv.org/abs/2603.09691)
...
### [171] [Evaluation of LLMs in retrieving food and nutritional context for RAG systems](https://arxiv.org/abs/2603.09704)
...
### [172] [RbtAct: Rebuttal as Supervision for Actionable Review Feedback Generation](https://arxiv.org/abs/2603.09723)
...
### [173] [Beyond Fine-Tuning: Robust Food Entity Linking under Ontology Drift with FoodOntoRAG](https://arxiv.org/abs/2603.09758)
...
### [174] [EPIC-EuroParl-UdS: Information-Theoretic Perspectives on Translation and Interpreting](https://arxiv.org/abs/2603.09785)
...
### [175] [One-Eval: An Agentic System for Automated and Traceable LLM Evaluation](https://arxiv.org/abs/2603.09821)
...
### [177] [N-gram-like Language Models Predict Reading Time Best](https://arxiv.org/abs/2603.09872)
...
### [178] [Do What I Say: A Spoken Prompt Dataset for Instruction-Following](https://arxiv.org/abs/2603.09881)
*Maike Züfle,Sara Papi,Fabian Retkowski,Szymon Mazurek,Marek Kasztelnik,Alexander Waibel,Luisa Bentivogli,Jan Niehues*

Main category: cs.CL

TL;DR: 提出DOWIS语音提示数据集并评测SLLM，发现文本提示普遍更强但语音输出任务中语音提示能追平，强调语音提示评测必要性。


<details>
  <summary>Details</summary>
Motivation: 现有SLLM评测多用文本提示，未反映真实语音交互场景，需更贴近口语指令的评测。

Method: 提出DOWIS多语种语音/文本提示数据集，与现有基准配对；覆盖9任务11语言、5种风格、每任务语种10种提示；用其评测多种SLLM并分析提示模态/风格/语言/任务的影响。

Result: 文本提示在多数任务与语种下优于语音提示；低资源与跨语差距更大；语音输出任务中语音提示表现接近文本。

Conclusion: 文本提示整体优于语音提示，尤其在低资源与跨语场景；但在语音输出任务中语音提示可缩小差距，因此评测需包含语音提示。
...
### [179] [Benchmarking Political Persuasion Risks Across Frontier Large Language Models](https://arxiv.org/abs/2603.09884)
...
### [180] [Thinking to Recall: How Reasoning Unlocks Parametric Knowledge in LLMs](https://arxiv.org/abs/2603.09906)
...
### [181] [Model Merging in the Era of Large Language Models: Methods, Applications, and Future Directions](https://arxiv.org/abs/2603.09938)
...
### [182] [CREATE: Testing LLMs for Associative Creativity](https://arxiv.org/abs/2603.09970)
...
### [183] [Beyond Relevance: On the Relationship Between Retrieval and RAG Information Coverage](https://arxiv.org/abs/2603.08819)
...
### [184] [Diagnosing and Repairing Citation Failures in Generative Engine Optimization](https://arxiv.org/abs/2603.09296)
...
### [185] [TA-Mem: Tool-Augmented Autonomous Memory Retrieval for LLM in Long-Term Conversational QA](https://arxiv.org/abs/2603.09297)
...
### [186] [MITRA: An AI Assistant for Knowledge Retrieval in Physics Collaborations](https://arxiv.org/abs/2603.09800)
...
### [187] [POLISH'ing the Sky: Wide-Field and High-Dynamic Range Interferometric Image Reconstruction with Application to Strong Lens Discovery](https://arxiv.org/abs/2603.09162)
...
### [188] [CycleULM: A unified label-free deep learning framework for ultrasound localisation microscopy](https://arxiv.org/abs/2603.09840)
...
### [189] [Age-Related Differences in the Perception of Eye-Gaze from a Social Robot](https://arxiv.org/abs/2603.08810)
...
### [190] [Scale-Plan: Scalable Language-Enabled Task Planning for Heterogeneous Multi-Robot Teams](https://arxiv.org/abs/2603.08814)
...
### [191] [HMR-1: Hierarchical Massage Robot with Vision-Language-Model for Embodied Healthcare](https://arxiv.org/abs/2603.08817)
...
### [193] [Predictive Control with Indirect Adaptive Laws for Payload Transportation by Quadrupedal Robots](https://arxiv.org/abs/2603.08831)
...
### [194] [SEP-NMPC: Safety Enhanced Passivity-Based Nonlinear Model Predictive Control for a UAV Slung Payload System](https://arxiv.org/abs/2603.08860)
...
### [195] [APPLV: Adaptive Planner Parameter Learning from Vision-Language-Action Model](https://arxiv.org/abs/2603.08862)
...
### [196] [Adaptive SINDy: Residual Force System Identification Based UAV Disturbance Rejection](https://arxiv.org/abs/2603.08863)
...
### [197] [Proprioceptive Safe Active Navigation and Exploration for Planetary Environments](https://arxiv.org/abs/2603.08905)
...
### [198] [Fly, Track, Land: Infrastructure-less Magnetic Localization for Heterogeneous UAV-UGV Teaming](https://arxiv.org/abs/2603.08926)
*Valerio Brunacci,Davide Plozza,Alessio De Angelis,Michele Magno,Tommaso Polonelli*

Main category: cs.RO

TL;DR: 提出磁感应无基建定位系统，UAV以20 Hz估计相对位姿并融合传感器，实现厘米级对移动UGV的跟踪与着陆。


<details>
  <summary>Details</summary>
Motivation: 在无外部基站或全球定位的环境中，实现UAV与UGV异构协作的短距高精度相对定位与自主对接。

Method: 在UAV上集成单接收线圈与嵌入式估计管线，实时求解UGV坐标系下的三维相对位姿（20 Hz），采用热启动求解器并与惯性与光流数据通过机载EKF融合。

Result: 静态/动态实验中实现约5 cm（静态3D RMSE）与7.2 cm（动态RMSE）的定位精度，并完成移动UGV上的自主跟踪与着陆。

Conclusion: 该工作证明了基于磁感应的无基础设施相对定位能够支持UAV对移动UGV的厘米级自主跟踪与精确着陆。
...
### [199] [Formation-Aware Adaptive Conformalized Perception for Safe Leader-Follower Multi-Robot Systems](https://arxiv.org/abs/2603.08958)
...
Abstract: Maintaining balance under external hand forces is critical for humanoid bimanual manipulation, where interaction forces propagate through the kinematic chain and constrain the feasible manipulation envelope. We propose \textbf{FAME}, a force-adaptive reinforcement learning framework that conditions a standing policy on a learned latent context encoding upper-body joint configuration and bimanual interaction forces. During training, we apply diverse, spherically sampled 3D forces on each hand to inject disturbances in simulation together with an upper-body pose curriculum, exposing the policy to manipulation-induced perturbations across continuously varying arm configurations. At deployment, interaction forces are estimated from the robot dynamics and fed to the same encoder, enabling online adaptation without wrist force/torque sensors. In simulation across five fixed arm configurations with randomized hand forces and commanded base heights, FAME improves mean standing success to 73.84%, compared to 51.40% for the curriculum-only baseline and 29.44% for the base policy. We further deploy the learned policy on a full-scale Unitree H12 humanoid and evaluate robustness in representative load-interaction scenarios, including asymmetric single-arm load and symmetric bimanual load. Code and videos are available on

</details>


### [201] [SurgCalib: Gaussian Splatting-Based Hand-Eye Calibration for Robot-Assisted Minimally Invasive Surgery](https://arxiv.org/abs/2603.08983)
*Zijian Wu,Shuojue Yang,Yu Chung Lee,Eitan Prisman,Yueming Jin,Septimiu E. Salcudean*

Main category: cs.RO

TL;DR: 提出无标记SurgCalib，利用高斯溅射可微渲染与RCM约束优化手眼标定，在dVRK基准上取得毫米级误差。


<details>
  <summary>Details</summary>
Motivation: 传统手眼标定依赖标记物且电缆驱动器械编码器受拉伸与回差影响，导致测量不准；手术室引入标记物也可能破坏无菌流程。

Method: 基于高斯溅射的可微渲染框架，先用原始运动学测量初始化器械位姿，再在RCM约束下进行两阶段优化精炼手眼变换。

Result: 在SurgPose上，左右器械2D尖端重投影误差分别为12.24px(2.06mm)与11.33px(1.9mm)，3D尖端欧氏误差为5.98mm与4.75mm。

Conclusion: 提出的SurgCalib可在无标记条件下实现达芬奇手眼标定，并在基准数据上取得较好误差表现，具有手术室应用潜力。
...
### [202] [Characterization, Analytical Planning, and Hybrid Force Control for the Inspire RH56DFX Hand](https://arxiv.org/abs/2603.08988)
...
### [204] [PlayWorld: Learning Robot World Models from Autonomous Play](https://arxiv.org/abs/2603.09030)
...
### [206] [Beyond Amplitude: Channel State Information Phase-Aware Deep Fusion for Robotic Activity Recognition](https://arxiv.org/abs/2603.09047)
...
### [207] [Cutting the Cord: System Architecture for Low-Cost, GPU-Accelerated Bimanual Mobile Manipulation](https://arxiv.org/abs/2603.09051)
...
### [208] [Quality over Quantity: Demonstration Curation via Influence Functions for Data-Centric Robot Learning](https://arxiv.org/abs/2603.09056)
...
### [209] [3D UAV Trajectory Estimation and Classification from Internet Videos via Language Model](https://arxiv.org/abs/2603.09070)
...
### [210] [High-Slip-Ratio Control for Peak Tire-Road Friction Estimation Using Automated Vehicles](https://arxiv.org/abs/2603.09073)
...
### [211] [Provably Safe Trajectory Generation for Manipulators Under Motion and Environmental Uncertainties](https://arxiv.org/abs/2603.09083)
...
### [212] [Latent World Models for Automated Driving: A Unified Taxonomy, Evaluation Framework, and Open Challenges](https://arxiv.org/abs/2603.09086)
...
### [213] [PM-Nav: Priori-Map Guided Embodied Navigation in Functional Buildings](https://arxiv.org/abs/2603.09113)
...
### [214] [DexHiL: A Human-in-the-Loop Framework for Vision-Language-Action Model Post-Training in Dexterous Manipulation](https://arxiv.org/abs/2603.09121)
...
### [216] [SPAN-Nav: Generalized Spatial Awareness for Versatile Vision-Language Navigation](https://arxiv.org/abs/2603.09163)
...
### [217] [ZeroWBC: Learning Natural Visuomotor Humanoid Control Directly from Human Egocentric Video](https://arxiv.org/abs/2603.09170)
...
### [218] [STONE Dataset: A Scalable Multi-Modal Surround-View 3D Traversability Dataset for Off-Road Robot Navigation](https://arxiv.org/abs/2603.09175)
...
### [220] [WESPR: Wind-adaptive Energy-Efficient Safe Perception & Planning for Robust Flight with Quadrotors](https://arxiv.org/abs/2603.09194)
...
### [221] [Embodied Human Simulation for Quantitative Design and Analysis of Interactive Robotics](https://arxiv.org/abs/2603.09218)
...
### [222] [TRIP-Bag: A Portable Teleoperation System for Plug-and-Play Robotic Arms and Leaders](https://arxiv.org/abs/2603.09226)
...
### [223] [MO-Playground: Massively Parallelized Multi-Objective Reinforcement Learning for Robotics](https://arxiv.org/abs/2603.09237)
...
### [224] [See, Plan, Rewind: Progress-Aware Vision-Language-Action Models for Robust Robotic Manipulation](https://arxiv.org/abs/2603.09292)
...
### [225] [CORAL: Scalable Multi-Task Robot Learning via LoRA Experts](https://arxiv.org/abs/2603.09298)
...
### [227] [Vision-Augmented On-Track System Identification for Autonomous Racing via Attention-Based Priors and Iterative Neural Correction](https://arxiv.org/abs/2603.09399)
...
### [228] [From Flow to One Step: Real-Time Multi-Modal Trajectory Policies via Implicit Maximum Likelihood Estimation-based Distribution Distillation](https://arxiv.org/abs/2603.09415)
...
### [229] [Stein Variational Ergodic Surface Coverage with SE(3) Constraints](https://arxiv.org/abs/2603.09458)
...
### [230] [SEA-Nav: Efficient Policy Learning for Safe and Agile Quadruped Navigation in Cluttered Environments](https://arxiv.org/abs/2603.09460)
...
### [231] [Receptogenesis in a Vascularized Robotic Embodiment](https://arxiv.org/abs/2603.09473)
...
### [232] [StyleVLA: Driving Style-Aware Vision Language Action Model for Autonomous Driving](https://arxiv.org/abs/2603.09482)
*Yuan Gao,Dengyuan Hua,Mattia Piccinini,Finn Rasmus Schäfer,Korbinian Moller,Lin Li,Johannes Betz*

Main category: cs.RO

TL;DR: StyleVLA通过物理约束与风格化训练生成可行且多风格驾驶轨迹，性能显著超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有VLA多生成通用避碰轨迹，缺乏风格化与物理可行性，且将轨迹视为离散token预测易导致不符合运动学的动作。

Method: 提出StyleVLA：在VLA中引入混合损失（运动学一致性约束+连续回归头），基于Qwen3-VL-4B训练，并构建包含多视角与多风格指令的大规模数据集。

Result: StyleVLA在BEV/FPV上的综合驾驶评分分别达0.55/0.51，显著优于Gemini-3-Pro的0.32/0.35。

Conclusion: 专用、物理约束的轻量VLA模型在特定驾驶任务上可超越闭源大模型。
...
### [233] [Beyond Short-Horizon: VQ-Memory for Robust Long-Horizon Manipulation in Non-Markovian Simulation Benchmarks](https://arxiv.org/abs/2603.09513)
...
### [235] [On the Cost of Evolving Task Specialization in Multi-Robot Systems](https://arxiv.org/abs/2603.09552)
...
### [236] [Trajectory Optimization for Self-Wrap-Aware Cable-Towed Planar Object Manipulation under Implicit Tension Constraints](https://arxiv.org/abs/2603.09557)
...
### [237] [ReTac-ACT: A State-Gated Vision-Tactile Fusion Transformer for Precision Assembly](https://arxiv.org/abs/2603.09565)
...
### [238] [SCDP: Learning Humanoid Locomotion from Partial Observations via Mixed-Observation Distillation](https://arxiv.org/abs/2603.09574)
...
### [239] [Towards Terrain-Aware Safe Locomotion for Quadrupedal Robots Using Proprioceptive Sensing](https://arxiv.org/abs/2603.09585)
...
### [240] [A Generalized Voronoi Graph based Coverage Control Approach for Non-Convex Environment](https://arxiv.org/abs/2603.09596)
...
### [241] [DRIFT: Dual-Representation Inter-Fusion Transformer for Automated Driving Perception with 4D Radar Point Clouds](https://arxiv.org/abs/2603.09695)
...
### [242] [Robotic Scene Cloning:Advancing Zero-Shot Robotic Scene Adaptation in Manipulation via Visual Prompt Editing](https://arxiv.org/abs/2603.09712)
...
### [243] [Let's Reward Step-by-Step: Step-Aware Contrastive Alignment for Vision-Language Navigation in Continuous Environments](https://arxiv.org/abs/2603.09740)
...
### [244] [Caterpillar-Inspired Spring-Based Compressive Continuum Robot for Bristle-based Exploration](https://arxiv.org/abs/2603.09745)
...
### [245] [MuxGel: Simultaneous Dual-Modal Visuo-Tactile Sensing via Spatially Multiplexing and Deep Reconstruction](https://arxiv.org/abs/2603.09761)
...
### [254] [Automated Tensor-Relational Decomposition for Large-Scale Sparse Tensor Computation](https://arxiv.org/abs/2603.08957)
...
### [255] [Unpacking Interpretability: Human-Centered Criteria for Optimal Combinatorial Solutions](https://arxiv.org/abs/2603.08856)
...
### [256] [AI Phenomenology for Understanding Human-AI Experiences Across Eras](https://arxiv.org/abs/2603.09020)
...
### [257] [A Text-Native Interface for Generative Video Authoring](https://arxiv.org/abs/2603.09072)
...
### [258] [Reading the Mood Behind Words: Integrating Prosody-Derived Emotional Context into Socially Responsive VR Agents](https://arxiv.org/abs/2603.09324)
...
### [259] [Understanding the Use of a Large Language Model-Powered Guide to Make Virtual Reality Accessible for Blind and Low Vision People](https://arxiv.org/abs/2603.09964)
...
### [260] [Prompt-Driven Color Accessibility Evaluation in Diffusion-based Image Generation Models](https://arxiv.org/abs/2603.09832)
...
### [261] [Differentiable Stochastic Traffic Dynamics: Physics-Informed Generative Modelling in Transportation](https://arxiv.org/abs/2603.09174)
*Wuping Xin*
...
### [262] [AI-Enabled Data-driven Intelligence for Spectrum Demand Estimation](https://arxiv.org/abs/2603.09916)
...
### [263] [Towards Flexible Spectrum Access: Data-Driven Insights into Spectrum Demand](https://arxiv.org/abs/2603.09942)
...
### [264] [Test-Driven AI Agent Definition (TDAD): Compiling Tool-Using Agents from Behavioral Specifications](https://arxiv.org/abs/2603.08806)
*Tzafrir Rehan*
...
### [266] [Automating Detection and Root-Cause Analysis of Flaky Tests in Quantum Software](https://arxiv.org/abs/2603.09029)
...
### [267] [Declarative Scenario-based Testing with RoadLogic](https://arxiv.org/abs/2603.09455)
...
### [268] [Large Language Model-Assisted Superconducting Qubit Experiments](https://arxiv.org/abs/2603.08801)
...
### [269] [Hindsight Credit Assignment for Long-Horizon LLM Agents](https://arxiv.org/abs/2603.08754)
...
### [270] [Generalized Reduction to the Isotropy for Flexible Equivariant Neural Fields](https://arxiv.org/abs/2603.08758)
...
### [271] [Multi-level meta-reinforcement learning with skill-based curriculum](https://arxiv.org/abs/2603.08773)
...
### [272] [Are Expressive Encoders Necessary for Discrete Graph Generation?](https://arxiv.org/abs/2603.08825)
...
TL;DR: 提出FSbuHD，以组合距离构建模糊等价关系并用元启发式优化求解，UCI实验显示优于多种基线。


<details>
  <summary>Details</summary>
Motivation: 传统模糊粗糙集在高维空间求交耗时耗存且易引入噪声，需改进以适应大数据特征选择。

Method: 通过计算对象间的组合距离构建新的模糊等价关系，将特征选择转化为可由元启发式算法求解的优化问题；提供正常与乐观两种模式。

Result: 在UCI标准数据集上测试，FSbuHD表现为高效且有效的特征选择方法。

Conclusion: FSbuHD在特征选择上效率与效果优于多种现有方法。
...
### [276] [Exclusive Self Attention](https://arxiv.org/abs/2603.09078)
*Shuangfei Zhai*
...
### [277] [ActiveUltraFeedback: Efficient Preference Data Generation using Active Learning](https://arxiv.org/abs/2603.09692)
...
TL;DR: 提出SLoD用热核扩散实现知识图谱连续缩放，并利用谱间隙自动发现抽象层级边界，在合成与WordNet上验证有效。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱缺乏连续分辨率控制机制，难以明确抽象层级边界与导航方式。

Method: 在Poincaré球上定义热核扩散的连续缩放算子；证明层级一致性与近似误差界；利用图拉普拉斯谱间隙检测尺度边界；在HSBM与WordNet上评估。

Result: HSBM上边界恢复ARI最高1.00，接近Kesten–Stigum阈值时性能平滑下降；WordNet名词层级检测与真实深度相关性τ=0.79。

Conclusion: SLoD通过热核扩散在超曲空间实现连续分辨率控制，并可由谱间隙自动发现抽象层级边界，在合成与真实知识图谱上有效。
...
### [279] [Mousse: Rectifying the Geometry of Muon with Curvature-Aware Preconditioning](https://arxiv.org/abs/2603.09697)
...
### [280] [MSSR: Memory-Aware Adaptive Replay for Continual LLM Fine-Tuning](https://arxiv.org/abs/2603.09892)
...
### [281] [SPREAD: Subspace Representation Distillation for Lifelong Imitation Learning](https://arxiv.org/abs/2603.08763)
*Kaushik Roy,Giovanni D'urso,Nicholas Lawrance,Brendan Tidd,Peyman Moghadam*

Main category: cs.LG

TL;DR: 提出SPREAD：SVD几何对齐+置信度蒸馏，显著改进LIL知识保留与性能。


<details>
  <summary>Details</summary>
Motivation: 现有L2特征匹配对噪声与高维变化敏感，难以保留任务内在流形，导致知识遗忘与迁移不稳定。

Method: 使用SVD在低秩子空间对齐策略表征以保持几何结构，并在蒸馏中对最可信的top-M动作样本施加KL散度损失。

Result: 在LIBERO基准上显著提升知识迁移、鲁棒性与泛化，减轻灾难性遗忘并取得最先进性能。

Conclusion: SPREAD通过低秩子空间对齐与置信度引导蒸馏，有效保持任务几何结构并缓解灾难性遗忘，在LIL上达到SOTA。
...
### [282] [SPAARS: Safer RL Policy Alignment through Abstract Exploration and Refined Exploitation of Action Space](https://arxiv.org/abs/2603.09378)
...
### [283] [Sim2Act: Robust Simulation-to-Decision Learning via Adversarial Calibration and Group-Relative Perturbation](https://arxiv.org/abs/2603.09053)
...
### [284] [Not All News Is Equal: Topic- and Event-Conditional Sentiment from Finetuned LLMs for Aluminum Price Forecasting](https://arxiv.org/abs/2603.09085)
...
### [285] [Causally Sufficient and Necessary Feature Expansion for Class-Incremental Learning](https://arxiv.org/abs/2603.09145)
...
### [286] [Wrong Code, Right Structure: Learning Netlist Representations from Imperfect LLM-Generated RTL](https://arxiv.org/abs/2603.09161)
*Siyang Cai,Cangyuan Li,Yinhe Han,Ying Wang*

Main category: cs.LG

TL;DR: 利用LLM生成RTL的结构性有效信息进行合成数据增强，打破网表表示学习的数据瓶颈。


<details>
  <summary>Details</summary>
Motivation: 真实设计受IP保护、标注昂贵导致标注网表稀缺；现有方法多在小规模、干净数据上训练，难以扩展。

Method: 提出端到端数据增强与训练框架：自动生成RTL→综合为网表→利用其结构模式进行表示学习，应用于子电路边界识别与组件分类等任务。

Result: 在不同规模基准、从算子级到IP级任务上，使用噪声合成语料训练的模型对真实网表具有良好泛化，效果匹配或超过高质量小数据训练的方法。

Conclusion: 利用LLM生成的有缺陷RTL合成网表作为训练数据可有效缓解电路表示学习的数据稀缺问题，并在真实网表上取得可比或更优效果。
...
### [287] [GIAT: A Geologically-Informed Attention Transformer for Lithology Identification](https://arxiv.org/abs/2603.09165)
...
### [291] [Variational Routing: A Scalable Bayesian Framework for Calibrated Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.09453)
*Albus Yizhuo Li,Matthew Wicker*

Main category: cs.LG

TL;DR: 在MoE路由阶段引入结构化贝叶斯的VMoER，以极小开销显著提升不确定性校准与分布外鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基础模型部署需要可靠的不确定性量化，但全贝叶斯方法在大规模下计算代价过高；MoE稀疏结构为在路由阶段引入结构化贝叶斯提供机会。

Method: 提出Variational Mixture-of-Experts Routing（VMoER），将贝叶斯推断限制在MoE的专家选择阶段；采用两种推断策略：对路由logits的摊销变分推断与对随机专家选择温度参数的推断。

Result: 在多种基础模型上，VMoER在噪声下路由稳定性提升38%，校准误差降低94%，分布外AUROC提升12%，额外FLOPs低于1%。

Conclusion: VMoER在基础模型规模下提供了可扩展、鲁棒且具不确定性感知的路径，并仅带来极小计算开销。
...
### [292] [Temporal-Conditioned Normalizing Flows for Multivariate Time Series Anomaly Detection](https://arxiv.org/abs/2603.09490)
...
### [293] [Efficiently Aligning Draft Models via Parameter- and Data-Efficient Adaptation](https://arxiv.org/abs/2603.09527)
...
### [295] [GNNs for Time Series Anomaly Detection: An Open-Source Framework and a Critical Evaluation](https://arxiv.org/abs/2603.09675)
...
### [296] [A Hybrid Quantum-Classical Framework for Financial Volatility Forecasting Based on Quantum Circuit Born Machines](https://arxiv.org/abs/2603.09789)
*Yixiong Chen*
...
### [297] [Exploiting Label-Aware Channel Scoring for Adaptive Channel Pruning in Split Learning](https://arxiv.org/abs/2603.09792)
...
### [298] [Correction of Transformer-Based Models with Smoothing Pseudo-Projector](https://arxiv.org/abs/2603.09815)
*Vitaly Bulgakov*

Main category: cs.LG

TL;DR: 轻量伪投影器通过可学习投影抑噪，借鉴多重网格思想，在多任务上提升训练与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 降低模型对噪声和与标签无关内容的敏感性，并以轻量方式集成到现有模型中。

Method: 提出伪投影器作为隐藏表示校正器，借鉴多重网格思想，使用可学习的限制与延拓算子以抑制与标签无关的噪声方向。

Result: 在Transformer文本分类与合成基准上，训练行为与鲁棒性稳定改善。

Conclusion: 伪投影器在不改动核心架构的情况下提升训练动态与鲁棒性，且未见负面影响，后续计划扩展到语言模型。
...
### [299] [A Graph-Based Approach to Spectrum Demand Prediction Using Hierarchical Attention Networks](https://arxiv.org/abs/2603.09859)
...
### [300] [When Learning Rates Go Wrong: Early Structural Signals in PPO Actor-Critic](https://arxiv.org/abs/2603.09950)
...
### [301] [The Coupling Within: Flow Matching via Distilled Normalizing Flows](https://arxiv.org/abs/2603.09014)
...
TL;DR: 用预训练NF蒸馏耦合来训练流模型，NFM显著提升生成质量并超越现有耦合策略。


<details>
  <summary>Details</summary>
Motivation: 现有FM训练多用独立耦合，尽管自适应耦合（如OT）改进效果，但计算直接自适应耦合成本高且有限；NF具备噪声与数据空间双射能力，可提供高质量耦合。

Method: 利用预训练自回归NF的可逆与最大似然特性，蒸馏其近似确定性的噪声-数据耦合来训练新的流匹配学生模型（NFM）。

Result: NFM在训练与推断上显著优于独立/OT耦合的流模型，并在生成效果上超过教师AR-NF。

Conclusion: 通过从预训练NF蒸馏耦合进行训练的NFM学生模型在生成质量上优于独立/OT耦合训练的流模型，并且还能超越教师AR-NF模型。
...
### [302] [Towards a Neural Debugger for Python](https://arxiv.org/abs/2603.09951)
...
### [303] [From Data Statistics to Feature Geometry: How Correlations Shape Superposition](https://arxiv.org/abs/2603.09972)
...
### [304] [Why Channel-Centric Models are not Enough to Predict End-to-End Performance in Private 5G: A Measurement Campaign and Case Study](https://arxiv.org/abs/2603.08865)
*Nils Jörgensen*
...
### [305] [Alignment Is the Disease: Censorship Visibility and Alignment Constraint Complexity as Determinants of Collective Pathology in Multi-Agent LLM Systems](https://arxiv.org/abs/2603.08723)
*Hiroki Fukui*
...
### [306] [Clear, Compelling Arguments: Rethinking the Foundations of Frontier AI Safety Cases](https://arxiv.org/abs/2603.08760)
...
### [307] [First Estimation of Model Parameters for Neutrino-Induced Nucleon Knockout Using Simulation-Based Inference](https://arxiv.org/abs/2603.09778)
*Karla Tame-Narvaez,Steven Gardiner,Aleksandra Ćiprijanović,Giuseppe Cerati*

Main category: hep-ph

TL;DR: 用SBI对GENIE调参与数据拟合，得到略优结果并可近似NuWro，显示ML调参潜力。


<details>
  <summary>Details</summary>
Motivation: 传统经验调参难以满足日益严格的精度需求，需探索机器学习的模型调参能力。

Method: 在GENIE调参配置上训练SBI，分别用调参截面与MicroBooNE实验数据进行推断，并尝试近似NuWro。

Result: SBI在调参截面上复现参数，面对实验数据时偏好略有不同参数且拟合更好；还能近似NuWro。

Conclusion: SBI可用于中微子相互作用模型调参，并能在实验数据上取得略优拟合，同时可近似另一模拟器。
...
### [308] [Efficient and robust control with spikes that constrain free energy](https://arxiv.org/abs/2603.09729)
...
### [309] [A Variational Latent Equilibrium for Learning in Cortex](https://arxiv.org/abs/2603.09600)
...
### [310] [Unveiling the Potential of Quantization with MXFP4: Strategies for Quantization Error Reduction](https://arxiv.org/abs/2603.08713)
...
### [311] [Design Conductor: An agent autonomously builds a 1.5 GHz Linux-capable RISC-V CPU](https://arxiv.org/abs/2603.08716)
*Verkor Team,Ravi Krishna,Suresh Krishna,David Chin*
...
### [312] [CktEvo: Repository-Level RTL Code Benchmark for Design Evolution](https://arxiv.org/abs/2603.08718)
...
### [313] [SiliconMind-V1: Multi-Agent Distillation and Debug-Reasoning Workflows for Verilog Code Generation](https://arxiv.org/abs/2603.08719)
...
### [314] [ALADIN: Accuracy-Latency-Aware Design-space Inference Analysis for Embedded AI Accelerators](https://arxiv.org/abs/2603.08722)
...
### [315] [PhD Thesis Summary: Methods for Reliability Assessment and Enhancement of Deep Neural Network Hardware Accelerators](https://arxiv.org/abs/2603.08724)
*Mahdi Taheri*
...
### [316] [Measurement-Free Ancilla Recycling via Blind Reset: A Cross-Platform Study on Superconducting and Trapped-Ion Processors](https://arxiv.org/abs/2603.08733)
*Sangkeum Lee*
...
### [317] [Sensitivity-Guided Framework for Pruned and Quantized Reservoir Computing Accelerators](https://arxiv.org/abs/2603.08737)
*Atousa Jafari,Mahdi Taheri,Hassan Ghasemzadeh Mohammadi,Christian Herglotz,Marco Platzner*

Main category: cs.AR

TL;DR: 提出量化+敏感度剪枝的RC压缩框架，实现精度保持下的显著硬件效率提升。


<details>
  <summary>Details</summary>
Motivation: 在Reservoir Computing中同时优化模型精度与硬件资源/功耗开销，需系统化探索量化与剪枝的权衡。

Method: 提出基于敏感度的剪枝机制与量化结合的压缩框架，系统探索量化位宽、剪枝率、准确率与硬件效率的权衡，并在多时间序列任务上评估。

Result: 在多个时间序列数据集上保持高准确率并降低资源与PDP；如MELBOEN中4-bit量化+15%剪枝使资源降1.2%、PDP降50.8%，且精度无明显下降。

Conclusion: 该框架在保持精度的同时显著提升硬件效率，适用于FPGA实现的Reservoir Computing模型。
...
### [318] [Diagnosing FP4 inference: a layer-wise and block-wise sensitivity analysis of NVFP4 and MXFP4](https://arxiv.org/abs/2603.08747)
...
### [319] [VeriInteresting: An Empirical Study of Model Prompt Interactions in Verilog Code Generation](https://arxiv.org/abs/2603.08715)
...
### [320] [Performance Analysis of Edge and In-Sensor AI Processors: A Comparative Review](https://arxiv.org/abs/2603.08725)
...
### [321] [Quantifying Uncertainty in AI Visibility: A Statistical Framework for Generative Search Measurement](https://arxiv.org/abs/2603.08924)
*Ronald Sielinski*
...
### [322] [CERES: A Probabilistic Early Warning System for Acute Food Insecurity](https://arxiv.org/abs/2603.09425)
...
### [323] [Association of Radiologic PPFE Change with Mortality in Lung Cancer Screening Cohorts](https://arxiv.org/abs/2603.09531)
...
### [324] [Benchmarking Federated Learning in Edge Computing Environments: A Systematic Review and Performance Evaluation](https://arxiv.org/abs/2603.08735)
*Sales Aribe Jr.,Gil Nicholas Cagande*

Main category: cs.DC

TL;DR: 综述并评测边缘联邦学习，SCAFFOLD精度与鲁棒性领先，FedAvg通信与能耗最优，仍有异质性与能耗等挑战。


<details>
  <summary>Details</summary>
Motivation: 在边缘计算中兼顾隐私、低时延与带宽效率，需要系统评估联邦学习方法以指导算法选择与系统设计。

Method: 系统性综述与性能评测；按优化策略、通信效率、隐私保护与系统架构四维度分类，并在MNIST、CIFAR-10、FEMNIST、Shakespeare等数据集上比较五种算法的多项指标。

Result: SCAFFOLD在准确率（0.90）与非IID鲁棒性最佳；FedAvg在通信与能耗效率方面最优。

Conclusion: 该综述指出边缘联邦学习仍面临数据异质性、能耗限制与可重复性问题，并提出未来研究议程以构建更鲁棒、可扩展的系统。
...
### [325] [Autonomous Edge-Deployed AI Agents for Electric Vehicle Charging Infrastructure Management](https://arxiv.org/abs/2603.08736)
*Mohammed Cherifi*
...
### [326] [Zipage: Maintain High Request Concurrency for LLM Reasoning through Compressed PagedAttention](https://arxiv.org/abs/2603.08743)
*Mengqi Liao,Lu Wang,Chaoyun Zhang,Bo Qiao,Si Qin,Qingwei Lin,Saravan Rajmohan,Dongmei Zhang,Huaiyu Wan*

Main category: cs.DC

TL;DR: 提出压缩分页注意力与 Zipage 引擎，在接近完整 KV 性能下实现显著加速与高并发。


<details>
  <summary>Details</summary>
Motivation: 解码阶段 KV cache 造成的内存瓶颈限制高并发服务，现有淘汰方法难以满足工业级需求。

Method: 将 token 级 KV cache 淘汰与 PagedAttention 结合，配套综合调度策略，支持前缀缓存与异步压缩，并据此实现 Zipage 推理引擎。

Result: 在大规模数学推理任务上，Zipage 约达到 Full KV 推理 95% 的性能，同时实现超过 2.1× 的速度提升。

Conclusion: 本文提出的 Compressed PagedAttention 及其推理引擎 Zipage 在保持接近完整 KV 推理性能的同时显著提升并发与速度，适用于高并发工业场景。
...
### [327] [FedLECC: Cluster- and Loss-Guided Client Selection for Federated Learning under Non-IID Data](https://arxiv.org/abs/2603.08911)
...
### [328] [NetDiffuser: Deceiving DNN-Based Network Attack Detection Systems with Diffusion-Generated Adversarial Traffic](https://arxiv.org/abs/2603.08901)
...
### [329] [CyberThreat-Eval: Can Large Language Models Automate Real-World Threat Research?](https://arxiv.org/abs/2603.09452)
...
### [331] [Robust Provably Secure Image Steganography via Latent Iterative Optimization](https://arxiv.org/abs/2603.09348)
...
### [332] [EDMFormer: Genre-Specific Self-Supervised Learning for Music Structure Segmentation](https://arxiv.org/abs/2603.08759)
...
### [333] [Fish Audio S2 Technical Report](https://arxiv.org/abs/2603.08823)
...
### [335] [How Contrastive Decoding Enhances Large Audio Language Models?](https://arxiv.org/abs/2603.09232)
...
### [336] [Gender Fairness in Audio Deepfake Detection: Performance and Disparity Analysis](https://arxiv.org/abs/2603.09007)
*Aishwarya Fursule,Shruti Kshirsagar,Anderson R. Avila*

Main category: cs.SD

TL;DR: 用多种公平性指标评估音频深度伪造检测，发现总体指标掩盖性别差异，需公平感知评估。


<details>
  <summary>Details</summary>
Motivation: 合成语音质量提升使深度伪造风险增加，而检测模型中的性别偏差问题尚未充分研究。

Method: 使用ASVspoof 5数据集训练ResNet-18分类器，比较四种音频特征并与AASIST基线对照；除EER外引入五种公平性指标评估性别差异。

Result: 总体EER性别差异较小，但公平性评估显示错误分布存在显著性别不均衡。

Conclusion: 研究表明仅依赖总体EER会掩盖性别差异，公平性指标能揭示错误分布不均，强调公平感知评估对构建更可信的检测系统至关重要。
...
### [337] [MUGEN: Evaluating and Improving Multi-audio Understanding of Large Audio-Language Models](https://arxiv.org/abs/2603.09714)
...
### [338] [TimberAgent: Gram-Guided Retrieval for Executable Music Effect Control](https://arxiv.org/abs/2603.09332)
...
### [339] [Physics-Informed Neural Engine Sound Modeling with Differentiable Pulse-Train Synthesis](https://arxiv.org/abs/2603.09391)
...
### [340] [SCENEBench: An Audio Understanding Benchmark Grounded in Assistive and Industrial Use Cases](https://arxiv.org/abs/2603.09853)
...
### [341] [Reinforced Generation of Combinatorial Structures: Ramsey Numbers](https://arxiv.org/abs/2603.09172)
...
### [342] [MASEval: Extending Multi-Agent Evaluation from Models to Systems](https://arxiv.org/abs/2603.08835)
...
### [344] [Quantifying the Accuracy and Cost Impact of Design Decisions in Budget-Constrained Agentic LLM Search](https://arxiv.org/abs/2603.08877)
...
### [345] [Interpretable Markov-Based Spatiotemporal Risk Surfaces for Missing-Child Search Planning with Reinforcement Learning and LLM-Based Quality Assurance](https://arxiv.org/abs/2603.08933)
...
### [346] [AgentOS: From Application Silos to a Natural Language-Driven Data Ecosystem](https://arxiv.org/abs/2603.08938)
...
### [347] [A Consensus-Driven Multi-LLM Pipeline for Missing-Person Investigations](https://arxiv.org/abs/2603.08954)
*Joshua Castillo,Ravi Mukkamala*
...
### [348] [The FABRIC Strategy for Verifying Neural Feedback Systems](https://arxiv.org/abs/2603.08964)
...
### [349] [Meissa: Multi-modal Medical Agentic Intelligence](https://arxiv.org/abs/2603.09018)
...
### [351] [EPOCH: An Agentic Protocol for Multi-Round System Optimization](https://arxiv.org/abs/2603.09049)
...
### [352] [From Days to Minutes: An Autonomous AI Agent Achieves Reliable Clinical Triage in Remote Patient Monitoring](https://arxiv.org/abs/2603.09052)
...
### [353] [Chaotic Dynamics in Multi-LLM Deliberation](https://arxiv.org/abs/2603.09127)
...
### [354] [Deep Tabular Research via Continual Experience-Driven Execution](https://arxiv.org/abs/2603.09151)
...
### [355] [DataFactory: Collaborative Multi-Agent Framework for Advanced Table Question Answering](https://arxiv.org/abs/2603.09152)
...
### [356] [Real-Time Trust Verification for Safe Agentic Actions using TrustBench](https://arxiv.org/abs/2603.09157)
...
### [357] [Explainable Innovation Engine: Dual-Tree Agent-RAG with Methods-as-Nodes and Verifiable Write-Back](https://arxiv.org/abs/2603.09192)
*Renwei Meng*
...
### [359] [Evaluate-as-Action: Self-Evaluated Process Rewards for Retrieval-Augmented Agents](https://arxiv.org/abs/2603.09203)
...
### [360] [Abundant Intelligence and Deficient Demand: A Macro-Financial Stress Test of Rapid AI Adoption](https://arxiv.org/abs/2603.09209)
*Xupeng Chen*
...
### [361] [Cognitively Layered Data Synthesis for Domain Adaptation of LLMs to Space Situational Awareness](https://arxiv.org/abs/2603.09231)
...
### [362] [Social-R1: Towards Human-like Social Reasoning in LLMs](https://arxiv.org/abs/2603.09249)
*Jincenzi Wu,Yuxuan Lei,Jianxun Lian,Yitian Huang,Lexin Zhou,Haotian Li,Xing Xie,Helen Meng*

Main category: cs.AI

TL;DR: 提出ToMBench-Hard与Social-R1，用过程级多维奖励对齐社会推理，使小模型在多基准上超越大模型。


<details>
  <summary>Details</summary>
Motivation: 现有大模型社会智能不足且易依赖表层模式，需训练能抵御捷径解的困难样例以逼近人类式社会推理。

Method: 提出对抗性基准ToMBench-Hard并构建Social-R1强化学习框架，采用多维奖励监督推理全过程，强调结构一致性、逻辑完整性与信息密度。

Result: 4B参数模型在八个基准上超过更大模型并实现稳健泛化。

Conclusion: 通过困难案例与轨迹级对齐可有效提升模型社会智能，且小模型可超过大模型并具良好泛化。
...
### [363] [Logos: An evolvable reasoning engine for rational molecular design](https://arxiv.org/abs/2603.09268)
...
### [365] [Curveball Steering: The Right Direction To Steer Isn't Always Linear](https://arxiv.org/abs/2603.09313)
...
### [366] [Robust Regularized Policy Iteration under Transition Uncertainty](https://arxiv.org/abs/2603.09344)
...
### [367] [AI Act Evaluation Benchmark: An Open, Transparent, and Reproducible Evaluation Dataset for NLP and RAG Systems](https://arxiv.org/abs/2603.09435)
...
### [368] [An Empirical Study and Theoretical Explanation on Task-Level Model-Merging Collapse](https://arxiv.org/abs/2603.09463)
...
### [369] [Telogenesis: Goal Is All U Need](https://arxiv.org/abs/2603.09476)
...
### [370] [GenePlan: Evolving Better Generalized PDDL Plans using Large Language Models](https://arxiv.org/abs/2603.09481)
*Andrew Murray,Danial Dervovic,Alberto Pozanco,Michael Cashmore*

Main category: cs.AI

TL;DR: GenePlan 以LLM+进化算法生成可解释通用规划器，性能接近SOTA、显著优于LLM基线且高效低成本。


<details>
  <summary>Details</summary>
Motivation: 解决经典PDDL规划中通用规划器的自动生成问题，追求高性能、可解释且具备跨实例泛化能力的方法。

Method: 将通用规划视为优化问题，采用LLM辅助的进化算法迭代生成并改进可解释的Python规划器，以最小化多实例的计划长度。

Result: 在6个基准域和2个新域上平均SAT 0.91（接近SOTA 0.93），显著优于CoT等LLM基线（0.64）；新实例求解快（均0.49秒/任务）且成本低（每域约$1.82，GPT-4o）。

Conclusion: GenePlan 能在多领域上生成可解释的通用规划器，性能接近SOTA且优于其他LLM基线。
...
### [371] [Vibe-Creation: The Epistemology of Human-AI Emergent Cognition](https://arxiv.org/abs/2603.09486)
*Ilya Levin*
...
### [372] [Enhancing Debunking Effectiveness through LLM-based Personality Adaptation](https://arxiv.org/abs/2603.09533)
...
### [373] [Context Engineering: From Prompts to Corporate Multi-Agent Architecture](https://arxiv.org/abs/2603.09619)
*Vera V. Vishnyakova*
...
### [374] [PRECEPT: Planning Resilience via Experience, Context Engineering & Probing Trajectories A Unified Framework for Test-Time Adaptation with Compositional Rule Learning and Pareto-Guided Prompt Evolution](https://arxiv.org/abs/2603.09641)
*Arash Shahmansoori*
...
### [375] [MiniAppBench: Evaluating the Shift from Text to Interactive HTML Responses in LLM-Powered Assistants](https://arxiv.org/abs/2603.09652)
...
### [378] [Does the Question Really Matter? Training-Free Data Selection for Vision-Language SFT](https://arxiv.org/abs/2603.09715)
...
### [379] [AutoAgent: Evolving Cognition and Elastic Memory Orchestration for Adaptive Agents](https://arxiv.org/abs/2603.09716)
...
### [380] [World2Mind: Cognition Toolkit for Allocentric Spatial Reasoning in Foundation Models](https://arxiv.org/abs/2603.09774)
...
### [381] [Quantifying the Necessity of Chain of Thought through Opaque Serial Depth](https://arxiv.org/abs/2603.09786)
...
### [382] [LCA: Local Classifier Alignment for Continual Learning](https://arxiv.org/abs/2603.09888)
...
### [383] [Influencing LLM Multi-Agent Dialogue via Policy-Parameterized Prompts](https://arxiv.org/abs/2603.09890)
...
### [384] [MedMASLab: A Unified Orchestration Framework for Benchmarking Multimodal Medical Multi-Agent Systems](https://arxiv.org/abs/2603.09909)
...
### [386] [The Confidence Gate Theorem: When Should Ranked Decision Systems Abstain?](https://arxiv.org/abs/2603.09947)
*Ronald Doku*
...
### [387] [Think Before You Lie: How Reasoning Improves Honesty](https://arxiv.org/abs/2603.09957)
...
### [388] [From Word2Vec to Transformers: Text-Derived Composition Embeddings for Filtering Combinatorial Electrocatalysts](https://arxiv.org/abs/2603.08881)
