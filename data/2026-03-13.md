...
TL;DR: 通过表面光场子样本编码构建统一的3D潜空间并用潜流匹配条件生成，实现视角相关外观与几何的高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法多只重建几何或视角无关外观，难以刻画真实的视角相关效应。

Method: 提出联合建模几何与视角相关外观的3D潜表示；将RGB-D采样为表面光场随机子样本并编码为潜向量；训练潜流匹配模型以单图像为条件生成该潜表示。

Result: 能够复现镜面高光与菲涅尔反射等视角相关效果，并在实验中取得更高视觉质量与更好输入保真度。

Conclusion: 该方法在视觉质量与输入一致性方面优于现有方法，能生成与输入光照和材质一致的3D对象。
...
TL;DR: 用VLM内在推理+GRPO强化与推理轨迹校准，实现开放实例视频分类提升。


<details>
  <summary>Details</summary>
Motivation: 开放实例视频分类中类内差异巨大，传统视频编码器泛化不足，VLM具泛化但推理能力未充分利用。

Method: 提出DeepIntuit：冷启动监督对齐→GRPO强化学习提升推理一致性→基于推理轨迹的直觉校准分类器训练。

Result: 实验表明DeepIntuit显著提升开放实例视频分类性能。

Conclusion: 该框架通过内在推理与校准提升开放实例视频分类表现，优于传统特征模仿。
...
TL;DR: 用视觉token的双重校准机制在潜空间协同增强视觉、纠正偏置，降低幻觉并小幅增加时延。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅增强视觉或抑制语言，存在互相权衡且简单叠加无效，需要统一框架解决视觉-语言失衡与幻觉。

Method: 提出以视觉token为核心的训练无关框架：SVC用增强图像token强化视觉表征；CRC通过剪除视觉token生成潜在负样本以校正内部偏置，并在潜空间进行校准。

Result: 在LLaVA-1.5上POPE准确率平均提升2%（绝对值），推理时延约1.06倍。

Conclusion: 该统一框架通过协同视觉校准与因果表示校准平衡视觉与语言，显著降低幻觉并带来小额时延开销。
...
TL;DR: DSFlash通过高效架构实现高速、全面的场景图生成，兼顾性能与低资源需求。


<details>
  <summary>Details</summary>
Motivation: 现有SGG研究较少关注速度与资源效率，限制了在边缘设备和真实应用中的部署。

Method: 提出低延迟的全景场景图生成模型DSFlash，面向视频流高帧率处理并生成全面场景图。

Result: 在RTX 3090上达56 FPS，性能不逊于SOTA；单张老旧GTX1080上训练<24小时。

Conclusion: DSFlash在保持性能的同时实现低延迟与资源友好，适合实际部署与有限算力环境。
...
TL;DR: 单步晚融合的Marigold-SSD用扩散先验但免测试时优化，推理更快、训练成本低，在多基准上表现强且具零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式深度补全方法测试时优化开销大、延迟高，难以满足现实场景3D感知需求。

Method: 提出单步、晚融合的深度补全框架，将扩散先验用于微调阶段，避免测试时优化；通过将计算负担从推理转移到微调实现高效推理。

Result: 在4个室内和2个室外基准上取得强跨域泛化与零样本表现；推理显著加速，训练成本约4.5 GPU天；缩小扩散与判别式模型的效率差距，并分析不同稀疏度下性能。

Conclusion: Marigold-SSD在保持扩散先验优势的同时显著提升推理效率，并在多基准上表现出强泛化与零样本能力。
...
TL;DR: JiT通过空间域稀疏计算与微流扩展，实现扩散Transformer采样约7×加速且几乎不损质量。


<details>
  <summary>Details</summary>
Motivation: 现有加速多聚焦时间域，忽略生成过程中空间冗余与全局结构先成形的特性，导致对所有空间区域一视同仁的计算低效。

Method: 构建空间近似生成ODE，仅在动态选取的稀疏锚点token上计算，并用确定性微流在扩展潜在维度时保证连续性与统计正确性。

Result: 在FLUX.1-dev上实现最高约7×加速，性能几乎无损，优于现有加速方法并带来更优速度-质量折中。

Conclusion: 提出JiT作为训练无关的空间域加速框架，通过稀疏锚点与微流机制在保持质量的同时显著加速扩散Transformer采样。
...
TL;DR: 基于NTK二次型视角提出SGA，显式减轻梯度冲突，在DiT/U-Net上加速收敛并提升生成结构质量。


<details>
  <summary>Details</summary>
Motivation: 标准训练对跨样本相关的交叉项缺乏显式控制且数据同质性假设限制容量，需更精细地对齐语义粒度以降低干扰。

Method: 在Flow Matching下将MSE写成由动态NTK控制的二次型，提出SGA在向量残差场中进行针对性干预以缓解梯度冲突，并在DiT与U-Net上评估。

Result: SGA加速收敛并改善结构完整性，在文本到图像任务上表现更好。

Conclusion: SGA在不同架构上提升了收敛速度与结构质量，实现更优的效率-质量权衡。
...
TL;DR: 针对cue-conflict评估缺陷，提出REFINED-BIAS，用平衡线索对和排名度量在全标签空间可靠测量形状/纹理偏置。


<details>
  <summary>Details</summary>
Motivation: 现有基于风格化的cue-conflict评估不稳定且含糊：线索不可分、信息量不受控、比率偏置遮蔽绝对敏感性、仅评估预选类别导致失真。

Method: 构建REFINED-BIAS数据集与评测框架，使用显式形状/纹理定义、平衡可识别线索对，并以全标签空间的排名度量评估线索敏感性。

Result: 在多种训练方案与架构上实现更公平的跨模型比较、更可信的形状/纹理偏置诊断与更清晰结论。

Conclusion: 提出REFINED-BIAS可更可靠诊断形状-纹理偏好，化解以往评估不一致。
...
TL;DR: 提出与临床流程对齐的层级多智能体超声诊断框架，通过渐进训练提升可解释性与性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端或弱证据方法缺乏细粒度病灶线索与可审计性，难以符合临床流程。

Method: 层级多智能体框架：主智能体定位并裁剪，子智能体预测四类属性；主智能体基于属性推理BI-RADS与良恶性。训练采用解耦渐进策略与自蒸馏。

Result: 在诊断准确率与属性一致性上优于强视觉语言基线，并提供结构化可追踪证据。

Conclusion: 提出的UltrasoundAgents与渐进式训练策略能提供可追溯证据并提升诊断与属性一致性。
...
TL;DR: OC-RPAv2用单例+批处理将口腔癌检测RPA推理速度提升至0.06秒/张，较常规实现快60–100倍。


<details>
  <summary>Details</summary>
Motivation: 实现口腔癌病灶的快速、准确检测以支持早期诊断与治疗。

Method: 比较两种RPA实现（OC-RPAv1与OC-RPAv2），在31张图像测试集上测量单张与批处理预测耗时。

Result: OC-RPAv1平均0.29秒/张；OC-RPAv2通过单例与批处理降至0.06秒/张，效率提升约60–100倍。

Conclusion: 采用设计模式与批处理可显著提升口腔癌检测RPA的可扩展性并降低成本。
...
### [90] [COMIC: Agentic Sketch Comedy Generation](https://arxiv.org/abs/2603.11048)
...
### [91] [GhazalBench: Usage-Grounded Evaluation of LLMs on Persian Ghazals](https://arxiv.org/abs/2603.09979)
...
### [93] [AraModernBERT: Transtokenized Initialization and Long-Context Encoder Modeling for Arabic](https://arxiv.org/abs/2603.09982)
...
### [94] [An Efficient Hybrid Deep Learning Approach for Detecting Online Abusive Language](https://arxiv.org/abs/2603.09984)
...
### [95] [The Dunning-Kruger Effect in Large Language Models: An Empirical Study of Confidence Calibration](https://arxiv.org/abs/2603.09985)
...
### [96] [Quantifying Hallucinations in Language Language Models on Medical Textbooks](https://arxiv.org/abs/2603.09986)
*Brandon C. Colelough,Davis Bartels,Dina Demner-Fushman*

Main category: cs.CL

TL;DR: 教材证据约束下医学QA仍有约20%幻觉；跨模型幻觉率与有用性负相关，临床评审一致性中高。


<details>
  <summary>Details</summary>
Motivation: 现有医学QA基准很少在固定证据源上评估幻觉，需量化幻觉发生频率及模型间差异。

Method: 两项实验：实验一评估LLaMA-70B-Instruct在新提示下的教材证据约束医学QA幻觉率；实验二比较多模型幻觉率并评估临床医生偏好与评分。

Result: 实验一中LLaMA-70B-Instruct幻觉率19.7%(95%CI 18.6–20.7)，尽管98.8%回答被评为最高可置信度；实验二中幻觉率越低有用性越高（ρ=-0.71, p=0.058）。临床一致性：实验一κ=0.92；实验二τ_b=0.06–0.18、κ=0.57–0.61。

Conclusion: 该研究显示在基于教材证据的医学问答中，大模型仍存在显著幻觉，且幻觉率与有用性评分呈负相关；临床评审一致性较高。
...
### [97] [Evolving Demonstration Optimization for Chain-of-Thought Feature Transformation](https://arxiv.org/abs/2603.09987)
*Xinyuan Wang,Kunpeng Liu,Arun Vignesh Malarkkan,Yanjie Fu*

Main category: cs.CL

TL;DR: 用闭环进化的上下文与经验库引导LLM生成特征变换，提高多样性与下游性能，效果优于基线且更稳定。


<details>
  <summary>Details</summary>
Motivation: 现有特征变换方法受限于样本效率低、无效候选与冗余生成，且LLM方法依赖静态示例导致多样性不足与目标对齐弱。

Method: 从强化学习探索的高性能特征变换序列出发，构建并持续更新经下游任务验证的变换轨迹经验库；采用多样性选择器组装上下文，结合链式思维引导LLM生成更优特征变换，形成闭环优化。

Result: 在多样化表格基准上取得更高性能与更稳定表现，相比一次性生成更可靠；对API与开源LLM及不同下游评估器均具鲁棒性。

Conclusion: 提出一种通过闭环演化上下文数据的LLM特征变换框架，在多种表格基准上优于传统与LLM基线且更稳定，并能跨模型与评估器泛化。
...
### [98] [Causally Grounded Mechanistic Interpretability for LLMs with Faithful Natural-Language Explanations](https://arxiv.org/abs/2603.09988)
*Ajay Pravin Mahale*

Main category: cs.CL

TL;DR: 提出电路到语言解释的管线；在IOI上验证，解释充分但不完备，LLM解释更优且置信度与忠实度无关。


<details>
  <summary>Details</summary>
Motivation: 弥合机制可解释性中的电路发现与人可理解解释之间的鸿沟。

Method: 通过激活修补识别关键注意力头，结合模板与LLM生成解释，并用改造的ERASER指标评估电路级归因的忠实性。

Result: 在GPT-2 Small的IOI任务上发现6个注意力头贡献61.4% logit差；解释充分性100%、完备性22%；LLM解释质量较模板提升64%；识别三类失败模式。

Conclusion: 该管线能将电路级发现转化为语言解释，但解释的完备性受限，存在分布式备份机制，且模型置信度与解释忠实度无关。
...
### [99] [The System Hallucination Scale (SHS): A Minimal yet Effective Human-Centered Instrument for Evaluating Hallucination-Related Behavior in Large Language Models](https://arxiv.org/abs/2603.09989)
...
### [100] [A Two-Stage Architecture for NDA Analysis: LLM-based Segmentation and Transformer-based Clause Classification](https://arxiv.org/abs/2603.09990)
*Ana Begnini,Matheus Vicente,Leonardo Souza*

Main category: cs.CL

TL;DR: 提出LLM架构自动分割并分类NDA条款，取得高分割与良好分类性能。


<details>
  <summary>Details</summary>
Motivation: NDA格式与写作差异大，人工分析慢且易错。

Method: 使用LLaMA-3.1-8B-Instruct进行条款分割，Legal-Roberta-Large微调进行条款分类。

Result: 分割任务ROUGE F1=0.95±0.0036，分类任务加权F1=0.85。

Conclusion: 基于LLM的NDA自动分割与条款分类架构可行且精度高。
...
### [101] [PoultryLeX-Net: Domain-Adaptive Dual-Stream Transformer Architecture for Large-Scale Poultry Stakeholder Modeling](https://arxiv.org/abs/2603.09991)
...
### [102] [TAMUSA-Chat: A Domain-Adapted Large Language Model Conversational System for Research and Responsible Deployment](https://arxiv.org/abs/2603.09992)
...
### [104] [Evaluating Adjective-Noun Compositionality in LLMs: Functional vs Representational Perspectives](https://arxiv.org/abs/2603.09994)
...
### [105] [Context Over Compute Human-in-the-Loop Outperforms Iterative Chain-of-Thought Prompting in Interview Answer Quality](https://arxiv.org/abs/2603.09995)
...
### [106] [There Are No Silly Questions: Evaluation of Offline LLM Capabilities from a Turkish Perspective](https://arxiv.org/abs/2603.09996)
...
### [107] [Empathy Is Not What Changed: Clinical Assessment of Psychological Safety Across GPT Model Generations](https://arxiv.org/abs/2603.09997)
...
### [108] [Automated evaluation of LLMs for effective machine translation of Mandarin Chinese to English](https://arxiv.org/abs/2603.09998)
...
### [109] [A Retrieval-Augmented Language Assistant for Unmanned Aircraft Safety Assessment and Regulatory Compliance](https://arxiv.org/abs/2603.09999)
...
### [110] [Beyond the Prompt in Large Language Models: Comprehension, In-Context Learning, and Chain-of-Thought](https://arxiv.org/abs/2603.10000)
*Yuling Jiao,Yanming Lai,Huazhen Lin,Wensen Ma,Houduo Qi,Defeng Sun*

Main category: cs.CL

TL;DR: 提出理论框架解释LLM的提示理解、ICL增益与CoT分解能力，并证明其统计优势。


<details>
  <summary>Details</summary>
Motivation: 解释LLM在提示理解、ICL与CoT中的经验性成功背后的理论机制。

Method: 通过分析自回归过程的转移概率推断，并对ICL与CoT机制进行理论建模与误差界比较。

Result: LLM可由提示精确推断跨任务的转移概率；ICL通过减少提示歧义使后验集中于目标任务；CoT激活任务分解能力。

Conclusion: 本文从理论上解释了LLM在提示语义理解、ICL与CoT上的能力来源，指出先进提示工程在统计上具有更优的误差界。
...
### [111] [Leveraging Wikidata for Geographically Informed Sociocultural Bias Dataset Creation: Application to Latin America](https://arxiv.org/abs/2603.10001)
...
### [112] [SpreadsheetArena: Decomposing Preference in LLM Generation of Spreadsheet Workbooks](https://arxiv.org/abs/2603.10002)
...
### [113] [Probing the Limits of the Lie Detector Approach to LLM Deception](https://arxiv.org/abs/2603.10003)
*Tom-Felix Berger*
...
### [114] [Fine-Tune, Don't Prompt, Your Language Model to Identify Biased Language in Clinical Notes](https://arxiv.org/abs/2603.10004)
...
### [115] [SENS-ASR: Semantic Embedding injection in Neural-transducer for Streaming Automatic Speech Recognition](https://arxiv.org/abs/2603.10005)
...
### [116] [Adaptive Engram Memory System for Indonesian Language Model: Generative AI Based on TOBA LM for Batak and Minang Language](https://arxiv.org/abs/2603.10006)
...
### [117] [GATech at AbjadGenEval Shared Task: Multilingual Embeddings for Arabic Machine-Generated Text Classification](https://arxiv.org/abs/2603.10007)
*Ahmed Khaled Khamis*
...
### [118] [GATech at AbjadMed: Bidirectional Encoders vs. Causal Decoders: Insights from 82-Class Arabic Medical Classification](https://arxiv.org/abs/2603.10008)
*Ahmed Khaled Khamis*
...
### [119] [FERRET: Framework for Expansion Reliant Red Teaming](https://arxiv.org/abs/2603.10010)
...
### [120] [Gemma Needs Help: Investigating and Mitigating Emotional Instability in LLMs](https://arxiv.org/abs/2603.10011)
...
### [121] [Measuring and Eliminating Refusals in Military Large Language Models](https://arxiv.org/abs/2603.10012)
...
### [122] [Evaluating Progress in Graph Foundation Models: A Comprehensive Benchmark and New Insights](https://arxiv.org/abs/2603.10033)
...
### [124] [TriageSim: A Conversational Emergency Triage Simulation Framework from Structured Electronic Health Records](https://arxiv.org/abs/2603.10035)
...
### [125] [The Prediction-Measurement Gap: Toward Meaning Representations as Scientific Instruments](https://arxiv.org/abs/2603.10130)
*Hubert Plisiecki*

Main category: cs.CL

TL;DR: 为科学测量优化语义表征需强调可解释几何、抗混淆与可追溯证据；静态嵌入更透明，上下文嵌入更丰富但更难测量，未来应推进几何优先设计与测量评估。


<details>
  <summary>Details</summary>
Motivation: 现有表示学习多为预测/检索优化，导致“预测—测量鸿沟”，难以作为科学测量工具。

Method: 基于认知与神经心理学的意义观，系统评估静态词向量与上下文Transformer表征在可解释性、几何可读性、抗混淆与回归推断兼容性上的表现，并提出几何优先设计、可逆后处理与测量评估体系。

Result: 静态嵌入在透明测量上仍具优势；上下文嵌入语义更丰富但意义与非语义信号纠缠，几何与可解释性问题增加推断难度。

Conclusion: 论文主张以“科学可用性”为核心构建可测量的语义表征，并提出面向测量的新研究议程。
...
### [126] [The Generation-Recognition Asymmetry: Six Dimensions of a Fundamental Divide in Formal Language Theory](https://arxiv.org/abs/2603.10139)
*Romain Peyrichou*
...
### [127] [Reason and Verify: A Framework for Faithful Retrieval-Augmented Generation](https://arxiv.org/abs/2603.10143)
...
### [128] [Lost in Backpropagation: The LM Head is a Gradient Bottleneck](https://arxiv.org/abs/2603.10145)
...
### [129] [OpenClaw-RL: Train Any Agent Simply by Talking](https://arxiv.org/abs/2603.10165)
...
### [130] [Adaptive Activation Cancellation for Hallucination Mitigation in Large Language Models](https://arxiv.org/abs/2603.10195)
...
### [131] [ViDia2Std: A Parallel Corpus and Methods for Low-Resource Vietnamese Dialect-to-Standard Translation](https://arxiv.org/abs/2603.10211)
...
### [132] [Sabiá-4 Technical Report](https://arxiv.org/abs/2603.10213)
...
### [133] [S-GRADES -- Studying Generalization of Student Response Assessments in Diverse Evaluative Settings](https://arxiv.org/abs/2603.10233)
*Tasfia Seuti,Sagnik Ray Choudhury*

Main category: cs.CL

TL;DR: 提出S-GRADES统一AES/ASAG评测并验证其揭示模型泛化差距的能力。


<details>
  <summary>Details</summary>
Motivation: AES与ASAG长期割裂、数据与指标分散，缺乏统一可复现的评测框架。

Method: 构建S-GRADES基准，整合14个数据集并统一评测协议；用多种提示推理策略评测三种大语言模型，并分析示例选择与跨数据集迁移。

Result: 基准驱动的评测展示了不同任务与数据集上的性能差异及泛化不足，示例选择与迁移显著影响表现。

Conclusion: 标准化的跨范式基准有助于揭示作文与简答评分模型的可靠性与泛化差距。
...
### [134] [Is this Idea Novel? An Automated Benchmark for Judgment of Research Ideas](https://arxiv.org/abs/2603.10303)
...
### [136] [Mitigating Translationese Bias in Multilingual LLM-as-a-Judge via Disentangled Information Bottleneck](https://arxiv.org/abs/2603.10351)
...
### [138] [Aligning Large Language Models with Searcher Preferences](https://arxiv.org/abs/2603.10473)
...
### [139] [Learning to Negotiate: Multi-Agent Deliberation for Collective Value Alignment in LLMs](https://arxiv.org/abs/2603.10476)
...
### [140] [PEEM: Prompt Engineering Evaluation Metrics for Interpretable Joint Evaluation of Prompts and Responses](https://arxiv.org/abs/2603.10477)
...
### [141] [Human-AI Co-reasoning for Clinical Diagnosis with Evidence-Integrated Language Agent](https://arxiv.org/abs/2603.10492)
...
### [142] [VERI-DPO: Evidence-Aware Alignment for Clinical Summarization via Claim Verification and Direct Preference Optimization](https://arxiv.org/abs/2603.10494)
...
### [143] [Safe and Scalable Web Agent Learning via Recreated Websites](https://arxiv.org/abs/2603.10505)
...
Abstract: Training autonomous web agents is fundamentally limited by the environments they learn from: real-world websites are unsafe to explore, hard to reset, and rarely provide verifiable feedback. We propose VeriEnv, a framework that treats language models as environment creators, automatically cloning real-world websites into fully executable, verifiable synthetic environments. By exposing controlled internal access via a Python SDK, VeriEnv enables agents to self-generate tasks with deterministic, programmatically verifiable rewards, eliminating reliance on heuristic or LLM-based judges. This design decouples agent learning from unsafe real-world interaction while enabling scalable self-evolution through environment expansion. Through experiments on web agent benchmarks, we show that agents trained with VeriEnv generalize to unseen websites, achieve site-specific mastery through self-evolving training, and benefit from scaling the number of training environments. Code and resources will be released at upon acceptance.

</details>


### [144] [AILS-NTUA at SemEval-2026 Task 8: Evaluating Multi-Turn RAG Conversations](https://arxiv.org/abs/2603.10524)
*Dimosthenis Athanasiou,Maria Lymperaiou,Giorgos Filandrianos,Athanasios Voulodimos,Giorgos Stamou*

Main category: cs.CL

TL;DR: 通过查询多样化+多阶段生成的统一RAG系统，在SemEval-2026 Task 8中检索与生成名列前茅，发现端到端瓶颈在可回答性校准。


<details>
  <summary>Details</summary>
Motivation: 在多轮RAG评测中同时优化检索、生成与端到端表现，并验证查询多样性与检索器多样性的取舍。

Method: 提出统一架构：基于LLM的五种查询改写+单一稀疏检索器，使用方差感知嵌套RRF融合；生成端采用证据片段抽取、双候选草拟与多评审校准选择的多阶段流程。

Result: Task A第1（nDCG@5 0.5776，较最强基线+20.5%），Task B第2（HM 0.7698）；查询多样性+单一对齐检索器优于多检索器集成。

Conclusion: 系统在检索与生成子任务取得优异排名，且端到端性能瓶颈主要在答案可回答性校准而非检索覆盖。
...
### [145] [Automatic End-to-End Data Integration using Large Language Models](https://arxiv.org/abs/2603.10547)
...
### [146] [End-to-End Chatbot Evaluation with Adaptive Reasoning and Uncertainty Filtering](https://arxiv.org/abs/2603.10570)
...
### [147] [MUNIChus: Multilingual News Image Captioning Benchmark](https://arxiv.org/abs/2603.10613)
...
### [148] [Disentangling Similarity and Relatedness in Topic Models](https://arxiv.org/abs/2603.10619)
...
### [149] [Making Bielik LLM Reason (Better): A Field Report](https://arxiv.org/abs/2603.10640)
...
### [150] [HeartAgent: An Autonomous Agent System for Explainable Differential Diagnosis in Cardiology](https://arxiv.org/abs/2603.10764)
...
### [151] [mAceReason-Math: A Dataset of High-Quality Multilingual Math Problems Ready For RLVR](https://arxiv.org/abs/2603.10767)
*Konstantin Dobler,Simon Lehnerer,Federico Scozzafava,Jonathan Janke,Mohamed Ali*

Main category: cs.CL

TL;DR: 提出并发布mAceReason-Math：面向RLVR的高难度数学题多语言译文数据集，覆盖14种语言、每种>1万样本，填补多语言RLVR训练与评测空缺。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR研究与数据集过于英语中心，已有多语言数据未针对RLVR与当前模型能力，难度不足，无法提供有效训练信号。

Method: 从为RLVR精心构建的AceReason-Math语料中选取高难度数学题，进行多语言高质量翻译与清洗优化，覆盖14种语言，每种语言超1万样本，并公开发布。

Result: 构建并发布覆盖14种语言、每种语言超过1万样本的高质量多语言数学题数据集mAceReason-Math。

Conclusion: 论文发布了mAceReason-Math多语言高质量译文数据集，用于推动多语言RLVR研究与评测。
...
### [152] [Word Recovery in Large Language Models Enables Character-Level Tokenization Robustness](https://arxiv.org/abs/2603.10771)
*Zhipeng Yang,Shu Yang,Lijie Hu,Di Wang*

Main category: cs.CL

TL;DR: LLM能从字符级输入重建规范词（词恢复），该机制由同词字符注意力支撑，移除相关子空间或注意力会显著损害性能。


<details>
  <summary>Details</summary>
Motivation: 解释LLM在非规范字符级分词下仍表现稳健的机制。

Method: 提出基于解码的词恢复检测方法；通过移除隐藏态对应子空间进行因果检验；做细粒度注意力分析并在早层掩蔽组内注意力。

Result: 隐藏态可重构规范词级身份；去除词恢复子空间会降低任务性能；组内注意力对词恢复与性能至关重要。

Conclusion: 词恢复是LLM在字符级输入下保持稳健性的关键机制，依赖同词字符的组内注意力。
...
### [153] [Large Language Models as Annotators for Machine Translation Quality Estimation](https://arxiv.org/abs/2603.10775)
...
### [154] [Interpretable Chinese Metaphor Identification via LLM-Assisted MIPVU Rule Script Generation: A Comparative Protocol Study](https://arxiv.org/abs/2603.10784)
...
### [155] [LuxBorrow: From Pompier to Pompjee, Tracing Borrowing in Luxembourgish](https://arxiv.org/abs/2603.10789)
...
### [156] [Multilingual Reasoning Gym: Multilingual Scaling of Procedural Reasoning Environments](https://arxiv.org/abs/2603.10793)
*Konstantin Dobler,Simon Lehnerer,Federico Scozzafava,Jonathan Janke,Mohamed Ali*
...
### [157] [PivotAttack: Rethinking the Search Trajectory in Hard-Label Text Attacks via Pivot Words](https://arxiv.org/abs/2603.10842)
...
### [158] [SiDiaC-v.2.0: Sinhala Diachronic Corpus Version 2.0](https://arxiv.org/abs/2603.10861)
...
### [160] [From Images to Words: Efficient Cross-Modal Knowledge Distillation to Language Models from Black-box Teachers](https://arxiv.org/abs/2603.10877)
...
### [161] [GLM-OCR Technical Report](https://arxiv.org/abs/2603.10910)
...
TL;DR: GLM-OCR是一个0.9B轻量文档多模态模型：用MTP把逐字解码改成多token并行预测，配合“版面分析+区域并行识别”两阶段流水线，在多项OCR/文档理解任务上实现高精度与高吞吐，兼顾边缘与大规模部署。


<details>
  <summary>Details</summary>
Motivation: 真实场景文档理解需要同时满足高精度与高效率，但传统自回归OCR解码逐token生成，吞吐低、部署成本高，难以兼顾边缘设备与生产系统需求。

Method: 模型由0.4B的CogViT视觉编码器与0.5B的GLM语言解码器组成；在解码端引入Multi-Token Prediction（每步预测多个token）以提升OCR确定性任务吞吐；系统端采用两阶段流程：先用PP-DocLayout-V3做版面分析，再并行进行区域级识别与结构化生成。

Result: 在公开基准与工业场景中，GLM-OCR在文档解析、文本/公式转写、表格结构恢复、关键信息抽取等任务上达到有竞争力或SOTA水平；同时保持较低计算与内存开销。

Conclusion: GLM-OCR通过紧凑的0.9B多模态架构、MTP多Token预测与两阶段系统设计，在准确率与效率之间取得了优良折中，适合边缘部署与工业级大规模文档理解。
...
### [162] [LLM2Vec-Gen: Generative Embeddings from Large Language Models](https://arxiv.org/abs/2603.10913)
...
### [163] [Beyond the Illusion of Consensus: From Surface Heuristics to Knowledge-Grounded Evaluation in LLM-as-a-Judge](https://arxiv.org/abs/2603.11027)
...
### [165] [Human Presence Detection via Wi-Fi Range-Filtered Doppler Spectrum on Commodity Laptops](https://arxiv.org/abs/2603.10845)
...
### [166] [Social Knowledge for Cross-Domain User Preference Modeling](https://arxiv.org/abs/2603.10148)
...
### [167] [Sublinear-Time Reconfiguration of Programmable Matter with Joint Movements](https://arxiv.org/abs/2603.10720)
...
### [168] [Where Do Flow Semantics Reside? A Protocol-Native Tabular Pretraining Paradigm for Encrypted Traffic Classification](https://arxiv.org/abs/2603.10051)
...
### [170] [Towards Intelligent Spectrum Management: Spectrum Demand Estimation Using Graph Neural Networks](https://arxiv.org/abs/2603.10802)
...
### [172] [Explainable LLM Unlearning Through Reasoning](https://arxiv.org/abs/2603.09980)
...
### [173] [MoE-SpAc: Efficient MoE Inference Based on Speculative Activation Utility in Heterogeneous Edge Scenarios](https://arxiv.org/abs/2603.09983)
...
### [175] [Personalized Group Relative Policy Optimization for Heterogenous Preference Alignment](https://arxiv.org/abs/2603.10009)
...
### [176] [Taming Score-Based Denoisers in ADMM: A Convergent Plug-and-Play Framework](https://arxiv.org/abs/2603.10281)
...
### [177] [Variance-Aware Adaptive Weighting for Diffusion Model Training](https://arxiv.org/abs/2603.10391)
*Nanlong Sun,Lei Shi*

Main category: cs.LG

TL;DR: 用损失方差驱动的自适应权重平衡噪声级别训练，提升扩散模型性能与稳定性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在不同噪声级别的训练动态不平衡，导致优化低效与学习不稳定。

Method: 从log-SNR层面的损失方差入手，提出基于观测方差分布动态调整训练权重的自适应加权策略。

Result: 在CIFAR-10/100上较标准训练取得更低FID且随机种子间性能方差更小，分析与消融表明训练更稳定。

Conclusion: 方差感知自适应加权可平衡不同噪声级别的训练动态并稳定优化，从而提升扩散模型表现。
...
### [178] [Unlearning the Unpromptable: Prompt-free Instance Unlearning in Diffusion Models](https://arxiv.org/abs/2603.10445)
...
### [179] [Historical Consensus: Preventing Posterior Collapse via Iterative Selection of Gaussian Mixture Priors](https://arxiv.org/abs/2603.10935)
...
### [184] [HTMuon: Improving Muon via Heavy-Tailed Spectral Correction](https://arxiv.org/abs/2603.10067)
...
### [186] [Dissecting Chronos: Sparse Autoencoders Reveal Causal Feature Hierarchies in Time Series Foundation Models](https://arxiv.org/abs/2603.10071)
*Anurag Mishra*
...
### [187] [CLIPO: Contrastive Learning in Policy Optimization Generalizes RLVR](https://arxiv.org/abs/2603.10101)
...
### [189] [ReMix: Reinforcement routing for mixtures of LoRAs in LLM finetuning](https://arxiv.org/abs/2603.10160)
...
### [190] [Marginals Before Conditionals](https://arxiv.org/abs/2603.10074)
*Mihir Sahasrabudhe*
...
### [192] [KernelSkill: A Multi-Agent Framework for GPU Kernel Optimization](https://arxiv.org/abs/2603.10085)
...
### [194] [Tackling Length Inflation Without Trade-offs: Group Relative Reward Rescaling for Reinforcement Learning](https://arxiv.org/abs/2603.10535)
*Zichao Li,Jie Lou,Fangchen Dong,Zhiyuan Fan,Mengjie Ren,Hongyu Lin,Xianpei Han,Debing Zhang,Le Sun,Yaojie Lu,Xing Yu*

Main category: cs.LG

TL;DR: 提出GR^3的乘性长度控制与正则化/校准机制，在不损失性能的情况下显著抑制RL训练中的长度膨胀。


<details>
  <summary>Details</summary>
Motivation: 强化学习训练的大模型出现长度膨胀，现有加性惩罚或启发式门控难以通用且易引入优化捷径。

Method: 提出GR^3，将长度控制建模为与奖励相关的乘性重缩放，并加入组相对正则化与优势感知校准以动态调整长度预算、保留优势信号。

Result: 在RLHF与RLVR设置下，训练动态与下游性能接近GRPO，同时显著降低长度膨胀并优于SOTA基线。

Conclusion: GR^3通过乘性重缩放与正则化在不牺牲优化的前提下显著缓解长度膨胀，整体表现优于现有长度正则化方法。
...
### [195] [Reinforcement Learning with Conditional Expectation Reward](https://arxiv.org/abs/2603.10624)
...
### [197] [$V_{0.5}$: Generalist Value Model as a Prior for Sparse RL Rollouts](https://arxiv.org/abs/2603.10848)
*Yi-Kai Zhang,Yueqing Sun,Hongyan Hao,Qi Gu,Xunliang Cai,De-Chuan Zhan,Han-Jia Ye*

Main category: cs.LG

TL;DR: V_{0.5}用统计检验驱动的动态预算将价值模型先验与稀疏采样融合，降低MSE、稳定梯度并在多基准上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: RLVR中基线构造影响策略梯度稳定性；现有价值模型虽高效但有系统偏差，稀疏采样均值方差高，需要兼顾效率与低方差的稳健基线。

Method: 提出V_{0.5}基线：将预训练价值模型预测作为先验，与稀疏rollout的经验均值自适应融合；引入实时统计检验评估先验可靠性，并据此动态追加rollout预算以平衡方差与偏差。

Result: 在六个数学推理基准上优于GRPO与DAPO，收敛更快，性能提升超过10%，即使组大小4的极端稀疏也保持稳定。

Conclusion: V_{0.5}通过融合先验价值模型与稀疏采样均值并动态分配采样预算，显著降低基线估计MSE，在极稀疏设置下仍能提供稳定梯度并提升性能。
...
### [198] [Equivariant Asynchronous Diffusion: An Adaptive Denoising Schedule for Accelerated Molecular Conformation Generation](https://arxiv.org/abs/2603.10093)
...
### [199] [Hardware Efficient Approximate Convolution with Tunable Error Tolerance for CNNs](https://arxiv.org/abs/2603.10100)
...
### [200] [TOSSS: a CVE-based Software Security Benchmark for Large Language Models](https://arxiv.org/abs/2603.10969)
*Marc Damie,Murat Bilgehan Ertan,Domenico Essoussi,Angela Makhanu,Gaëtan Peter,Roos Wensveen*

Main category: cs.LG

TL;DR: 提出TOSSS基准，用CVE驱动的二选一任务评估LLM安全选码能力，14个模型得分0.48–0.89，可作为安全评测补充指标。


<details>
  <summary>Details</summary>
Motivation: LLM广泛用于软件工程，但其安全能力未知且可能引入新漏洞，需要系统性评测。

Method: 构建TOSSS基准：基于CVE数据库生成安全/漏洞代码二选一任务，对14个开源与闭源模型在C/C++与Java上评测并计算0-1安全分数。

Result: 模型安全分数范围约0.48到0.89，表现差异明显。

Conclusion: TOSSS可作为补充性安全基准，量化模型在选择安全代码片段上的能力，并适合集成到模型评测报告中。
...
### [201] [Mashup Learning: Faster Finetuning by Remixing Past Checkpoints](https://arxiv.org/abs/2603.10156)
...
### [205] [Optimal Expert-Attention Allocation in Mixture-of-Experts: A Scalable Law for Dynamic Model Design](https://arxiv.org/abs/2603.10379)
...
### [206] [On the Learning Dynamics of Two-layer Linear Networks with Label Noise SGD](https://arxiv.org/abs/2603.10397)
...
### [208] [Domain-Adaptive Health Indicator Learning with Degradation-Stage Synchronized Sampling and Cross-Domain Autoencoder](https://arxiv.org/abs/2603.10430)
*Jungho Choo,Hanbyeol Park,Gawon Lee,Yunkyung Park,Hyerim Bae*

Main category: cs.LG

TL;DR: 用阶段同步采样与大核自编码跨注意力模型改进跨域HI建模，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有HI建模在工况分布不匹配下对齐失效，且小核1D-CNN难以捕获长程时序依赖。

Method: 提出DSSBS进行退化阶段同步批采样，并设计CAFLAE结合大核时序特征提取与跨注意力实现跨域对齐表示学习。

Result: 在韩国防系统与XJTU-SY轴承数据集上平均性能提升24.1%。

Conclusion: 该框架通过阶段一致采样与大核时序特征融合显著提升跨域健康指标建模效果。
...
### [209] [The Curse and Blessing of Mean Bias in FP4-Quantized LLM Training](https://arxiv.org/abs/2603.10444)
...
TL;DR: 低比特训练不稳主要由秩一均值偏置导致；通过均值减除即可在低成本下显著提升FP4训练稳定性与性能。


<details>
  <summary>Details</summary>
Motivation: 低比特训练中块量化受极值影响导致长尾语义被压缩，训练不稳定且性能下降，需要找出主因并提供轻量稳定化方法。

Method: 分析LLM表示的谱各向异性与量化动态范围膨胀机制，定位主导不稳定性的秩一均值分量，并在训练中引入源级均值减除；与SVD类谱方法对比其稳定性与硬件成本。

Result: 均值移除可显著缩小FP4与BF16的损失差距并恢复下游性能，且仅需简单归约与标准量化核。

Conclusion: 该工作指出低比特训练中的主要不稳定性源于激活的秩一均值偏置，简单的均值消除即可显著改善稳定性与性能。
...
### [210] [UAV-MARL: Multi-Agent Reinforcement Learning for Time-Critical and Dynamic Medical Supply Delivery](https://arxiv.org/abs/2603.10528)
...
### [211] [SCORE: Replacing Layer Stacking with Contractive Recurrent Depth](https://arxiv.org/abs/2603.10544)
*Guillaume Godin*
...
### [214] [CUPID: A Plug-in Framework for Joint Aleatoric and Epistemic Uncertainty Estimation with a Single Model](https://arxiv.org/abs/2603.10745)
*Xinran Xu,Xiuyi Fan*
...
### [216] [Protein Counterfactuals via Diffusion-Guided Latent Optimization](https://arxiv.org/abs/2603.10811)
...
### [218] [Dynamics-Predictive Sampling for Active RL Finetuning of Large Reasoning Models](https://arxiv.org/abs/2603.10887)
...
### [219] [LookaheadKV: Fast and Accurate KV Cache Eviction by Glimpsing into the Future without Generation](https://arxiv.org/abs/2603.10899)
...
### [221] [JEDI: Jointly Embedded Inference of Neural Dynamics](https://arxiv.org/abs/2603.10489)
...
### [222] [One Model, Many Skills: Parameter-Efficient Fine-Tuning for Multitask Code Analysis](https://arxiv.org/abs/2603.09978)
...
### [223] [STADA: Specification-based Testing for Autonomous Driving Agents](https://arxiv.org/abs/2603.10940)
...
### [224] [Safety Under Scaffolding: How Evaluation Conditions Shape Measured Safety](https://arxiv.org/abs/2603.10044)
*David Gringras*
...
### [225] [Toward Epistemic Stability: Engineering Consistent Procedures for Industrial LLM Hallucination Reduction](https://arxiv.org/abs/2603.10047)
...
### [226] [DUCTILE: Agentic LLM Orchestration of Engineering Analysis in Product Development Practice](https://arxiv.org/abs/2603.10249)
...
### [227] [Artificial Intelligence as a Catalyst for Innovation in Software Engineering](https://arxiv.org/abs/2603.10994)
...
### [228] [The Orthogonal Vulnerabilities of Generative AI Watermarks: A Comparative Empirical Benchmark of Spatial and Latent Provenance](https://arxiv.org/abs/2603.10323)
...
### [229] [Naïve Exposure of Generative AI Capabilities Undermines Deepfake Detection](https://arxiv.org/abs/2603.10504)
...
### [230] [SBOMs into Agentic AIBOMs: Schema Extensions, Agentic Orchestration, and Reproducibility Evaluation](https://arxiv.org/abs/2603.10057)
...
### [231] [Tool Receipts, Not Zero-Knowledge Proofs: Practical Hallucination Detection for AI Agents](https://arxiv.org/abs/2603.10060)
*Abhinaba Basu*
...
### [232] [ADVERSA: Measuring Multi-Turn Guardrail Degradation and Judge Reliability in Large Language Models](https://arxiv.org/abs/2603.10068)
...
### [233] [Why LLMs Fail: A Failure Analysis and Partial Success Measurement for Automated Security Patch Generation](https://arxiv.org/abs/2603.10072)
*Amir Al-Maamari*
...
### [234] [TASER: Task-Aware Spectral Energy Refine for Backdoor Suppression in UAV Swarms Decentralized Federated Learning](https://arxiv.org/abs/2603.10075)
*Sizhe Huang,Shujie Yang*
...
### [235] [Amnesia: Adversarial Semantic Layer Specific Activation Steering in Large Language Models](https://arxiv.org/abs/2603.10080)
...
### [236] [Multi-Stream Perturbation Attack: Breaking Safety Alignment of Thinking LLMs Through Concurrent Task Interference](https://arxiv.org/abs/2603.10091)
*Fan Yang*
...
### [237] [Compatibility at a Cost: Systematic Discovery and Exploitation of MCP Clause-Compliance Vulnerabilities](https://arxiv.org/abs/2603.10163)
...
### [238] [MCP-in-SoS: Risk assessment framework for open-source MCP servers](https://arxiv.org/abs/2603.10194)
...
### [239] [Detecting and Eliminating Neural Network Backdoors Through Active Paths with Application to Intrusion Detection](https://arxiv.org/abs/2603.10641)
...
### [240] [Repurposing Backdoors for Good: Ephemeral Intrinsic Proofs for Verifiable Aggregation in Cross-silo Federated Learning](https://arxiv.org/abs/2603.10692)
*Xian Qin,Xue Yang,Xiaohu Tang*

Main category: cs.CR

TL;DR: 用可遗忘的内嵌信号替代重型密码学证明，实现与安全聚合兼容的轻量完整性验证，检测效果好且大模型上极快。


<details>
  <summary>Details</summary>
Motivation: 现有安全聚合仅保证机密性，缺乏聚合完整性；基于ZKP/HE的可验证方案计算开销大且随模型规模扩展性差。

Method: 利用后门注入在模型参数中嵌入可验证信号，借助灾难性遗忘使信号短期可验证且随训练衰减；设计随机单验证者审计框架，与安全聚合兼容，避免信号碰撞且无需可信第三方。

Result: 在SVHN、CIFAR-10/100上对恶意服务器具有高检测概率；在ResNet-18上较加密基线加速超过1000×。

Conclusion: 提出基于“内在证明”的轻量可验证聚合架构，在保持安全聚合匿名性的同时实现高效完整性审计，并具备可扩展性。
...
### [241] [ID-LoRA: Identity-Driven Audio-Video Personalization with In-Context LoRA](https://arxiv.org/abs/2603.10256)
*Aviad Dahan,Moran Yanuka,Noa Kraicer,Lior Wolf,Raja Giryes*

Main category: cs.SD

TL;DR: ID-LoRA通过负时间位置与身份引导，在同一扩散模型中联合生成外观与声音，少量数据下显著提升语音相似度与风格一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频个性化将视觉与音频分离，音频无法与画面同步，且文本难以控制说话风格与声学环境，需联合建模外观与声音。

Method: 在LTX-2音视频扩散骨干上引入参数高效的In-Context LoRA；用负时间位置将参考与生成token在RoPE中分离；采用身份引导的CFG变体增强说话人特征。

Result: 相较Kling 2.6 Pro，ID-LoRA在人类偏好测试中语音相似度优选73%、说话风格优选65%；跨环境说话人相似度提升24%，条件差异越大优势越明显；约3K训练对、单GPU即可训练。

Conclusion: ID-LoRA在单次生成中联合个性化外观与声音，并以较少数据获得更高偏好与相似度，具有有效性与可扩展性。
...
### [242] [MoXaRt: Audio-Visual Object-Guided Sound Interaction for XR](https://arxiv.org/abs/2603.10465)
...
### [243] [AlphaFlowTSE: One-Step Generative Target Speaker Extraction via Conditional AlphaFlow](https://arxiv.org/abs/2603.10701)
...
### [244] [Towards Robust Speech Deepfake Detection via Human-Inspired Reasoning](https://arxiv.org/abs/2603.10725)
...
### [245] [Speaker Verification with Speech-Aware LLMs: Evaluation and Augmentation](https://arxiv.org/abs/2603.10827)
...
### [246] [When Fine-Tuning Fails and when it Generalises: Role of Data Diversity and Mixed Training in LLM-based TTS](https://arxiv.org/abs/2603.10904)
...
### [247] [Calibration-Reasoning Framework for Descriptive Speech Quality Assessment](https://arxiv.org/abs/2603.10175)
...
### [248] [Speech Codec Probing from Semantic and Phonetic Perspectives](https://arxiv.org/abs/2603.10371)
...
### [249] [G-STAR: End-to-End Global Speaker-Tracking Attributed Recognition](https://arxiv.org/abs/2603.10468)
...
### [250] [OmniGuide: Universal Guidance Fields for Enhancing Generalist Robot Policies](https://arxiv.org/abs/2603.10052)
...
### [253] [AR-VLA: True Autoregressive Action Expert for Vision-Language-Action Models](https://arxiv.org/abs/2603.10126)
...
### [254] [Cross-Hand Latent Representation for Vision-Language-Action Models](https://arxiv.org/abs/2603.10158)
...
### [255] [Dance2Hesitate: A Multi-Modal Dataset of Dancer-Taught Hesitancy for Understandable Robot Motion](https://arxiv.org/abs/2603.10166)
...
### [257] [Autonomous Search for Sparsely Distributed Visual Phenomena through Environmental Context Modeling](https://arxiv.org/abs/2603.10174)
...
### [258] [Octopus-inspired Distributed Control for Soft Robotic Arms: A Graph Neural Network-Based Attention Policy with Environmental Interaction](https://arxiv.org/abs/2603.10198)
...
### [259] [Perceptive Hierarchical-Task MPC for Sequential Mobile Manipulation in Unstructured Semi-Static Environments](https://arxiv.org/abs/2603.10227)
...
### [260] [Hierarchical Task Model Predictive Control for Sequential Mobile Manipulation Tasks](https://arxiv.org/abs/2603.10232)
...
### [261] [Degeneracy-Resilient Teach and Repeat for Geometrically Challenging Environments Using FMCW Lidar](https://arxiv.org/abs/2603.10248)
...
### [262] [From Prior to Pro: Efficient Skill Mastery via Distribution Contractive RL Finetuning](https://arxiv.org/abs/2603.10263)
...
### [263] [Design of a Robot-Assisted Chemical Dialysis System](https://arxiv.org/abs/2603.10264)
...
### [264] [Update-Free On-Policy Steering via Verifiers](https://arxiv.org/abs/2603.10282)
...
### [265] [SteadyTray: Learning Object Balancing Tasks in Humanoid Tray Transport via Residual Reinforcement Learning](https://arxiv.org/abs/2603.10306)
...
### [266] [PC-Diffuser: Path-Consistent Capsule CBF Safety Filtering for Diffusion-Based Trajectory Planner](https://arxiv.org/abs/2603.10330)
*Eugene Ku,Yiwei Lyu*
...
### [267] [Adaptive Manipulation Potential and Haptic Estimation for Tool-Mediated Interaction](https://arxiv.org/abs/2603.10352)
...
### [268] [Few-Shot Adaptation to Non-Stationary Environments via Latent Trend Embedding for Robotics](https://arxiv.org/abs/2603.10373)
...
### [269] [ScanDP: Generalizable 3D Scanning with Diffusion Policy](https://arxiv.org/abs/2603.10390)
...
### [272] [Rethinking Gaussian Trajectory Predictors: Calibrated Uncertainty for Safe Planning](https://arxiv.org/abs/2603.10407)
...
### [273] [COHORT: Hybrid RL for Collaborative Large DNN Inference on Multi-Robot Systems Under Real-Time Constraints](https://arxiv.org/abs/2603.10436)
...
### [274] [AsyncMDE: Real-Time Monocular Depth Estimation via Asynchronous Spatial Memory](https://arxiv.org/abs/2603.10438)
...
### [275] [KnowDiffuser: A Knowledge-Guided Diffusion Planner with LM Reasoning and Prior-Informed Trajectory Initialization](https://arxiv.org/abs/2603.10441)
...
### [276] [DiT4DiT: Jointly Modeling Video Dynamics and Actions for Generalizable Robot Control](https://arxiv.org/abs/2603.10448)
...
### [278] [DepthCache: Depth-Guided Training-Free Visual Token Merging for Vision-Language-Action Model Inference](https://arxiv.org/abs/2603.10469)
...
### [279] [BinWalker: Development and Field Evaluation of a Quadruped Manipulator Platform for Sustainable Litter Collection](https://arxiv.org/abs/2603.10529)
...
### [281] [Safety-critical Control Under Partial Observability: Reach-Avoid POMDP meets Belief Space Control](https://arxiv.org/abs/2603.10572)
...
### [282] [Recover to Predict: Progressive Retrospective Learning for Variable-Length Trajectory Prediction](https://arxiv.org/abs/2603.10597)
...
### [284] [AdaClearGrasp: Learning Adaptive Clearing for Zero-Shot Robust Dexterous Grasping in Densely Cluttered Environments](https://arxiv.org/abs/2603.10616)
*Zixuan Chen,Wenquan Zhang,Jing Fang,Ruiming Zeng,Zhixuan Xu,Yiwen Hou,Xinke Wang,Jieqi Shi,Jing Huo,Yang Gao*

Main category: cs.RO

TL;DR: 用VLM决策清理或直接抓取，RL实现零样本灵巧抓取，闭环重规划；在新基准与真实实验中显著提升杂乱环境抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 密集杂乱环境中直接抓取易受遮挡与接触不稳影响而失败，单纯清理又可能牺牲安全性，需要自适应选择清理或直接抓取。

Method: 提出闭环决策-执行框架，VLM结合视觉与语言生成高层计划骨架并调用原子技能；训练基于相对手-物距离表示的强化学习灵巧抓取策略；执行中视觉反馈监测失败并触发重规划。

Result: 在Clutter-Bench仿真（210任务）与仿真到真实（18场景）评测中，方法在高杂乱度下提升抓取成功率。

Conclusion: 所提AdaClearGrasp在密集杂乱场景中显著提升抓取成功率，闭环重规划机制增强鲁棒性。
...
### [285] [Interleaving Scheduling and Motion Planning with Incremental Learning of Symbolic Space-Time Motion Abstractions](https://arxiv.org/abs/2603.10651)
...
### [286] [STM32-Based Smart Waste Bin for Hygienic Disposal Using Embedded Sensing and Automated Control](https://arxiv.org/abs/2603.10660)
...
### [287] [Dynamic Modeling and Attitude Control of a Reaction-Wheel-Based Low-Gravity Bipedal Hopper](https://arxiv.org/abs/2603.10670)
...
### [288] [OnFly: Onboard Zero-Shot Aerial Vision-Language Navigation toward Safety and Efficiency](https://arxiv.org/abs/2603.10682)
...
### [290] [Parallel-in-Time Nonlinear Optimal Control via GPU-native Sequential Convex Programming](https://arxiv.org/abs/2603.10711)
...
### [291] [FutureVLA: Joint Visuomotor Prediction for Vision-Language-Action Model](https://arxiv.org/abs/2603.10712)
...
### [292] [MAVEN: A Meta-Reinforcement Learning Framework for Varying-Dynamics Expertise in Agile Quadrotor Maneuvers](https://arxiv.org/abs/2603.10714)
...
### [293] [ASTER: Attitude-aware Suspended-payload Quadrotor Traversal via Efficient Reinforcement Learning](https://arxiv.org/abs/2603.10715)
...
### [294] [Semantic Landmark Particle Filter for Robot Localisation in Vineyards](https://arxiv.org/abs/2603.10847)
...
### [295] [GRACE: A Unified 2D Multi-Robot Path Planning Simulator & Benchmark for Grid, Roadmap, And Continuous Environments](https://arxiv.org/abs/2603.10858)
*Chuanlong Zang,Anna Mannucci,Isabelle Barz,Philipp Schillinger,Florian Lier,Wolfgang Hönig*

Main category: cs.RO

TL;DR: GRACE统一多层表示与评测，实证比较规划器并量化保真度-效率权衡，提升跨表示研究可比性。


<details>
  <summary>Details</summary>
Motivation: 现有MAPF/MRMP工具要么在简化假设下可扩展但可比性弱，要么高保真但缺少可比的评测与仪器化，需要统一平台实现透明、可复现的跨表示比较。

Method: 提出一个统一的2D模拟器+基准GRACE，在网格、路标图和连续空间三种抽象层级上实例化同一任务，使用显式、可复现的算子和通用评测协议，并在公共地图与代表性规划器上实证比较。

Result: 实验显示在共享实例集上可进行可比比较，并量化表示与保真度的权衡：MRMP在高保真下求解速度较慢，而网格/路标图规划器可扩展到更大规模。

Conclusion: GRACE通过统一表示、执行与评测，使跨表示的多机器人规划研究更可比，并促进实践转化。
...
### [296] [FG-CLTP: Fine-Grained Contrastive Language Tactile Pretraining for Robotic Manipulation](https://arxiv.org/abs/2603.10871)
...
### [297] [RL-Augmented MPC for Non-Gaited Legged and Hybrid Locomotion](https://arxiv.org/abs/2603.10878)
...
### [298] [A gripper for flap separation and opening of sealed bags](https://arxiv.org/abs/2603.10890)
...
### [299] [Contact Coverage-Guided Exploration for General-Purpose Dexterous Manipulation](https://arxiv.org/abs/2603.10971)
...
### [300] [Learning Adaptive Force Control for Contact-Rich Sample Scraping with Heterogeneous Materials](https://arxiv.org/abs/2603.10979)
...
### [301] [PPGuide: Steering Diffusion Policies with Performance Predictive Guidance](https://arxiv.org/abs/2603.10980)
...
### [302] [AMB-DSGDN: Adaptive Modality-Balanced Dynamic Semantic Graph Differential Network for Multimodal Emotion Recognition](https://arxiv.org/abs/2603.10043)
...
### [303] [EvoSchema: Towards Text-to-SQL Robustness Against Schema Evolution](https://arxiv.org/abs/2603.10697)
*Tianshu Zhang,Kun Qian,Siddhartha Sahai,Yuan Tian,Shaddy Garg,Huan Sun,Yunyao Li*

Main category: cs.DB

TL;DR: 提出 EvoSchema 评测模式演化下文本到 SQL 的鲁棒性，表级变更影响更大，基准训练可显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到 SQL 模型多在静态模式上训练，面对模式演化性能下降；现有工作对演化扰动覆盖不足且缺乏可控评测。

Method: 提出 EvoSchema 基准与模式演化分类法，包含列级与表级共十类扰动；在多种开源与闭源 LLM 上进行评测，并通过在该基准上的训练测试鲁棒性提升。

Result: 表级扰动对性能影响显著大于列级；在 EvoSchema 多样模式上训练的模型平均鲁棒性更强。

Conclusion: EvoSchema 提供了系统化的模式演化基准，能揭示并提升文本到 SQL 模型在真实动态数据库中的鲁棒性。
...
### [304] [RedFuser: An Automatic Operator Fusion Framework for Cascaded Reductions on AI Accelerators](https://arxiv.org/abs/2603.10026)
...
### [305] [The DMA Streaming Framework: Kernel-Level Buffer Orchestration for High-Performance AI Data Paths](https://arxiv.org/abs/2603.10030)
*Marco Graziano*
...
### [306] [Architecture-Aware LLM Inference Optimization on AMD Instinct GPUs: A Comprehensive Benchmark and Deployment Study](https://arxiv.org/abs/2603.10031)
*Athos Georgiou*
...
### [307] [HTM-EAR: Importance-Preserving Tiered Memory with Hybrid Routing under Saturation](https://arxiv.org/abs/2603.10032)
*Shubham Kumar Singh*
...
### [309] [Multi-Agent Memory from a Computer Architecture Perspective: Visions and Challenges Ahead](https://arxiv.org/abs/2603.10062)
...
### [310] [A Diffusion Analysis of Policy Gradient for Stochastic Bandits](https://arxiv.org/abs/2603.10219)
*Tor Lattimore*
...
### [311] [Simulation-in-the-Reasoning (SiR): A Conceptual Framework for Empirically Grounded AI in Autonomous Transportation](https://arxiv.org/abs/2603.10294)
*Wuping Xin*
...
### [312] [ARCHE: Autoregressive Residual Compression with Hyperprior and Excitation](https://arxiv.org/abs/2603.10188)
...
### [313] [The Epistemic Support-Point Filter: Jaynesian Maximum Entropy Meets Popperian Falsification](https://arxiv.org/abs/2603.10065)
*Moriba Kemessia Jah*
...
### [314] [Deep Randomized Distributed Function Computation (DeepRDFC): Neural Distributed Channel Simulation](https://arxiv.org/abs/2603.10750)
...
### [315] [Learning from Radio using Variational Quantum RF Sensing](https://arxiv.org/abs/2603.10239)
*Ivana Nikoloska*
...
### [316] [Quantum entanglement provides a competitive advantage in adversarial games](https://arxiv.org/abs/2603.10289)
...
### [317] [A Systematic Study of Pseudo-Relevance Feedback with LLMs](https://arxiv.org/abs/2603.11008)
...
### [318] [Does Reasoning Make Search More Fair? Comparing Fairness in Reasoning and Non-Reasoning Rerankers](https://arxiv.org/abs/2603.10332)
...
### [319] [Beyond Interleaving: Causal Attention Reformulations for Generative Recommender Systems](https://arxiv.org/abs/2603.10369)
*Hailing Cheng*
...
Abstract: Generative Recommender Systems (GR) increasingly model user behavior as a sequence generation task by interleaving item and action tokens. While effective, this formulation introduces significant structural and computational inefficiencies: it doubles sequence length, incurs quadratic overhead, and relies on implicit attention to recover the causal relationship between an item and its associated action. Furthermore, interleaving heterogeneous tokens forces the Transformer to disentangle semantically incompatible signals, leading to increased attention noise and reduced representation this work, we propose a principled reformulation of generative recommendation that aligns sequence modeling with underlying causal structures and attention theory. We demonstrate that current interleaving mechanisms act as inefficient proxies for similarity-weighted action pooling. To address this, we introduce two novel architectures that eliminate interleaved dependencies to reduce sequence complexity by 50%: Attention-based Late Fusion for Actions (AttnLFA) and Attention-based Mixed Value Pooling (AttnMVP). These models explicitly encode the $i_n \rightarrow a_n$ causal dependency while preserving the expressive power of Transformer-based sequence evaluate our framework on large-scale product recommendation data from a major social network. Experimental results show that AttnLFA and AttnMVP consistently outperform interleaved baselines, achieving evaluation loss improvements of 0.29% and 0.80%, and significant gains in Normalized Entropy (NE). Crucially, these performance gains are accompanied by training time reductions of 23% and 12%, respectively. Our findings suggest that explicitly modeling item-action causality provides a superior design paradigm for scalable and efficient generative ranking.

</details>


### [320] [Modeling Stage-wise Evolution of User Interests for News Recommendation](https://arxiv.org/abs/2603.10471)
...
### [321] [Code-Space Response Oracles: Generating Interpretable Multi-Agent Policies with Large Language Models](https://arxiv.org/abs/2603.10098)
*Daniel Hennes,Zun Li,John Schultz,Marc Lanctot*

Main category: cs.GT

TL;DR: 用LLM生成代码策略替代RL黑箱，CSRO在保持性能的同时提升可解释性与策略多样性。


<details>
  <summary>Details</summary>
Motivation: PSRO等方法依赖黑箱深度RL策略，难以解释、信任与调试，需要可解释的策略生成框架。

Method: 将最佳响应计算改写为代码生成任务，用LLM替代RL oracle；采用零样本提示、迭代改写与AlphaEvolve分布式进化优化。

Result: CSRO在多个设定下达到与基线竞争的表现，同时生成可读、可解释且多样的策略。

Conclusion: CSRO通过LLM生成可解释代码策略，在多智能体学习中实现与基线相当的性能并提供多样可解释策略。
...
### [322] [Conversational AI-Enhanced Exploration System to Query Large-Scale Digitised Collections of Natural History Museums](https://arxiv.org/abs/2603.10285)
...
### [323] [NasoVoce: A Nose-Mounted Low-Audibility Speech Interface for Always-Available Speech Interaction](https://arxiv.org/abs/2603.10324)
...
### [324] [Reactive Writers: How Co-Writing with AI Changes How We Engage with Ideas](https://arxiv.org/abs/2603.10374)
...
### [325] [A Platform-Agnostic Multimodal Digital Human Modelling Framework: Neurophysiological Sensing in Game-Based Interaction](https://arxiv.org/abs/2603.10680)
...
### [326] [Risk-Adjusted Harm Scoring for Automated Red Teaming for LLMs in Financial Services](https://arxiv.org/abs/2603.10807)
...
### [327] [Agentic Control Center for Data Product Optimization](https://arxiv.org/abs/2603.10133)
...
### [328] [Hybrid Self-evolving Structured Memory for GUI Agents](https://arxiv.org/abs/2603.10291)
...
### [329] [HEAL: Hindsight Entropy-Assisted Learning for Reasoning Distillation](https://arxiv.org/abs/2603.10359)
...
### [330] [Beyond Scalars: Evaluating and Understanding LLM Reasoning via Geometric Progress and Stability](https://arxiv.org/abs/2603.10384)
...
### [331] [Verbalizing LLM's Higher-order Uncertainty via Imprecise Probabilities](https://arxiv.org/abs/2603.10396)
...
### [332] [Resource-constrained Amazons chess decision framework integrating large language models and graph attention](https://arxiv.org/abs/2603.10512)
*Tianhao Qian,Zhuoxuan Li,Jinde Cao,Xinli Shi,Hanjie Liu,Leszek Rutkowski*

Main category: cs.AI

TL;DR: 用图注意力+MCTS+遗传算法并借助LLM生成数据，实现资源受限下的高性能亚马逊棋AI，显著超越基线与教师模型。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习在资源受限环境中依赖大量数据与算力，难以部署；需要探索弱到强泛化的新范式。

Method: 提出轻量混合框架：Graph Attention Autoencoder引导多步MCTS，Stochastic Graph Genetic Algorithm优化评估信号，并用GPT‑4o‑mini生成合成训练数据，在弱监督与噪声数据上学习。

Result: 在10×10亚马逊棋上，决策准确率较基线提升15%–56%，并在N=30节点胜率45.0%，N=50节点胜率66.5%，超过教师模型GPT‑4o‑mini。

Conclusion: 该工作证明在资源受限条件下，结合图结构推理与LLM生成的混合框架可从通用模型中演化出高性能博弈AI。
...
### [333] [Adaptive RAN Slicing Control via Reward-Free Self-Finetuning Agents](https://arxiv.org/abs/2603.10564)
...
### [334] [CUAAudit: Meta-Evaluation of Vision-Language Models as Auditors of Autonomous Computer-Use Agents](https://arxiv.org/abs/2603.10577)
...
### [335] [Does LLM Alignment Really Need Diversity? An Empirical Study of Adapting RLVR Methods for Moral Reasoning](https://arxiv.org/abs/2603.10588)
...
### [336] [Trajectory-Informed Memory Generation for Self-Improving Agent Systems](https://arxiv.org/abs/2603.10600)
...
### [337] [FAME: Formal Abstract Minimal Explanation for Neural Networks](https://arxiv.org/abs/2603.10661)
...
### [338] [Emulating Clinician Cognition via Self-Evolving Deep Clinical Research](https://arxiv.org/abs/2603.10677)
...
### [339] [Nurture-First Agent Development: Building Domain-Expert AI Agents Through Conversational Knowledge Crystallization](https://arxiv.org/abs/2603.10808)
*Linghao Zhang*
...
### [340] [A Hybrid Knowledge-Grounded Framework for Safety and Traceability in Prescription Verification](https://arxiv.org/abs/2603.10891)
...
### [341] [Daily-Omni: Towards Audio-Visual Reasoning with Temporal Alignment across Modalities](https://arxiv.org/abs/2505.17862)
...
### [342] [Intrinsic Numerical Robustness and Fault Tolerance in a Neuromorphic Algorithm for Scientific Computing](https://arxiv.org/abs/2603.10246)
...
### [343] [Assessing Cognitive Biases in LLMs for Judicial Decision Support: Virtuous Victim and Halo Effects](https://arxiv.org/abs/2603.10016)
*Sierra S. Liu*
...
### [344] [DeliberationBench: A Normative Benchmark for the Influence of Large Language Models on Users' Views](https://arxiv.org/abs/2603.10018)
...
### [345] [Prompts and Prayers: the Rise of GPTheology](https://arxiv.org/abs/2603.10019)
...
<details>
  <summary>Details</summary>
Motivation: 未提供可安全分析的动机细节。

Method: 未提供可安全分析的方法细节。

Result: 未提供可安全分析的结果细节。

Conclusion: 该内容涉及宗教相关主题，按要求不可输出具体分析。
...
### [346] [Defining AI Models and AI Systems: A Framework to Resolve the Boundary Problem](https://arxiv.org/abs/2603.10023)
...
### [347] [A Governance and Evaluation Framework for Deterministic, Rule-Based Clinical Decision Support in Empiric Antibiotic Prescribing](https://arxiv.org/abs/2603.10027)
...
### [348] [How to Count AIs: Individuation and Liability for AI Agents](https://arxiv.org/abs/2603.10028)
...
### [349] [RCTs & Human Uplift Studies: Methodological Challenges and Practical Solutions for Frontier AI Evaluation](https://arxiv.org/abs/2603.11001)
...
### [350] [FC-4DFS: Frequency-controlled Flexible 4D Facial Expression Synthesizing](https://arxiv.org/abs/2603.10326)
...
### [351] [Landmark Guided 4D Facial Expression Generation](https://arxiv.org/abs/2603.10337)
...
### [352] [Exact Interpolation under Noise: A Reproducible Comparison of Clough-Tocher and Multiquadric RBF Surfaces](https://arxiv.org/abs/2603.10590)
*Mirkan Emir Sancak*

Main category: cs.GR

TL;DR: 作者在同一可复现实验框架下公平比较三次插值和RBF：无噪声时两者都很准；有噪声时都会过拟合并降泛化，但三次插值更稳。对环境工程而言，噪声测量不应直接丢弃，可通过插值重建有物理意义的过程行为。


<details>
  <summary>Details</summary>
Motivation: 现有多变量曲面插值比较常受评估协议不一致影响，难以公平判断方法优劣；同时工程数据常含噪声，需明确插值方法在噪声下的稳健性及其实用价值。

Method: 构建同一合成函数族，采用统一的切片式训练/测试协议，对三次插值与RBF进行公平对比；分别在无噪声与有噪声两种工况下，用RMSE、MAE、R²评估，并通过固定随机种子、重复划分与bootstrap给出不确定性摘要，基于SciPy/NumPy单脚本实现全流程复现。

Result: 无噪声条件下，两类插值均达到高精度，但优势随输出而变化；有噪声条件下，两者的“精确通过样本点”特性均导致对噪声节点过拟合、测试性能下降；在本文实验中，三次插值相对RBF表现出更好的稳定性。

Conclusion: 在统一且可复现的评估框架下，三次插值与RBF在无噪声场景均表现优异；有噪声时精确插值会过拟合并损害泛化，其中三次插值在该实验设定下更稳定。工程上不应默认剔除噪声测量，而应通过结构化插值挖掘其物理信息。
...
### [353] [TopGen: Learning Structural Layouts and Cross-Fields for Quadrilateral Mesh Generation](https://arxiv.org/abs/2603.10606)
